...
import { ItemSourceXPRPG, MeleeSource } from "@item/data";
...
export function isActorSource(docSource: CompendiumSource): docSource is ActorSourceXPRPG {
    return (
        "system" in docSource && isObject(docSource.system) && "items" in docSource && Array.isArray(docSource.items)
    );
}

export function isItemSource(docSource: CompendiumSource): docSource is ItemSourceXPRPG {
    return (
        "system" in docSource &&
        "type" in docSource &&
        !("text" in docSource) &&
...
export class CompendiumPack {
    ...
    static LINK_PATTERNS = {
        ...
        compendium: /@Compendium\[xprpg\.(?<packName>[^.]+)\.(?<docName>[^\]]+)\]\{?/g,
        uuid: /@UUID\[Compendium\.xprpg\.(?<packName>[^.]+)\.(?<docName>[^\]]+)\]\{?/g,
    };
            ...
            if ("img" in docSource && typeof docSource.img === "string") {
                ...
                for (const imgPath of imgPaths) {
                    ...
                    const repoImgPath = path.resolve(
                        process.cwd(),
                        "static",
                        decodeURIComponent(imgPath).replace("systems/xprpg/", "")
                    );
            ...
            if ("type" in docSource) {
                if (docSource.type === "script") {
                    ...
                    for (const item of attackItems) {
                        const { linkedWeapon } = item.flags?.xprpg ?? {};
    ...
    static convertRuleUUIDs(
        ...
        const toIDRef = (uuid: string): string => {
            const match = /(?<=^Compendium\.xprpg\.)([^.]+)\.(.+)$/.exec(uuid);
    ...
    #assertSizeValid(source: ActorSourceXPRPG | ItemSourceXPRPG): void {
