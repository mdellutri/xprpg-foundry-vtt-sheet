...
import { ActionItemSource, ItemSourceXPRPG, MeleeSource, SpellSource } from "@item/data";
...
const packsPath = (() => {
    try {
        ...
        return path.join(config.dataPath, "Data", "systems", config.systemName ?? "xprpg", "packs");
    } catch (_error) {
...
type CompendiumDocumentXPRPG = ActorXPRPG | ItemXPRPG | JournalEntry | MacroXPRPG | RollTable;
type PackEntry = CompendiumDocumentXPRPG["_source"];
...
let lastActor: ActorSourceXPRPG | undefined;
    ...
    for (const key in docSource) {
        if (key === "_id") {
            ...
            docSource.img &&= docSource.img.replace(
                "https://assets.forge-vtt.com/bazaar/systems/xprpg/assets/",
                "systems/xprpg/"
            ) as ImageFilePath;

            if (isObject(docSource.flags?.xprpg) && Object.keys(docSource.flags.xprpg).length === 0) {
                delete docSource.flags.xprpg;
            }
            ...
            if ("type" in docSource) {
                ...
                if (isActorSource(docSource)) {
                    ...
                    if (isPhysicalData(docSource)) {
                        ...
                        if (isFeat && docSource.img === "systems/xprpg/icons/default-icons/feat.svg") {
                            docSource.img = "systems/xprpg/icons/features/feats/feats.webp";
                        }
...
function sanitizeDocument<T extends PackEntry>(docSource: T, { isEmbedded } = { isEmbedded: false }): T {
    // Clear non-core/xprpg flags
    for (const flagScope in docSource.flags) {
        if (!["core", "xprpg"].includes(flagScope) || !isEmbedded) {
    ...
    if (!isEmbedded) {
        ...
        if (isItemSource(docSource)) {
            ...
            docSource.flags = docSource.type === "condition" ? { xprpg: { condition: true } } : {};
            if (isPhysicalData(docSource)) {
...
function sortDataItems(docSource: PackEntry): ItemSourceXPRPG[] {
    ...
    const groupedItems: Map<string, Set<ItemSourceXPRPG>> = new Map();

    // Separate the data items into type collections.
    for (const item of ownedItems) {
        if (!groupedItems.has(item.type)) {
            groupedItems.set(item.type, new Set<ItemSourceXPRPG>());
        }
    ...
    const sortedItems: ItemSourceXPRPG[] = Array(ownedItems.length);
    let itemIndex = 0;
...
function sortAttacks(docName: string, attacks: Set<ItemSourceXPRPG>): ItemSourceXPRPG[] {
...
function sortItemsWithOverrides(
    docName: string,
    actions: ItemSourceXPRPG[],
    overrides: Map<RegExp, "top" | "bottom">
): ItemSourceXPRPG[] {
    const topActions: ItemSourceXPRPG[] = [];
    const middleActions: ItemSourceXPRPG[] = [];
    const bottomActions: ItemSourceXPRPG[] = [];
...
function sortSpellcastingEntries(docName: string, actions: Set<ItemSourceXPRPG>): ItemSourceXPRPG[] {
...
function sortInteractions(docName: string, actions: ItemSourceXPRPG[]): ItemSourceXPRPG[] {
...
function sortDefensiveActions(docName: string, actions: ItemSourceXPRPG[]): ItemSourceXPRPG[] {
    const overrides: Map<RegExp, "top" | "bottom"> = new Map([
...
function sortOffensiveActions(docName: string, actions: ItemSourceXPRPG[]): ItemSourceXPRPG[] {
    const overrides: Map<RegExp, "top" | "bottom"> = new Map([
...
function sortActions(docName: string, actions: Set<ItemSourceXPRPG>): ItemSourceXPRPG[] {
    ...
    const actionsMap: Map<string, ItemSourceXPRPG[]> = new Map([
        ["interaction", []],
        ["defensive", []],
        ["offensive", []],
        ["other", []],
...
function sortSpells(spells: Set<ItemSourceXPRPG>): SpellSource[] {
