import { FeatXPRPG, ItemXPRPG } from "@item";
...
class CharacterFeats extends Collection<FeatCategory> {
    ...
    constructor(private actor: CharacterXPRPG) {
        super();

        const classFeatSlots = actor.class?.grantedFeatSlots;
        const skillPrepend = (() => {
            if (actor.background && Object.keys(actor.background.system.items).length) {
                return [{ id: actor.background?.id, label: game.i18n.localize("XPRPG.FeatBackgroundShort") }];
            }
            return [];
        })();
        ...
        this.createCategory({
            id: "ancestryfeature",
            label: "XPRPG.FeaturesAncestryHeader",
            supported: ["ancestryfeature"],
        });
        this.createCategory({
            id: "classfeature",
            label: "XPRPG.FeaturesClassHeader",
            supported: ["classfeature"],
        });
        this.createCategory({
            id: "ancestry",
            label: "XPRPG.FeatAncestryHeader",
        ...
        const classTrait =
            (classSlug ?? "") in CONFIG.XPRPG.featTraits
                ? classSlug
                : `hb_${classSlug}` in CONFIG.XPRPG.featTraits
                ? `hb_${classSlug}`
                : null;

        this.createCategory({
            id: "class",
            label: "XPRPG.FeatClassHeader",
        ...
        if (game.settings.get("xprpg", "dualClassVariant")) {
            this.createCategory({
                id: "dualclass",
                label: "XPRPG.FeatDualClassHeader",
                supported: ["class"],
                slots: [1, ...evenLevels],
            });
        }
        ...
        if (game.settings.get("xprpg", "freeArchetypeVariant")) {
            this.createCategory({
                id: "archetype",
                label: "XPRPG.FeatArchetypeHeader",
                supported: ["class"],
                slots: evenLevels,
            });
        }
        ...
        this.createCategory({
            id: "skill",
            label: "XPRPG.FeatSkillHeader",
            supported: ["skill"],
            slots: [...skillPrepend, ...(classFeatSlots?.skill ?? [])],
        });
        this.createCategory({
            id: "general",
            label: "XPRPG.FeatGeneralHeader",
            supported: ["general", "skill"],
            slots: classFeatSlots?.general ?? [],
        });

        // Add campaign feats if enabled
        if (game.settings.get("xprpg", "campaignFeats")) {
            this.createCategory({ id: "campaign", label: "XPRPG.FeatCampaignHeader" });
        }
    }
    ...
    #combineGrants(feat: FeatXPRPG): { feat: FeatXPRPG; grants: GrantedFeat[] } {
        const getGrantedItems = (grants: Record<string, ItemGrantData>): GrantedFeat[] => {
            return Object.values(grants).flatMap((grant) => {
                ...
                return (item?.isOfType("feat") && !item.system.location) || item?.isOfType("heritage")
                    ? { feat: item, grants: getGrantedItems(item.flags.xprpg.itemGrants) }
                    : [];
            });
        };

        return { feat, grants: getGrantedItems(feat.flags.xprpg.itemGrants) };
    }

    /** Inserts a feat into the character. If category is empty string, its a bonus feat */
    async insertFeat(feat: FeatXPRPG, options: { categoryId: string; slotId?: string }): Promise<ItemXPRPG[]> {
        ...
        if (options.categoryId !== "bonus" && !category) {
            ...
            if (badCategory) {
                ui.notifications.warn(
                    game.i18n.format("XPRPG.Item.Feat.Warning.InvalidCategory", {
                        item: feat.name,
        ...
        if (!alreadyHasFeat && (isFeatValidInSlot || !location)) {
            ...
            const label = game.i18n.localize(location && category?.label ? category.label : "XPRPG.FeatBonusHeader");
            ui.notifications.info(game.i18n.format("XPRPG.Item.Feat.Info.Added", { item: feat.name, category: label }));
        }
    ...
    assignFeats(): void {
        ...
        for (const feat of feats) {
            if (feat.flags.xprpg.grantedBy && !feat.system.location) {
                const granter = this.actor.items.get(feat.flags.xprpg.grantedBy.id);
                if (granter?.isOfType("feat")) continue;
            }

            // We don't handle certain feat types here
            if (["xpsboon", "deityboon", "curse"].includes(feat.featType)) {
                continue;
            }
            ...
            if (slot && slot.feat) {
                console.debug(`XPRPG System | Multiple feats with same index: ${feat.name}, ${slot.feat.name}`);
...
class FeatCategory {
    ...
    constructor(actor: CharacterXPRPG, options: FeatCategoryOptions) {
    ...
    isFeatValid(feat: FeatXPRPG): boolean {
