...
import { ActionItemXPRPG, ItemXPRPG } from "@item";
import { isSpellConsumable } from "@item/consumable/spell-consumables";
import { ItemSourceXPRPG, LoreData } from "@item/data";
...
import { CreatureSheetXPRPG } from "../creature/sheet";
...
class CharacterSheetXPRPG extends CreatureSheetXPRPG<CharacterXPRPG> {
    ...
    override get template(): string {
        ...
        return `systems/xprpg/templates/actors/character/${template}.hbs`;
    }
    ...
    override async getData(options?: ActorSheetOptions): Promise<CharacterSheetData> {
        ...
        for (const [key, proficiency] of proficiencies) {
            ...
            const label = ((): string => {
                if (objectHasKey(CONFIG.XPRPG.martialSkills, key)) {
                    return CONFIG.XPRPG.martialSkills[key];
                }
                if (objectHasKey(CONFIG.XPRPG.weaponCategories, key)) {
                    return CONFIG.XPRPG.weaponCategories[key];
                }
                if (Array.isArray(groupMatch)) {
                    const weaponGroup = groupMatch[1] as WeaponGroup;
                    return CONFIG.XPRPG.weaponGroups[weaponGroup];
                }
                if (Array.isArray(baseWeaponMatch)) {
                    const baseWeapon = baseWeaponMatch[1] as BaseWeaponType;
                    return LocalizeXPRPG.translations.XPRPG.Weapon.Base[baseWeapon];
                }
        ...
        sheetData.data.resources.heroPoints.hover = game.i18n.format(
            this.actor.heroPoints.value === 1 ? "XPRPG.HeroPointRatio.One" : "XPRPG.HeroPointRatio.Many",
        ...
        const classDCs = allClassDCs
            ...
            .map(
                (classDC): ClassDCSheetData => ({
                    ...
                    hover: CONFIG.XPRPG.proficiencyLevels[classDC.rank],
                    rankSlug: PROFICIENCY_RANKS[classDC.rank],
                    rankName: game.i18n.format(`XPRPG.ProficiencyLevel${classDC.rank}`),
                })
            )
        ...
        // Spell Details
        sheetData.magicTraditions = CONFIG.XPRPG.magicTraditions;
        sheetData.preparationType = CONFIG.XPRPG.preparationType;
        ...
        sheetData.data.attributes.perception.rankName = game.i18n.format(
            `XPRPG.ProficiencyLevel${sheetData.data.attributes.perception.rank}`
        );
        ...
        for (const save of Object.values(sheetData.data.saves)) {
            save.rankName = game.i18n.format(`XPRPG.ProficiencyLevel${save.rank}`);
        }

        // limiting the amount of characters for the save labels
        for (const save of Object.values(sheetData.data.saves)) {
            save.short = game.i18n.format(`XPRPG.Saves${save.label}Short`);
        }
        ...
        sheetData.hasStamina = game.settings.get("xprpg", "staminaVariant") > 0;
        ...
        const flags = this.actor.flags.xprpg;
        ...
        sheetData.tabVisibility = deepClone(this.actor.flags.xprpg.sheetTabs);
    ...
    override async prepareItems(sheetData: ActorSheetDataXPRPG<CharacterXPRPG>): Promise<void> {
        const actorData = sheetData.actor;

        // Actions
        type AnnotatedAction = RawObject<ActionItemXPRPG["data"]> & {
            encounter?: boolean;
            exploration?: boolean;
            downtime?: boolean;
        };
        ...
        for (const itemData of sheetData.items) {
            ...
            // Lore Skills
                ...
                itemData.system.hover = CONFIG.XPRPG.proficiencyLevels[(itemData.system.proficient || {}).value];
        ...
        // Assign and return
        actorData.xpsBoons = this.actor.xpsBoons;
    ...
    disableInitiativeButton(): void {
        this.element
            ...
            .addClass("disabled")
            .attr({ title: game.i18n.localize("XPRPG.Encounter.NoActiveEncounter") });
    }
    ...
    override activateListeners($html: JQuery): void {
        ...
        $actions.find(".toggle-unready-strikes").on("click", () => {
            this.actor.setFlag("xprpg", "showUnreadyStrikes", !this.actor.flags.xprpg.showUnreadyStrikes);
        });
        ...
        $strikesList.find(".melee-icon").tooltipster({
            content: game.i18n.localize("XPRPG.Item.Weapon.MeleeUsage.Label"),
            position: "left",
            theme: "crb-hover",
        });
        ...
        $html
            ...
            .on("click", (event) => {
                game.xprpg.actions.restForTheNight({ event, actors: [this.actor] });
            });
        ...
        $craftingOptions.on("click", async (event) => {
            ...
            flags.forEach(async (flag) => {
                await this.actor.setFlag("xprpg", flag, false);
            });
        });
        ...
        $craftingTab.find("a[data-action=quick-add]").on("click", async (event) => {
            ...
            if (entries.length === 0) {
                ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.NoEligibleEntry"));
            }
        });
        ...
        $formulas.find("a[data-action=craft-item]").on("click", async (event) => {
            ...
            if (prepared === "true") {
                ...
                if (expendedState === "true") {
                    ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.Alerts.FormulaExpended"));
                    return;
                }
                ...
                if (!craftingEntry) throw ErrorXPRPG("Crafting entry not found");
                await craftingEntry.toggleFormulaExpended(Number(index), itemUuid);
            }

            if (this.actor.flags.xprpg.quickAlchemy) {
            ...
            if (this.actor.flags.xprpg.freeCrafting) {
        ...
        $formulas.find("[data-action=enter-quantity]").on("change", async (event) => {
            ...
            const formula = this.knownFormulas[itemUUID ?? ""];
            if (!formula) throw ErrorXPRPG("Formula not found");
    ...
    async #onClickBrowseFeats(event: MouseEvent): Promise<void> {
        ...
        for (const filterCode of checkboxesFilterCodes) {
            ...
            if (!(filterType && value)) {
                ...
                throw ErrorXPRPG(`Invalid filter value for opening the compendium browser:\n${codesData}`);
            }
        ...
        await game.xprpg.compendiumBrowser.openTab("feat", filter);
    }
    ...
    #onChangeAdjustStat(selectElem: HTMLSelectElement): void {
        ...
        if (typeof currentValue !== "number" || Number.isNaN(selectedValue)) {
            throw ErrorXPRPG("Actor property not found");
        }
        ...
        if (newValue !== getProperty(clone, propertyKey)) {
            ui.notifications.warn("XPRPG.ErrorMessage.MinimumProfLevelSetByFeatures", { localize: true });
    ...
    async #onClickAdjustStat(event: JQuery.TriggeredEvent<HTMLElement>): Promise<void> {
        ...
        if (typeof currentValue !== "number") throw ErrorXPRPG("Actor property not found");
    ...
    async #onChangeAdjustItemStat(event: JQuery.TriggeredEvent<HTMLElement>): Promise<void> {
        ...
        if (!item) throw ErrorXPRPG("Item not found");
        ...
        const newValue = ((): number | undefined => {
            if (item.isOfType("spellcastingEntry")) {
                ...
                throw ErrorXPRPG("Item not recognized");
            }
        })();
        ...
        if (newValue !== getProperty(item, propertyKey)) {
            ui.notifications.warn(game.i18n.localize("XPRPG.ErrorMessage.MinimumProfLevelSetByFeatures"));
        }
    }
    ...
    #onAddCustomModifier(event: JQuery.ClickEvent<HTMLElement, undefined, HTMLElement>): void {
        ...
        const name =
            ...
            game.i18n.localize(`XPRPG.ModifierType.${type}`);
        const errors: string[] = [];
        if (!stat.trim()) {
            // This is a UI error rather than a user error
            throw ErrorXPRPG("No character attribute found");
        }
    ...
    protected override async _onDropItem(event: ElementDragEvent, data: DropCanvasItemDataXPRPG): Promise<ItemXPRPG[]> {
        ...
        if (!item) throw ErrorXPRPG("Unable to create item from drop data!");
    ...
    protected override async _onDrop(event: ElementDragEvent): Promise<boolean | void> {
        ...
        if (isObject<Record<string, unknown>>(dropData.xprpg) && dropData.xprpg.type === "CraftingFormula") {
            ...
            if (dropContainerType === "craftingEntry") {
                ...
                const craftingFormulas = await this.actor.getCraftingFormulas();
                const uuid = dropData.xprpg.itemUuid;
...
interface CharacterSheetXPRPG extends CreatureSheetXPRPG<CharacterXPRPG> {
    getStrikeFromDOM(target: HTMLElement): CharacterStrike | null;
}

export { CharacterSheetXPRPG };
