...
import { ConsumableXPRPG, PhysicalItemXPRPG, SpellXPRPG } from "@item";
...
interface Costs {
    reductionPerDay: CoinsXPRPG;
    materials: CoinsXPRPG;
    itemPrice: CoinsXPRPG;
    lostMaterials: CoinsXPRPG;
}
...
async function prepStrings(costs: Costs, item: PhysicalItemXPRPG) {
    const rollData = item.getRollData();

    return {
        reductionPerDay: costs.reductionPerDay.toString(),
        materialCost: game.i18n.format("XPRPG.Actions.Craft.Details.PayMaterials", {
            cost: costs.materials.toString(),
        }),
        itemCost: game.i18n.format("XPRPG.Actions.Craft.Details.PayFull", {
            cost: costs.itemPrice.toString(),
        }),
        lostMaterials: game.i18n.format("XPRPG.Actions.Craft.Details.LostMaterials", {
...
function calculateCosts(
    ...
    const itemPrice = CoinsXPRPG.fromPrice(item.price, quantity);
    const materialCosts = itemPrice.scale(0.5);
    const lostMaterials = new CoinsXPRPG();
    const reductionPerDay = new CoinsXPRPG();
...
export async function craftItem(
    ...
    if (!result) {
        ui.notifications.warn(game.i18n.localize("XPRPG.Actions.Craft.Warning.CantAddItem"));
        return;
    }

    await ChatMessageXPRPG.create({
        user: game.user.id,
        content: game.i18n.format("XPRPG.Actions.Craft.Information.ReceiveItem", {
...
export async function craftSpellConsumable(
    ...
    const validSpells = actor.itemTypes.spell
        ...
        .reduce((result, spell) => {
            ...
            return result;
        }, <Record<number, Embedded<SpellXPRPG>[]>>{});
    const content = await renderTemplate("systems/xprpg/templates/actors/crafting-select-spell-dialog.hbs", {
        spells: validSpells,
    });

    new Dialog({
        title: game.i18n.localize("XPRPG.Actions.Craft.SelectSpellDialog.Title"),
        content,
        buttons: {
            cancel: {
            ...
            craft: {
                ...
                label: game.i18n.localize("XPRPG.Actions.Craft.SelectSpellDialog.CraftButtonLabel"),
                callback: async ($dialog) => {
                    ...
                    return craftItem(new ConsumableXPRPG(item), itemQuantity, actor);
                },
            },
        },
        default: "craft",
...
export async function renderCraftingInline(
    ...
    return await renderTemplate("systems/xprpg/templates/chat/crafting-result.hbs", {
