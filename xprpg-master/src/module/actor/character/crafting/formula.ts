import { ConsumableXPRPG, PhysicalItemXPRPG, WeaponXPRPG } from "@item";
...
export class CraftingFormula implements CraftingFormulaData {
    ...
    constructor(
        ...
        this.dc =
            ...
            calculateDC(item.level, {
                rarity: item.rarity,
                proficiencyWithoutLevel: game.settings.get("xprpg", "proficiencyVariant") === "ProficiencyWithoutLevel",
            });
    ...
    get cost(): Coins {
        return CoinsXPRPG.fromPrice(this.price, this.batchSize);
    }
    ...
    get defaultBatchSize(): number {
        const { item } = this;
        const isMundaneAmmo = item instanceof ConsumableXPRPG && item.isAmmunition && !item.isMagical;
        const isConsumable =
            (item instanceof ConsumableXPRPG && item.category !== "wand") ||
            (item instanceof WeaponXPRPG && item.baseType === "alchemical-bomb");
