...
export class CraftingEntry implements Omit<CraftingEntryData, "parentItem"> {
    ...
    isDailyPrep: boolean;
    isPrepared: boolean;
    craftableItems: PredicateXPRPG;
    maxSlots: number;
    fieldDiscovery: PredicateXPRPG | null;
    batchSize?: number;
    fieldDiscoveryBatchSize?: number;
    maxItemLevel: number;
    parentItem: Embedded<ItemXPRPG>;

    constructor(actor: CharacterXPRPG, knownFormulas: CraftingFormula[], data: CraftingEntryData) {
        ...
        this.fieldDiscovery = data.fieldDiscovery ? new PredicateXPRPG(data.fieldDiscovery) : null;
        this.batchSize = data.batchSize;
        this.fieldDiscoveryBatchSize = data.fieldDiscoveryBatchSize;
        this.craftableItems = new PredicateXPRPG(data.craftableItems);
    ...
    checkEntryRequirements(formula: CraftingFormula, { warn = true } = {}): boolean {
        if (!!this.maxSlots && this.formulas.filter((f) => f !== null).length >= this.maxSlots) {
            if (warn) ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.Alerts.MaxSlots"));
            return false;
        }
        if (this.actor.level < formula.level) {
            if (warn) ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.Alerts.CharacterLevel"));
            return false;
        }
        if (formula.level > this.maxItemLevel) {
            if (warn) ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.Alerts.MaxItemLevel"));
            return false;
        }

        if (!this.craftableItems.test(formula.item.getRollOptions("item"))) {
            if (warn) {
                ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.Alerts.ItemMissingTraits"));
            }
            return false;
        }
