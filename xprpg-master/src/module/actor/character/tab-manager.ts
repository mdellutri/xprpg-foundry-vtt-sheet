...
export class PCSheetTabManager {
    constructor(public actor: CharacterXPRPG, public link: HTMLAnchorElement) {
        renderTemplate("systems/xprpg/templates/actors/character/manage-tabs.hbs").then((template) => {
            $(this.link).tooltipster({
                ...
                interactive: true,
                theme: "crb-hover",
                title: game.i18n.localize("XPRPG.TabManageTabsLabel"),
                trigger: "custom",
    ...
    static initialize(actor: CharacterXPRPG, link: HTMLAnchorElement): void {
        new this(actor, link);
    }

    /** Set each checkbox to be checked according to its corresponding tab visibility */
    private onReady(tooltip: HTMLElement): void {
        const tabVisibility: Record<string, boolean> = this.actor.flags.xprpg.sheetTabs;
    ...
    private handleOnChange(checkbox: HTMLInputElement, checkboxes: HTMLInputElement[]): void {
        checkbox.addEventListener("change", async () => {
            ...
            if (checkbox.checked) {
                ...
                await this.actor.update({ [`flags.xprpg.sheetTabs.-=${tabName}`]: null }, { render: false });
            } else {
                tab?.classList.add("to-hide");
                await this.actor.update({ [`flags.xprpg.sheetTabs.${tabName}`]: false }, { render: false });
            }
