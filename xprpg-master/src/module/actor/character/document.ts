import { CreatureXPRPG, FamiliarXPRPG } from "@actor";
...
import { DamageXPRPG, DamageRollContext } from "@system/damage";
import { DamageRoll } from "@system/damage/roll";
import { WeaponDamageXPRPG } from "@system/damage/weapon";
...
import { ErrorXPRPG, getActionGlyph, objectHasKey, sluggify, sortedStringify, traitSlugToObject } from "@util";
...
class CharacterXPRPG extends CreatureXPRPG {
    /** Core singular embeds for PCs */
    ancestry!: Embedded<AncestryXPRPG> | null;
    heritage!: Embedded<HeritageXPRPG> | null;
    background!: Embedded<BackgroundXPRPG> | null;
    class!: Embedded<ClassXPRPG> | null;
    deity!: Embedded<DeityXPRPG> | null;

    /** A cached reference to this PC's familiar */
    familiar: FamiliarXPRPG | null = null;

    feats!: CharacterFeats;
    xpsBoons!: FeatXPRPG[];
    deityBoonsCurses!: FeatXPRPG[];
    ...
    async getCraftingFormulas(): Promise<CraftingFormula[]> {
        ...
        return items
            .filter((item): item is PhysicalItemXPRPG => item instanceof PhysicalItemXPRPG)
            .map((item) => {
    ...
    async performDailyCrafting(): Promise<void> {
        ...
        if (reagentValue < 0) {
            ui.notifications.warn(game.i18n.localize("XPRPG.CraftingTab.Alerts.MissingReagents"));
            return;
        } else {
    ...
    override prepareBaseData(): void {
        ...
        // Flags
        const { flags } = this;
        flags.xprpg.favoredWeaponRank = 0;
        flags.xprpg.freeCrafting ??= false;
        flags.xprpg.quickAlchemy ??= false;
        flags.xprpg.sheetTabs = mergeObject(
            ...
            flags.xprpg.sheetTabs ?? {}
        );
        flags.xprpg.showBasicUnarmed ??= true;

        // Build selections: boosts and skill trainings
        const isGradual = game.settings.get("xprpg", "gradualBoostsVariant");
        ...
        for (const save of SAVE_TYPES) {
            saves[save] = {
                ability: CONFIG.XPRPG.savingThrowDefaultAbilities[save],
        ...
        // Size
        this.system.traits.size = new ActorSizeXPRPG({ value: "med" });
        ...
        const homebrewCategories = game.settings.get("xprpg", "homebrew.weaponCategories").map((tag) => tag.id);
    ...
    override prepareDerivedData(): void {
        ...
        game.xprpg.variantRules.AutomaticBonusProgression.concatModifiers(this);
        ...
        if (systemData.xps.levelBump) {
            const params = { slug: "level-bump", label: "XPRPG.PFS.LevelBump", modifier: 1 };
            statisticsModifiers.all.push(() => new ModifierXPRPG(params));
            statisticsModifiers.damage.push(() => new ModifierXPRPG(params));
        }
        ...
        // Calculate HP and SP
        {
            ...
            const modifiers = [new ModifierXPRPG("XPRPG.AncestryHP", ancestryHP, MODIFIER_TYPE.UNTYPED)];
            ...
            if (game.settings.get("xprpg", "staminaVariant")) {
                ...
                modifiers.push(new ModifierXPRPG("XPRPG.ClassHP", halfClassHp * this.level, MODIFIER_TYPE.UNTYPED));
            } else {
                modifiers.push(new ModifierXPRPG("XPRPG.ClassHP", classHP * this.level, MODIFIER_TYPE.UNTYPED));
            ...
            if (systemData.xps.levelBump) {
                ...
                stat.push(new ModifierXPRPG("XPRPG.PFS.LevelBump", hitPointsBump, MODIFIER_TYPE.UNTYPED));
            }
        ...
        this.traditions = Array.from(MAGIC_TRADITIONS).reduce((traditions, tradition) => {
            traditions[tradition] = new Statistic(this, {
                slug: tradition,
                label: CONFIG.XPRPG.magicTraditions[tradition],
        ...
        const { wornArmor, heldShield } = this;
        {
            ...
            stat.value += stat.totalModifier;
            stat.breakdown = [game.i18n.localize("XPRPG.ArmorClassBase")]
                .concat(
                    stat.modifiers
                        .filter((m) => m.enabled)
                        .map((m) => `${m.label} ${m.modifier < 0 ? "" : "+"}${m.modifier}`)
        ...
        if (heldShield?.speedPenalty) {
            const speedPenalty = new ModifierXPRPG(heldShield.name, heldShield.speedPenalty, MODIFIER_TYPE.UNTYPED);
        ...
        for (const saveType of SAVE_TYPES) {
            ...
            if (wornArmor?.system.resiliencyRune.value) {
                ...
                if (resilientBonus > 0 && wornArmor.isInvested) {
                    modifiers.push(new ModifierXPRPG(wornArmor.name, resilientBonus, MODIFIER_TYPE.ITEM));
                }
            }
            ...
            if (affectedByBulwark) {
                const slug = "bulwark";
                const bulwarkModifier = new ModifierXPRPG({
                    slug,
                    type: MODIFIER_TYPE.UNTYPED,
                    label: CONFIG.XPRPG.armorTraits.bulwark,
                    modifier: 3,
                ...
                reflexAdjustments.push({
                    slug: "dex",
                    predicate: new PredicateXPRPG("damaging-effect"),
                    suppress: true,
                });
            }
    ...
    private prepareSkills(): Record<SkillAbbreviation, CharacterSkillData> {
        ...
        const skills = Array.from(SKILL_ABBREVIATIONS).reduce((builtSkills, shortForm) => {
            ...
            if (skill.armor && typeof wornArmor?.checkPenalty === "number") {
                ...
                const armorCheckPenalty = new ModifierXPRPG({
                    slug,
                    label: "XPRPG.ArmorCheckPenalty",
            ...
            stat.roll = async (params: RollParameters): Promise<Rolled<CheckRoll> | null> => {
                ...
                const label = game.i18n.format("XPRPG.SkillCheckWithName", {
                    skillName: game.i18n.localize(CONFIG.XPRPG.skills[shortForm]),
                });
                const rollOptions = new Set(params.options ?? []);
        ...
        for (const loreItem of this.itemTypes.lore) {
            ...
            stat.roll = async (params: RollParameters): Promise<Rolled<CheckRoll> | null> => {
                ...
                const label = game.i18n.format("XPRPG.SkillCheckWithName", { skillName: loreItem.name });
                const rollOptions = new Set(params.options ?? []);
    ...
    override prepareSpeed(movementType: MovementType): CreatureSpeeds | (LabeledSpeed & StatisticModifier) | null {
        ...
        const modifierName = wornArmor?.name ?? "XPRPG.ArmorSpeedLabel";
    ...
    prepareFeats(): void {
        ...
        const campaignFeatSections = game.settings.get("xprpg", "campaignFeatSections");
        ...
        const feats = this.itemTypes.feat
            .filter((f) => ["xpsboon", "deityboon", "curse"].includes(f.featType))
            .sort((f1, f2) => f1.sort - f2.sort);
        for (const feat of feats) {
            if (feat.featType === "xpsboon") {
                this.xpsBoons.push(feat);
            } else {
    ...
    prepareClassDC(slug: string, classDC: Pick<ClassDCData, "label" | "ability" | "rank" | "primary">): Statistic {
        ...
        const classNames: Record<string, string | undefined> = CONFIG.XPRPG.classTraits;
    ...
    createAuxAction({ weapon, action, purpose, hands }: CreateAuxiliaryParams): AuxiliaryAction {
        ...
        return {
            label: game.i18n.localize(`XPRPG.Actions.${action}.${fullPurpose}.Title`),
            img: actionGlyph,
            execute: async (): Promise<void> => {
                ...
                const flavorAction = {
                    title: `XPRPG.Actions.${action}.Title`,
                    subtitle: `XPRPG.Actions.${action}.${fullPurpose}.Title`,
                ...
                const content = await renderTemplate(templates.content, {
                    imgPath: weapon.img,
                    message: game.i18n.format(`XPRPG.Actions.${action}.${fullPurpose}.Description`, {
                        actor: this.name,
                        weapon: weapon.name,
                    }),
                });
                ...
                await ChatMessageXPRPG.create({
                    content,
                    speaker: ChatMessageXPRPG.getSpeaker({ actor: this }),
                    flavor,
    ...
    prepareStrikes({ includeBasicUnarmed = true } = {}): CharacterStrike[] {
        ...
        const basicUnarmed = includeBasicUnarmed
            ? ((): Embedded<WeaponXPRPG> => {
                  const source: PreCreate<WeaponSource> & { system: { damage?: Partial<WeaponDamage> } } = {
                      _id: "xxXPRPGxUNARMEDxx",
                      name: game.i18n.localize("XPRPG.WeaponTypeUnarmed"),
                      type: "weapon",
                  ...
                  return new WeaponXPRPG(source, { parent: this, xprpg: { ready: true } }) as Embedded<WeaponXPRPG>;
              })()
            : null;
        ...
        const homebrewCategoryTags = game.settings.get("xprpg", "homebrew.weaponCategories");
    ...
    private prepareStrike(
        weapon: Embedded<WeaponXPRPG>,
        options: {
            categories: WeaponCategory[];
            ammos?: Embedded<ConsumableXPRPG>[];
        ...
        const equivalentWeapons: Record<string, string | undefined> = CONFIG.XPRPG.equivalentWeapons;
        ...
        const selectors = (() => {
            ...
            const abilityModifier = [...modifiers, ...extractModifiers(synthetics, baseSelectors, options)]
                .filter((m): m is ModifierXPRPG & { ability: AbilityString } => m.type === "ability")
            ...
            if (!abilityModifier) {
                console.warn(
                    `XPRPG System | No ability modifier was determined for attack roll with ${weapon.name} (${weapon.uuid})`
                );
        ...
        const ABP = game.xprpg.variantRules.AutomaticBonusProgression;

        if (weapon.group === "bomb" && !ABP.isEnabled(this)) {
            const attackBonus = Number(weapon.system.bonus?.value) || 0;
            if (attackBonus !== 0) {
                modifiers.push(new ModifierXPRPG("XPRPG.ItemBonusLabel", attackBonus, MODIFIER_TYPE.ITEM));
            }
        }
        ...
        if (weaponPotency) {
            modifiers.push(new ModifierXPRPG(weaponPotency.label, weaponPotency.bonus, weaponPotency.type));
        ...
        const action: CharacterStrike = mergeObject(strikeStat, {
            ...
            visible: weapon.slug !== "basic-unarmed" || this.flags.xprpg.showBasicUnarmed,
            glyph: "A",
            item: weapon,
            ...
            weaponTraits: Array.from(weaponTraits)
                .map((t) => traitSlugToObject(t, CONFIG.XPRPG.npcAttackTraits))
        ...
        action.traits = actionTraits.map((t) => traitSlugToObject(t, CONFIG.XPRPG.actionTraits));
        ...
        const checkName = game.i18n.format(
            weapon.isMelee ? "XPRPG.Action.Strike.MeleeLabel" : "XPRPG.Action.Strike.RangedLabel",
            { weapon: weapon.name }
        );

        const noMAPLabel = ((): string => {
            const strike = game.i18n.localize("XPRPG.WeaponStrikeLabel");
        ...
        const labels: [string, string, string] = [
            noMAPLabel,
            game.i18n.format("XPRPG.MAPAbbreviationLabel", { penalty: multipleAttackPenalty.map1 }),
            game.i18n.format("XPRPG.MAPAbbreviationLabel", { penalty: multipleAttackPenalty.map2 }),
        ];
        const checkModifiers = [
            (otherModifiers: ModifierXPRPG[]) => new CheckModifier(checkName, action, otherModifiers),
            (otherModifiers: ModifierXPRPG[]) =>
                new CheckModifier(checkName, action, [
                    ...otherModifiers,
                    new ModifierXPRPG(multipleAttackPenalty.label, multipleAttackPenalty.map1, MODIFIER_TYPE.UNTYPED),
                ]),
            (otherModifiers: ModifierXPRPG[]) =>
                new CheckModifier(checkName, action, [
                    ...otherModifiers,
                    new ModifierXPRPG(multipleAttackPenalty.label, multipleAttackPenalty.map2, MODIFIER_TYPE.UNTYPED),
                ]),
        ];

        action.variants = [0, 1, 2]
            .map((index): [string, (otherModifiers: ModifierXPRPG[]) => CheckModifier] => [
                labels[index],
                ...
                roll: async (params: AttackRollParams = {}): Promise<Rolled<CheckRoll> | null> => {
                    ...
                    if (weapon.requiresAmmo && params.consumeAmmo && !weapon.ammo) {
                        ui.notifications.warn(
                            game.i18n.format("XPRPG.Strike.Ranged.NoAmmo", { weapon: weapon.name, actor: this.name })
                        );
                        return null;
                    }
                    ...
                    if (context.self.item.isRanged && typeof context.target?.distance === "number") {
                        ...
                        if (context.target.distance > maxRange) {
                            ui.notifications.warn("XPRPG.Action.Strike.OutOfRange", { localize: true });
                            return null;
                        }
                    }
            ...
            action[method] = async (params: DamageRollParams = {}): Promise<string | Rolled<DamageRoll> | null> => {
                ...
                if (!context.self.item.dealsDamage) {
                    if (!params.getFormula) {
                        ui.notifications.warn("XPRPG.ErrorMessage.WeaponNoDamage", { localize: true });
                        return null;
                    }
                    return "";
                }
                ...
                const damage = await WeaponDamageXPRPG.calculate({
                ...
                if (params.getFormula) {
                    ...
                    return DamageXPRPG.roll(damage, damageContext, params.callback);
                }
            };
        }
    ...
    getStrikeDescription(weapon: WeaponXPRPG): { description: string; criticalSuccess: string; success: string } {
        const flavor = {
            description: "XPRPG.Strike.Default.Description",
            criticalSuccess: "XPRPG.Strike.Default.CriticalSuccess",
            success: "XPRPG.Strike.Default.Success",
        };
        const traits = weapon.traits;
        if (traits.has("unarmed")) {
            flavor.description = "XPRPG.Strike.Unarmed.Description";
            flavor.success = "XPRPG.Strike.Unarmed.Success";
        } else if ([...traits].some((trait) => trait.startsWith("thrown-") || trait === "combination")) {
            flavor.description = "XPRPG.Strike.Combined.Description";
            flavor.success = "XPRPG.Strike.Combined.Success";
        } else if (weapon.isMelee) {
            flavor.description = "XPRPG.Strike.Melee.Description";
            flavor.success = "XPRPG.Strike.Melee.Success";
        } else {
            flavor.description = "XPRPG.Strike.Ranged.Description";
            flavor.success = "XPRPG.Strike.Ranged.Success";
        }
        return flavor;
    }
    ...
    consumeAmmo(weapon: WeaponXPRPG, params: RollParameters): boolean {
        const ammo = weapon.ammo;
        if (!ammo) {
            return true;
        } else if (ammo.quantity < 1) {
            ui.notifications.warn(game.i18n.localize("XPRPG.ErrorMessage.NotEnoughAmmo"));
            return false;
        } else {
    ...
    async toggleInvested(itemId: string): Promise<boolean> {
        ...
        if (!item?.traits.has("invested")) {
            throw ErrorXPRPG("Unexpected error toggling item investment");
        }
    ...
    protected override async _preUpdate(
        ...
        for (const property of ["playerNumber", "characterNumber"] as const) {
            if (typeof changed.system?.xps?.[property] === "number") {
                const [min, max] = property === "playerNumber" ? [1, 9_999_999] : [2001, 9999];
                changed.system.xps[property] = Math.clamped(changed.system.xps[property] || 0, min, max);
            } else if (changed.system?.xps && changed.system.xps[property] !== null) {
                changed.system.xps[property] = this.system.xps[property] ?? null;
            }
        }
...
interface CharacterXPRPG extends CreatureXPRPG {
    readonly data: CharacterData;
    readonly system: CharacterSystemData;
    flags: CharacterFlags;
}

export { CharacterXPRPG };
