...
import { ConsumableXPRPG, ItemXPRPG, MeleeXPRPG, WeaponXPRPG } from "@item";
...
interface BaseActorSourceXPRPG<
    TType extends ActorType = ActorType,
    TSystemSource extends ActorSystemSource = ActorSystemSource
> extends foundry.data.ActorSource<TType, TSystemSource, ItemSourceXPRPG> {
    flags: DeepPartial<ActorFlagsXPRPG>;
    prototypeToken: PrototypeTokenSourceXPRPG;
}

interface BaseActorDataXPRPG<
    TActor extends ActorXPRPG = ActorXPRPG,
    TType extends ActorType = ActorType,
    TSource extends BaseActorSourceXPRPG<TType> = BaseActorSourceXPRPG<TType>
> extends Omit<BaseActorSourceXPRPG<TType, ActorSystemSource>, "effects" | "items" | "prototypeToken" | "system">,
        foundry.data.ActorData<TActor, ActiveEffectXPRPG, ItemXPRPG> {
    readonly type: TType;

    token: PrototypeTokenXPRPG;

    readonly _source: TSource;
}

interface ActorFlagsXPRPG extends foundry.data.ActorFlags {
    xprpg: {
...
interface StrikeData extends StatisticModifier {
    ...
    ammunition?: {
        compatible: ConsumableXPRPG[];
        incompatible: ConsumableXPRPG[];
        selected: {
            id: string;
            compatible: boolean;
        } | null;
...
interface PrototypeTokenSourceXPRPG extends foundry.data.PrototypeTokenSource {
...
interface PrototypeTokenXPRPG extends foundry.data.PrototypeToken {
    flags: foundry.data.PrototypeToken["flags"] & {
        xprpg: {
            linkToActorSize: boolean;
            autoscale: boolean;
