...
import { PredicateXPRPG, PredicateStatement } from "@system/predication";
...
abstract class IWRData<TType extends IWRType> {
    ...
    get applicationLabel(): string {
        ...
        return game.i18n
            .format(`XPRPG.Damage.IWR.CompositeLabel.${key}`, { type, ...exceptions, value: "" })
            .replace(/\s+/g, " ")
            .trim();
    }
    ...
    protected describe(iwrType: TType): PredicateStatement[] {
        ...
        switch (iwrType) {
            ...
            default: {
                if (iwrType in CONFIG.XPRPG.damageTypes) {
    ...
    get predicate(): PredicateXPRPG {
        ...
        return new PredicateXPRPG(statements);
    }
    ...
    /** Construct an object argument for Localization#format (see also XPRPG.Actor.IWR.CompositeLabel in en.json) */
...
class ImmunityData extends IWRData<ImmunityType> implements ImmunitySource {
    protected readonly typeLabels = CONFIG.XPRPG.immunityTypes;
...
class WeaknessData extends IWRData<WeaknessType> implements WeaknessSource {
    protected readonly typeLabels = CONFIG.XPRPG.weaknessTypes;
    ...
    get label(): string {
        ...
        const exceptions = this.createFormatData({ list: this.exceptions, prefix: "exception" });
        const key = `Exceptions${this.exceptions.length}DoubleVs0`;
        return game.i18n.format(`XPRPG.Damage.IWR.CompositeLabel.${key}`, { type, value: this.value, ...exceptions });
    }
...
class ResistanceData extends IWRData<ResistanceType> implements ResistanceSource {
    protected readonly typeLabels = CONFIG.XPRPG.resistanceTypes;
    ...
    get label(): string {
        ...
        return game.i18n.format(`XPRPG.Damage.IWR.CompositeLabel.${key}`, {
            type,
            value: this.value,
            ...exceptions,
            ...doubleVs,
    ...
    override get applicationLabel(): string {
        ...
        return game.i18n
            .format(`XPRPG.Damage.IWR.CompositeLabel.${key}`, {
                type,
                value: "",
                ...exceptions,
                ...doubleVs,
    ...
    getDoubledValue(damageDescription: Set<string>): number {
        ...
        const predicate = new PredicateXPRPG(this.doubleVs.flatMap((d) => this.describe(d)));
