...
import { ActorXPRPG, HitPointsSummary } from "../base";
import { TokenDimensions, VehicleData, VehicleSource, VehicleSystemData } from "./data";

class VehicleXPRPG extends ActorXPRPG {
    ...
    override prepareBaseData(): void {
        ...
        if (this.prototypeToken.flags?.xprpg?.linkToActorSize) {
    ...
    override prepareDerivedData(): void {
        ...
        if (this.hasCondition("broken")) {
            for (const selector of ["ac", "saving-throw"]) {
                ...
                const brokenModifier = new ModifierXPRPG({
                    slug: "broken",
                    label: "XPRPG.ConditionTypeBroken",
                    modifier: -2,
    ...
    private prepareSaves(): void {
        ...
        const fortitude = new Statistic(this, {
            slug: "fortitude",
            label: CONFIG.XPRPG.saves.fortitude,
            modifiers,
            domains,
            check: {
                type: "saving-throw",
    ...
    protected override async _preUpdate(
        ...
        if (this.prototypeToken.flags?.xprpg?.linkToActorSize) {
            ...
            if (canvas.scene) {
                ...
                await TokenDocumentXPRPG.updateDocuments(updates, { parent: canvas.scene });
            }
        }
    }
}

interface VehicleXPRPG extends ActorXPRPG {
