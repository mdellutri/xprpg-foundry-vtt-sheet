...
function createEmptySpellcastingEntry(actor: ActorXPRPG): Embedded<SpellcastingEntryXPRPG> {
    return new SpellcastingEntryXPRPG(
        {
            name: "Untitled",
        ...
        { actor }
    ) as Embedded<SpellcastingEntryXPRPG>;
}
...
class SpellcastingCreateAndEditDialog extends FormApplication<Embedded<SpellcastingEntryXPRPG>> {
    private actor: ActorXPRPG;

    constructor(object: ActorXPRPG | Embedded<SpellcastingEntryXPRPG>, options: Partial<FormApplicationOptions>) {
        super(
            object instanceof ActorXPRPG ? createEmptySpellcastingEntry(object) : object.clone({}, { keepId: true }),
            options
        );
        this.actor = object instanceof ActorXPRPG ? object : object.actor;
    }
    ...
    static override get defaultOptions(): FormApplicationOptions {
        ...
        options.classes = [];
        options.title = game.i18n.localize("XPRPG.SpellcastingSettings.Title");
        options.template = "systems/xprpg/templates/actors/spellcasting-dialog.hbs";
        options.width = 350;
    ...
    override async getData(): Promise<SpellcastingCreateAndEditDialogSheetData> {
        ...
        return {
            ...
            magicTraditions: CONFIG.XPRPG.magicTraditions,
            spellcastingTypes: CONFIG.XPRPG.preparationType,
        ...
        if (this.object.id === null) {
            updateData.name = (() => {
                const preparationType =
                    game.i18n.localize(CONFIG.XPRPG.preparationType[updateData.system.prepared.value]) ?? "";
                const magicTraditions: Record<string, string> = CONFIG.XPRPG.magicTraditions;
                ...
                if (this.object.isRitual || !traditionSpells) {
                    ...
                    return game.i18n.format("XPRPG.SpellCastingFormat", { preparationType, traditionSpells });
                }
            })();
...
interface SpellcastingCreateAndEditDialogSheetData extends FormApplicationData<Embedded<SpellcastingEntryXPRPG>> {
    actor: ActorXPRPG;
    data: SpellcastingEntrySystemSource;
    classDCs: ClassDCData[];
    magicTraditions: ConfigXPRPG["XPRPG"]["magicTraditions"];
    spellcastingTypes: ConfigXPRPG["XPRPG"]["preparationType"];
    abilities: ConfigXPRPG["XPRPG"]["abilities"];
    hasAbility: boolean;
}

export async function createSpellcastingDialog(event: MouseEvent, object: ActorXPRPG | Embedded<SpellcastingEntryXPRPG>) {
