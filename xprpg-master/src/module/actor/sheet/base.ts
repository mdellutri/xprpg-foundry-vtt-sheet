import type { ActorXPRPG, CharacterXPRPG } from "@actor";
import { RollFunction, StrikeData } from "@actor/data/base";
import { SAVE_TYPES } from "@actor/values";
import { ItemXPRPG, ItemProxyXPRPG, PhysicalItemXPRPG, SpellXPRPG } from "@item";
...
import { ErrorXPRPG, htmlClosest, htmlQuery, htmlQueryAll, objectHasKey, tupleHasValue } from "@util";
import { ActorSizeXPRPG } from "../data/size";
import { ActorSheetDataXPRPG, CoinageSummary, InventoryItem, SheetInventory } from "./data-types";
...
/**
 * Extend the basic ActorSheet class to do all the XPRPG things!
 * This sheet is an Abstract layer which is not used.
 * @category Actor
 */
abstract class ActorSheetXPRPG<TActor extends ActorXPRPG> extends ActorSheet<TActor, ItemXPRPG> {
    ...
    override async getData(options: ActorSheetOptions = this.options): Promise<ActorSheetDataXPRPG<TActor>> {
        options.id ||= this.id;
        // The Actor and its Items
        const actorData = this.actor.toObject(false) as ActorXPRPG;
        ...
        const totalCoinage = ActorSheetXPRPG.coinsToSheetData(coins);
        ...
        const traitsMap = ((): Record<string, string> => {
            switch (this.actor.type) {
                case "hazard":
                    return CONFIG.XPRPG.hazardTraits;
                case "vehicle":
                    return CONFIG.XPRPG.vehicleTraits;
                default:
                    return CONFIG.XPRPG.creatureTraits;
            }
        })();

        const sheetData: ActorSheetDataXPRPG<TActor> = {
    ...
    protected prepareInventory(): SheetInventory {
        const sections: SheetInventory["sections"] = {
            weapon: { label: game.i18n.localize("XPRPG.InventoryWeaponsHeader"), type: "weapon", items: [] },
            armor: { label: game.i18n.localize("XPRPG.InventoryArmorHeader"), type: "armor", items: [] },
            equipment: { label: game.i18n.localize("XPRPG.InventoryEquipmentHeader"), type: "equipment", items: [] },
            consumable: { label: game.i18n.localize("XPRPG.InventoryConsumablesHeader"), type: "consumable", items: [] },
            treasure: { label: game.i18n.localize("XPRPG.InventoryTreasureHeader"), type: "treasure", items: [] },
            backpack: { label: game.i18n.localize("XPRPG.InventoryBackpackHeader"), type: "backpack", items: [] },
        };

        const actorSize = new ActorSizeXPRPG({ value: this.actor.size });
        const createInventoryItem = (item: PhysicalItemXPRPG): InventoryItem => {
            ...
            const itemSize = new ActorSizeXPRPG({ value: item.size });
    ...
    protected static coinsToSheetData(coins: Coins): CoinageSummary {
        return DENOMINATIONS.reduce(
            (accumulated, d) => ({
                ...accumulated,
                [d]: { value: coins[d], label: CONFIG.XPRPG.currencies[d] },
            }),
            {} as CoinageSummary
        );
    }
    ...
    override activateListeners($html: JQuery): void {
        ...
        for (const link of htmlQueryAll(html, ".save-name")) {
            link.addEventListener("click", (event) => {
                ...
                if (!tupleHasValue(SAVE_TYPES, saveType)) {
                    throw ErrorXPRPG(`"${saveType}" is not a recognized save type`);
                }
        ...
        $strikesList.find(attackSelectors).on("click", (event) => {
            if (!("actions" in this.actor.system)) {
                throw ErrorXPRPG("Strikes are not supported on this actor");
            }
        ...
        $html.find(".item-toggle-identified").on("click", (event) => {
            ...
            if (identified) {
                ...
                if (!item?.isOfType("physical")) {
                    throw ErrorXPRPG(`${itemId} is not a physical item.`);
                }
        ...
        $html.find(".item-increase-quantity").on("click", (event) => {
            ...
            const item = this.actor.items.get(itemId);
            if (!item?.isOfType("physical")) {
                throw new Error("XPRPG System | Tried to update quantity on item that does not have quantity");
            }
        ...
        $html.find(".item-decrease-quantity").on("click", (event) => {
            ...
            const item = this.actor.items.get(itemId);
            if (!item?.isOfType("physical")) {
                throw ErrorXPRPG("Tried to update quantity on item that does not have quantity");
            }
            if (item.quantity > 0) {
                this.actor.updateEmbeddedDocuments("Item", [{ _id: itemId, "system.quantity": item.quantity - 1 }]);
            }
        ...
        $html.find(".formula-delete").on("click", (event) => {
            ...
            if (this.actor.isOfType("character")) {
                const actorFormulas = (this.actor.toObject().system as CharacterXPRPG["system"]).crafting.formulas ?? [];
    ...
    async onClickDeleteItem(event: JQuery.TriggeredEvent): Promise<void> {
        ...
        if (item?.isOfType("condition")) {
            ...
            const content = await renderTemplate("systems/xprpg/templates/actors/delete-condition-dialog.hbs", {
                question: game.i18n.format("XPRPG.DeleteConditionQuestion", { condition: item.name }),
                ref: references.html(),
            });
            new Dialog({
                title: game.i18n.localize("XPRPG.DeleteConditionTitle"),
                content,
                buttons: {
                    Yes: {
            ...
            const content = await renderTemplate("systems/xprpg/templates/actors/delete-item-dialog.hbs", {
                name: item.name,
            });
            new Dialog({
            ...
            throw ErrorXPRPG("Item not found");
        }
    }
    ...
    private async onClickBrowseEquipmentCompendia(event: JQuery.ClickEvent<HTMLElement>): Promise<void> {
        ...
        await game.xprpg.compendiumBrowser.openTab("equipment", filter);
    }
    ...
    async emulateItemDrop(data: DropCanvasItemDataXPRPG): Promise<ItemXPRPG[]> {
        return this._onDropItem({ preventDefault(): void {} } as ElementDragEvent, data);
    }

    protected override async _onDropItem(event: ElementDragEvent, data: DropCanvasItemDataXPRPG): Promise<ItemXPRPG[]> {
        event.preventDefault();

        const item = await ItemXPRPG.fromDropData(data);
        if (!item) return [];
    ...
    /**
     * XPRPG specific method called by _onDropItem() when this is a new item that needs to be dropped into the actor
    ...
    protected async _handleDroppedItem(
        ...
        if (item.isOfType("spell") && itemSource.type === "spell") {
            ...
            if (itemSource.system.traits?.value.length === 0 && originItem instanceof SpellXPRPG) {
        ...
        return this._onDropItemCreate(new ItemProxyXPRPG(itemSource).clone().toObject());
    }
    ...
    protected override async _onDropFolder(
        ...
        const folder = (await Folder.fromDropData(data)) as Folder<ItemXPRPG> | undefined;
        if (!folder) return [];
    ...
    async moveItemBetweenActors(
        ...
        if (!sourceActor || !targetActor) {
            throw ErrorXPRPG("Unexpected missing actor(s)");
        }
        if (!item?.isOfType("physical")) {
            throw ErrorXPRPG("Missing or invalid item");
        }
    ...
    private async repairItem(event: JQuery.ClickEvent): Promise<void> {
        ...
        await game.xprpg.actions.repair({ event, item });
    }
    ...
    private onClickCreateItem(event: JQuery.ClickEvent) {
        ...
        data.img = `systems/xprpg/icons/default-icons/${data.type}.svg`;

        if (data.type === "action") {
            const newLabel = game.i18n.localize("XPRPG.NewLabel");
            const actionTypeLabel = game.i18n.localize(`XPRPG.ActionType${data.actionType.capitalize()}`);
            data.name = `${newLabel} ${actionTypeLabel}`;
            mergeObject(data, { "system.actionType.value": data.actionType });
        } else if (data.type === "melee") {
            data.name = game.i18n.localize(`XPRPG.NewPlaceholders.${data.type.capitalize()}`);
            mergeObject(data, { "system.weaponType.value": data.actionType });
        } else if (data.type === "spell") {
            data.level = Number(data.level ?? 1);
            const newLabel = game.i18n.localize("XPRPG.NewLabel");
            const levelLabel = game.i18n.localize(`XPRPG.SpellLevel${data.level}`);
            const spellLabel = data.level > 0 ? game.i18n.localize("XPRPG.SpellLabel") : "";
            ...
            data.name =
                this.actor.type === "npc"
                    ? game.i18n.localize("XPRPG.SkillLabel")
                    : game.i18n.localize("XPRPG.NewPlaceholders.Lore");
        } else {
            data.name = game.i18n.localize(`XPRPG.NewPlaceholders.${data.type.capitalize()}`);
        }
    ...
    private onSellAllTreasure(event: JQuery.ClickEvent) {
        ...
        renderTemplate("systems/xprpg/templates/actors/sell-all-treasure-dialog.hbs").then((html) => {
            new Dialog({
                title: game.i18n.localize("XPRPG.SellAllTreasureTitle"),
                content: html,
                buttons: {
                    Yes: {
                        icon: '<i class="fa fa-check"></i>',
    ...
    protected openTagSelector(event: JQuery.ClickEvent | MouseEvent) {
        ...
        if (!tupleHasValue(TAG_SELECTOR_TYPES, selectorType)) {
            throw ErrorXPRPG(`Unrecognized trait selector type "${selectorType}"`);
        }
    ...
    protected tagSelector(
        ...
        if (selectorType === "basic" && options && "objectProperty" in options) {
            ...
            throw ErrorXPRPG("Insufficient options provided to render basic tag selector");
        } else {
            const TagSelector = {
...
interface ActorSheetXPRPG<TActor extends ActorXPRPG> extends ActorSheet<TActor, ItemXPRPG> {
    prepareItems?(sheetData: ActorSheetDataXPRPG<TActor>): Promise<void>;
}

export { ActorSheetXPRPG };
