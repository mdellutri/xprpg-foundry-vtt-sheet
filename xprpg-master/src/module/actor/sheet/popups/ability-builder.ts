...
import { AncestryXPRPG, BackgroundXPRPG, ClassXPRPG } from "@item";
import { ErrorXPRPG, htmlQuery, setHasElement } from "@util";
...
export class AbilityBuilderPopup extends Application {
    ...
    static override get defaultOptions(): ApplicationOptions {
        return {
            ...
            title: game.i18n.localize("XPRPG.AbilityScoresHeader"),
            template: "systems/xprpg/templates/actors/character/ability-builder.hbs",
            width: "auto",
            height: "auto",
        };
    }
        ...
        const tooltip = ((): string | null => {
            ...
            if (
                ...
                const choices = Object.values(boosts)[0].value.map((ability) =>
                    game.i18n.localize(CONFIG.XPRPG.abilities[ability])
                );
                return game.i18n.format("XPRPG.Actor.Character.AbilityBuilder.BackgroundBoostDescription", {
                    a: choices[0],
                    b: choices[1],
                });
            } else {
    ...
    #getBoostFlawLabels(
        ...
        return Object.values(boostData).flatMap((boosts) => {
            if (boosts.value.length === 6) {
                return game.i18n.localize("XPRPG.AbilityFree");
            } else if (boosts.value.length > 0) {
                return boosts.value.map((ability) => game.i18n.localize(CONFIG.XPRPG.abilities[ability])).join(" or ");
            } else {
                return [];
            }
        });
    ...
    override activateListeners($html: JQuery): void {
        ...
        $html.find("button[data-action=level]").on("click", async (event) => {
            ...
            if (
                ...
                throw ErrorXPRPG("Unexpected update requested to ability boosts and flaws");
            }
