 ...
 * @category Other
 */
export class DistributeCoinsPopup extends FormApplication<ActorXPRPG> {
    static override get defaultOptions(): FormApplicationOptions {
        const options = super.defaultOptions;
        options.id = "distribute-coins";
        options.classes = [];
        options.title = "Distribute Coins";
        options.template = "systems/xprpg/templates/actors/distribute-coins.hbs";
        options.width = "auto";
        return options;
    }
    ...
    override async _updateObject(_event: Event, formData: Record<string, unknown> & PopupFormData): Promise<void> {
        ...
        const selectedActors: CharacterXPRPG[] = formData.actorIds.flatMap((actorId) => {
            const maybeActor = game.actors.get(actorId);
            return maybeActor instanceof CharacterXPRPG ? maybeActor : [];
        });
        ...
        if (thisActor instanceof ActorXPRPG) {
            const coinShare = new CoinsXPRPG();
