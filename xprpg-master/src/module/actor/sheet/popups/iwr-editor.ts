...
import { ErrorXPRPG, htmlClosest, htmlQuery, htmlQueryAll, isObject, tagify } from "@util";

class IWREditor<TActor extends ActorXPRPG> extends DocumentSheet<TActor, IWREditorOptions> {
    ...
    constructor(actor: TActor, options: IWREditorConstructorOptions) {
        ...
        if (this.actor.isOfType("familiar", "loot")) {
            throw ErrorXPRPG(`Actor ${this.actor.name} (${this.actor.uuid}) may not have stored IWR data`);
        }
        this.category = options.category;
        this.types = {
            immunities: CONFIG.XPRPG.immunityTypes,
            weaknesses: CONFIG.XPRPG.weaknessTypes,
            resistances: CONFIG.XPRPG.resistanceTypes,
        }[this.category];
    }
    ...
    static override get defaultOptions(): DocumentSheetOptions {
        return {
            ...
            closeOnSubmit: false,
            classes: ["iwr-editor"],
            template: "systems/xprpg/templates/actors/iwr-editor.hbs",
            width: 500,
            height: "auto",
        };
    }
    ...
    override get title(): string {
        return game.i18n.format("XPRPG.Actor.IWREditor.Title", {
    ...
    get categoryLabel(): string {
        return {
            immunities: "XPRPG.ImmunitiesLabel",
            weaknesses: "XPRPG.WeaknessesLabel",
            resistances: "XPRPG.ResistancesLabel",
        }[this.category];
    }
    ...
    getUpdatedData({ includeNew = false } = {}): ProbablyIWRData[] {
        ...
        return entryElems.flatMap((entryElem): ProbablyIWRData | never[] => {
            ...
            if (
                ...
                throw ErrorXPRPG("Unexpected data encountered while submitting form");
            }
    ...
    async #updateIWR({ includeNew = false } = {}): Promise<void> {
        ...
        if (!formInput) throw ErrorXPRPG("Unexpected error getting for input element");
...
interface IWREditorData<TActor extends ActorXPRPG> extends DocumentSheetData<TActor> {
    header: string;
    category: ListCategory;
