...
class HazardXPRPG extends ActorXPRPG {
    ...
    override isAffectedBy(effect: DamageType | ConditionXPRPG): boolean {
        const damageType = objectHasKey(CONFIG.XPRPG.damageTypes, effect)
            ? effect
            : isObject(effect)
    ...
    override prepareDerivedData(): void {
        ...
        // Armor Class
        {
            ...
            const modifiers = [
                new ModifierXPRPG("XPRPG.BaseModifier", base, MODIFIER_TYPE.UNTYPED),
    ...
    protected prepareSaves(): { [K in SaveType]?: Statistic } {
        ...
        return SAVE_TYPES.reduce((saves: { [K in SaveType]?: Statistic }, saveType) => {
            ...
            const ability = CONFIG.XPRPG.savingThrowDefaultAbilities[saveType];
...
interface HazardXPRPG extends ActorXPRPG {
