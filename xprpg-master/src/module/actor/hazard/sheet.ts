...
export class HazardSheetXPRPG extends ActorSheetXPRPG<HazardXPRPG> {
    ...
    override get template(): string {
        return "systems/xprpg/templates/actors/hazard/sheet.hbs";
    }

    override get title() {
        if (this.editing) {
            return game.i18n.format("XPRPG.Actor.Hazard.TitleEdit", { name: super.title });
        }
    ...
    get editing() {
        return this.options.editable && !!this.actor.getFlag("xprpg", "editHazard.value");
    }
    ...
    override async getData(): Promise<HazardSheetData> {
        ...
        return {
            ...
            actorTraits: systemData.traits.value.map((t) => traitSlugToObject(t, CONFIG.XPRPG.hazardTraits)),
            rarity: CONFIG.XPRPG.rarityTraits,
            rarityLabel: CONFIG.XPRPG.rarityTraits[this.actor.rarity],
    ...
    #prepareSaves(): HazardSaveSheetData[] {
        ...
        for (const saveType of SAVE_TYPES) {
            ...
            if (this.editing || save) {
                results.push({
                    label: game.i18n.localize(`XPRPG.Saves${saveType.titleCase()}Short`),
                    type: saveType,
                    mod: save?.check.mod,
                });
            }
    ...
    override activateListeners($html: JQuery): void {
        ...
        if (traitsEl) {
            const tags = tagify(traitsEl, { whitelist: CONFIG.XPRPG.hazardTraits });
        ...
        $html.find(".edit-mode-button").on("click", () => {
            this.actor.setFlag("xprpg", "editHazard.value", !this.editing);
        });
        ...
        $hint.tooltipster({
            ...
            position: "right",
            theme: "crb-hover",
            content: game.i18n.localize("XPRPG.Actor.Hazard.EmitsSound.Hint"),
        });

        if (!this.options.editable) return;

        $html.find<HTMLInputElement>(".isHazardEditable").on("change", (event) => {
            this.actor.setFlag("xprpg", "editHazard", { value: event.target.checked });
        });
    }
