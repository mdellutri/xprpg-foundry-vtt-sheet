import { CharacterXPRPG, CreatureXPRPG } from "@actor";
import { CreatureSaves, LabeledSpeed } from "@actor/creature/data";
import { ActorSizeXPRPG } from "@actor/data/size";
import { applyStackingRules, CheckModifier, ModifierXPRPG, MODIFIER_TYPE, StatisticModifier } from "@actor/modifiers";
...
class FamiliarXPRPG extends CreatureXPRPG {
    /** The familiar's master, if selected */
    get master(): CharacterXPRPG | null {
        ...
        if (master instanceof CharacterXPRPG) {
    ...
    override prepareBaseData(): void {
        ...
        systemData.traits = {
            value: ["minion"],
            senses: [{ type: "lowLightVision", label: CONFIG.XPRPG.senses.lowLightVision, value: "" }],
            size: new ActorSizeXPRPG({ value: "tiny" }),
        };
        ...
        systemData.attributes.speed = {
            value: 25,
            label: game.i18n.localize("XPRPG.SpeedTypesLand"),
            otherSpeeds: [],
        };
    ...
    override prepareDerivedData(): void {
        ...
        const masterLevel =
            game.settings.get("xprpg", "proficiencyVariant") === "ProficiencyWithoutLevel" ? 0 : master.level;
        ...
        // Hit Points
        {
            ...
            const modifiers = [
                new ModifierXPRPG("XPRPG.MasterLevelHP", level * 5, MODIFIER_TYPE.UNTYPED),
        ...
        // Armor Class
        {
            ...
            stat.breakdown = [game.i18n.format("XPRPG.MasterArmorClass", { base })]
                .concat(
                    stat.modifiers
                        .filter((m) => m.enabled)
                        .map((m) => `${m.label} ${m.modifier < 0 ? "" : "+"}${m.modifier}`)
        ...
        this.saves = SAVE_TYPES.reduce((partialSaves, saveType) => {
            ...
            const stat = new Statistic(this, {
                ...
                label: saveName,
                domains: selectors,
                modifiers: [new ModifierXPRPG(`XPRPG.MasterSavingThrow.${saveType}`, totalMod, MODIFIER_TYPE.UNTYPED)],
        ...
        // Attack
        {
            ...
            const modifiers = [
                new ModifierXPRPG("XPRPG.MasterLevel", masterLevel, MODIFIER_TYPE.UNTYPED),
        ...
        // Perception
        {
            ...
            const modifiers = [
                new ModifierXPRPG("XPRPG.MasterLevel", masterLevel, MODIFIER_TYPE.UNTYPED),
                new ModifierXPRPG(
                    `XPRPG.MasterAbility.${systemData.master.ability}`,
        ...
        for (const shortForm of SKILL_ABBREVIATIONS) {
            ...
            const modifiers = [new ModifierXPRPG("XPRPG.MasterLevel", masterLevel, MODIFIER_TYPE.UNTYPED)];
            ...
            const label = CONFIG.XPRPG.skills[shortForm] ?? longForm;
...
interface FamiliarXPRPG extends CreatureXPRPG {
    readonly data: FamiliarData;
    readonly system: FamiliarSystemData;
}

export { FamiliarXPRPG };
