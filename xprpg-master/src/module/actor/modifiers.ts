...
import { ErrorXPRPG, setHasElement, signedInteger, sluggify, tupleHasValue } from "@util";
...
/**
 * The canonical paragon modifier types; modifiers of the same type do not stack (except for 'untyped' modifiers,
 * which fully stack).
 */
const MODIFIER_TYPE = {
    ABILITY: "ability",
...
interface DeferredValueParams {
    ...
    /** An object to merge into standard options for `RuleElementXPRPG#resolveInjectedProperties` */
...
class ModifierXPRPG implements RawModifier {
    slug: string;
    label: string;
    ...
    constructor(...args: [ModifierObjectParams] | ModifierOrderedParams) {
        ...
        this.predicate = PredicateXPRPG.create(params.predicate ?? []);
        ...
        if (this.force && this.type === "untyped") {
            throw ErrorXPRPG("A forced modifier must have a type");
        }
    }
    ...
    /** Return a copy of this ModifierXPRPG instance */
    clone(options: { test?: Set<string> | string[] } = {}): ModifierXPRPG {
        const clone =
            this.modifier === this.#originalValue
                ? new ModifierXPRPG(this)
                : new ModifierXPRPG({ ...this, modifier: this.#originalValue });
 ...
 */
function createAbilityModifier({ actor, ability, domains }: CreateAbilityModifierParams): ModifierXPRPG {
 ...
 */
function createProficiencyModifier({ actor, rank, domains, addLevel }: CreateProficiencyModifierParams): ModifierXPRPG {
    rank = Math.clamped(rank, 0, 4) as ZeroToFour;
    addLevel ??= rank > 0;
    const pwolVariant = game.settings.get("xprpg", "proficiencyVariant") === "ProficiencyWithoutLevel";

    const baseBonuses: [number, number, number, number, number] = pwolVariant
        ? [
              game.settings.get("xprpg", "proficiencyUntrainedModifier"),
              game.settings.get("xprpg", "proficiencyTrainedModifier"),
              game.settings.get("xprpg", "proficiencyExpertModifier"),
              game.settings.get("xprpg", "proficiencyMasterModifier"),
              game.settings.get("xprpg", "proficiencyLegendaryModifier"),
          ]
        : [0, 2, 4, 6, 8];
    ...
    return new ModifierXPRPG({
        slug: "proficiency",
        label: `XPRPG.ProficiencyLevel${rank}`,
        modifier: bonus,
        type: "proficiency",
...
const HIGHER_BONUS = (a: ModifierXPRPG, b: ModifierXPRPG) => a.modifier >= b.modifier;
/** A comparison which rates the first modifier as better than the second if it's modifier is at least as small. */
const LOWER_PENALTY = (a: ModifierXPRPG, b: ModifierXPRPG) => a.modifier <= b.modifier;
...
function applyStacking(
    best: Record<string, ModifierXPRPG>,
    modifier: ModifierXPRPG,
    isBetter: (first: ModifierXPRPG, second: ModifierXPRPG) => boolean
) {
 ...
 */
function applyStackingRules(modifiers: ModifierXPRPG[]): number {
    let total = 0;
    const highestBonus: Record<string, ModifierXPRPG> = {};
    const lowestPenalty: Record<string, ModifierXPRPG> = {};
    ...
    const bestAbility = abilityModifiers.reduce((best: ModifierXPRPG | null, modifier): ModifierXPRPG | null => {
        if (best === null) {
            return modifier;
        } else {
...
class StatisticModifier {
    ...
    protected _modifiers: ModifierXPRPG[];
     ...
     */
    constructor(slug: string, modifiers: ModifierXPRPG[] = [], rollOptions: string[] | Set<string> = new Set()) {
        rollOptions = rollOptions instanceof Set ? rollOptions : new Set(rollOptions);
        this.slug = slug;

        // De-duplication
        const seen: ModifierXPRPG[] = [];
        for (const modifier of modifiers) {
    ...
    get modifiers(): readonly ModifierXPRPG[] {
        return [...this._modifiers];
    }

    /** Add a modifier to the end of this collection. */
    push(modifier: ModifierXPRPG): number {
        // de-duplication
    ...
    unshift(modifier: ModifierXPRPG): number {
        // de-duplication
        if (this._modifiers.find((o) => o.slug === modifier.slug) === undefined) {
    ...
    delete(modifierSlug: string | ModifierXPRPG): boolean {
        const toDelete =
...
function adjustModifiers(modifiers: ModifierXPRPG[], rollOptions: Set<string>): void {
...
class CheckModifier extends StatisticModifier {
    ...
    constructor(
        slug: string,
        statistic: { modifiers: readonly ModifierXPRPG[] },
 ...
 * @category XPRPG
 */
class DiceModifierXPRPG implements BaseRawModifier {
    slug: string;
    label: string;
    ...
    constructor(params: Partial<Omit<DiceModifierXPRPG, "predicate">> & { slug?: string; predicate?: RawPredicate }) {
        this.label = game.i18n.localize(params.label ?? "");
        this.slug = sluggify(params.slug ?? this.label);
        if (!this.slug) {
            throw ErrorXPRPG("A DiceModifier must have a slug");
        }
        ...
        this.predicate = new PredicateXPRPG(params.predicate ?? []);
        this.enabled = params.enabled ?? this.predicate.test([]);
        this.ignored = params.ignored ?? !this.enabled;
    }
}

type PartialParameters = Partial<Omit<DamageDiceXPRPG, "predicate">> & Pick<DamageDiceXPRPG, "selector" | "slug">;
interface DamageDiceParameters extends PartialParameters {
    predicate?: RawPredicate;
}

class DamageDiceXPRPG extends DiceModifierXPRPG {
    /** The selector used to determine when *has a stroke*  */
    selector: string;

    constructor(params: DamageDiceParameters) {
        const predicate =
            params.predicate instanceof PredicateXPRPG ? params.predicate : new PredicateXPRPG(params.predicate ?? []);
        super({ ...params, predicate });

        if (params.selector) {
            this.selector = params.selector;
        } else {
            throw ErrorXPRPG("Selector is mandatory");
        }
    }

    clone(): DamageDiceXPRPG {
        return new DamageDiceXPRPG(this);
    }
}
