...
import { ArmorXPRPG, ConditionXPRPG, ItemXPRPG, PhysicalItemXPRPG } from "@item";
...
import { CombatantXPRPG } from "@module/encounter";
...
import { ErrorXPRPG, isObject, objectHasKey, setHasElement } from "@util";
import {
    CreatureSkills,
    CreatureSpeeds,
    CreatureSystemData,
...
import { CreatureSenseXPRPG } from "./sense";
import {
    Alignment,
    AlignmentTrait,
    CreatureTrait,
...
/** An "actor" in a Xenos Paragon sense rather than a Foundry one: all should contain attributes and abilities */
abstract class CreatureXPRPG extends ActorXPRPG {
    ...
    get skills(): CreatureSkills {
        ...
        this._skills = Object.entries(this.system.skills).reduce((current, [shortForm, skill]: [string, SkillData]) => {
            ...
            const skillName = game.i18n.localize(skill.label ?? CONFIG.XPRPG.skills[shortForm]) || skill.slug;
    ...
    get perception(): Statistic {
        ...
        return Statistic.from(this, stat, "perception", "XPRPG.PerceptionCheck", "perception-check");
    }

    get wornArmor(): Embedded<ArmorXPRPG> | null {
        return this.itemTypes.armor.find((armor) => armor.isEquipped && armor.isArmor) ?? null;
    }

    /** Get the held shield of most use to the wielder */
    override get heldShield(): Embedded<ArmorXPRPG> | null {
    ...
    isFlatFooted({ dueTo }: IsFlatFootedParams): boolean {
        ...
        if (dueTo === "flanking") {
            ...
            if (typeof flanking.flatFootable === "number") {
                flanking.flatFootable = !PredicateXPRPG.test(
                    [{ lte: ["origin:level", flanking.flatFootable] }],
                    rollOptions
                );
            }

            return flanking.flatFootable && PredicateXPRPG.test(["origin:flanking"], rollOptions);
        }
    ...
    override prepareBaseData(): void {
        ...
        // Bless raw custom modifiers as `ModifierXPRPG`s
        ...
        for (const selector of Object.keys(customModifiers)) {
            customModifiers[selector] = customModifiers[selector].map(
                (rawModifier: RawModifier) => new ModifierXPRPG(rawModifier)
            );
        }
        ...
        if (this.isOfType("character", "npc")) {
            this.system.attributes.shield = {
                itemId: null,
                name: game.i18n.localize("XPRPG.ArmorTypeShield"),
                ac: 0,
                hp: { value: 0, max: 0 },
                brokenThreshold: 0,
                hardness: 0,
                ...
                broken: false,
                destroyed: false,
                icon: "systems/xprpg/icons/actions/raise-a-shield.webp",
            };
        }
    ...
    override prepareDerivedData(): void {
        ...
        if (this.isFlatFooted({ dueTo: "flanking" })) {
            const name = game.i18n.localize("XPRPG.Item.Condition.Flanked");
            const condition = game.xprpg.ConditionManager.getCondition("flat-footed", { name });
            const flatFooted = new ConditionXPRPG(condition.toObject(), { parent: this }) as Embedded<ConditionXPRPG>;

            const rule = flatFooted.prepareRuleElements().shift();
            if (!rule) throw ErrorXPRPG("Unexpected error retrieving condition");
    ...
    protected prepareInitiative(): void {
        ...
        const [ability, initStat, proficiency, proficiencyLabel] =
            checkType === "perception"
                ? (["wis", systemData.attributes.perception, "perception", "XPRPG.PerceptionLabel"] as const)
                : ([
                      systemData.skills[checkType]?.ability ?? "int",
                      systemData.skills[checkType],
                      SKILL_DICTIONARY[checkType],
                      CONFIG.XPRPG.skills[checkType],
                  ] as const);
        ...
        const label = game.i18n.format("XPRPG.InitiativeWithSkill", { skillName: game.i18n.localize(proficiencyLabel) });
        const stat = mergeObject(new CheckModifier("initiative", initStat, modifiers, rollOptions), {
            ...
            roll: async (args: InitiativeRollParams): Promise<InitiativeRollResult | null> => {
                ...
                const combatant = await (async (): Promise<Embedded<CombatantXPRPG> | null> => {
                    if (!game.combat) {
                        ui.notifications.error(game.i18n.localize("XPRPG.Encounter.NoActiveEncounter"));
                        return null;
                    }
                    ...
                    if (existing) {
                        ...
                        ui.notifications.error(game.i18n.format("XPRPG.Encounter.NoTokenInScene", { actor: this.name }));
                        return null;
                    }
                })();
    ...
    protected getShieldBonus(): ModifierXPRPG | null {
    ...
    async adjustCarryType(
        item: Embedded<PhysicalItemXPRPG>,
        ...
        if (carryType === "stowed") {
            ...
            if (isEquipped(usage, equipped) && item instanceof ArmorXPRPG && item.isArmor) {
    ...
    async addCustomModifier(
        ...
        if (!this.isOfType("character", "npc")) return;
        if (stat.length === 0) throw ErrorXPRPG("A custom modifier's statistic must be a non-empty string");
        if (label.length === 0) throw ErrorXPRPG("A custom modifier's label must be a non-empty string");
        ...
        if (!modifiers.some((m) => m.label === label)) {
            ...
            if (objectHasKey(CONFIG.XPRPG.damageTypes, damageType)) {
    ...
    async removeCustomModifier(stat: string, slug: string): Promise<void> {
        ...
        if (typeof slug === "string") {
            ...
            throw ErrorXPRPG("Custom modifiers can only be removed by slug (string) or index (number)");
        }
    }
    ...
    async rollRecovery(event: JQuery.TriggeredEvent): Promise<Rolled<CheckRoll> | null> {
        ...
        return CheckXPRPG.roll(modifier, { actor: this, token, dc, notes }, event);
    }

    /** Prepare derived creature senses from Rules Element synthetics */
    prepareSenses(data: SenseData[], synthetics: RuleElementSynthetics): CreatureSenseXPRPG[] {
        const preparedSenses = data.map((datum) => new CreatureSenseXPRPG(datum));
    ...
    prepareSpeed(movementType: MovementType): CreatureSpeeds | (LabeledSpeed & StatisticModifier) | null {
        ...
        if (movementType === "land") {
            ...
            const typeLabel = game.i18n.localize("XPRPG.SpeedTypesLand");
            const statLabel = game.i18n.format("XPRPG.SpeedLabel", { type: typeLabel });
            const otherData = {
                type: "land",
                label: statLabel,
                total: base + stat.totalModifier,
                breakdown: [
                    `${game.i18n.format("XPRPG.SpeedBaseLabel", { type: typeLabel })} ${landSpeed.value}`,
            ...
            const label = game.i18n.format("XPRPG.SpeedLabel", {
                type: game.i18n.localize(CONFIG.XPRPG.speedTypes[movementType]),
            });
            ...
            stat.breakdown = [`${game.i18n.format("XPRPG.SpeedBaseLabel", { type: speed.label })} ${base}`]
                .concat(
                    stat.modifiers
                        .filter((m) => m.enabled)
...
interface CreatureXPRPG extends ActorXPRPG {
