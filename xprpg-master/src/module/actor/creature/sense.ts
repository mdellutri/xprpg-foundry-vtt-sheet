...
export class CreatureSenseXPRPG implements SenseData {
    ...
    get label(): string | null {
        const buildLabel = (type: string, acuity?: SenseAcuity, range?: number): string => {
            const senses: Record<string, string | undefined> = CONFIG.XPRPG.senses;
            const sense = game.i18n.localize(senses[type] ?? "") || type;
            const acuityLabel = acuity ? game.i18n.localize(CONFIG.XPRPG.senseAcuity[acuity]) : null;
            return acuity && range
                ? game.i18n.format("XPRPG.Actor.Creature.Sense.WithAcuityAndRange", {
                      sense,
                      acuity: acuityLabel,
                      range,
                  })
                : acuity
                ? game.i18n.format("XPRPG.Actor.Creature.Sense.WithAcuity", { sense, acuity: acuityLabel })
                : sense;
        };
