...
import { PhysicalItemXPRPG, TreasureXPRPG } from "@item";
import { Coins } from "@item/physical/data";
import { DENOMINATIONS } from "@item/physical/values";
import { coinCompendiumIds, CoinsXPRPG } from "@item/physical/helpers";
import { ErrorXPRPG, groupBy } from "@util";
import { InventoryBulk } from "./bulk";

class ActorInventory extends Collection<Embedded<PhysicalItemXPRPG>> {
    constructor(public readonly actor: ActorXPRPG, entries?: Embedded<PhysicalItemXPRPG>[]) {
        super(entries?.map((entry) => [entry.id, entry]));
    }

    get coins(): CoinsXPRPG {
        return this.filter((i) => i.isOfType("treasure") && i.isCoinage)
            .map((item) => item.assetValue)
            .reduce((first, second) => first.add(second), new CoinsXPRPG());
    }

    get totalWealth(): CoinsXPRPG {
        ...
        for (const denomination of DENOMINATIONS) {
            ...
            if (quantity > 0) {
                ...
                if (item) {
                    ...
                    const pack = game.packs.find<CompendiumCollection<PhysicalItemXPRPG>>(
                        (p) => p.collection === "xprpg.equipment-srd"
                    );
                    if (!pack) throw ErrorXPRPG("Unexpected error retrieving equipment compendium");
    ...
    async removeCoins(coins: Partial<Coins>, { byValue = true }: { byValue?: boolean } = {}) {
        const coinsToRemove = new CoinsXPRPG(coins);
        const actorCoins = this.coins;
        const coinsToAdd = new CoinsXPRPG();
        ...
        for (const denomination of DENOMINATIONS) {
            ...
            if (!!quantityToRemove && coinItems) {
                const itemsToUpdate: EmbeddedDocumentUpdateData<TreasureXPRPG>[] = [];
