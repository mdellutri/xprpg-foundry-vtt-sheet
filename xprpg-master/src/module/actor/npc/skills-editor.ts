...
export class NPCSkillsEditor extends FormApplication<NPCXPRPG> {
    ...
    static override get defaultOptions() {
        ...
        options.classes = ["xprpg", "npc"];
        options.title = game.i18n.localize("XPRPG.NPC.SkillsEditor.TitleLabel");
        options.template = "systems/xprpg/templates/actors/npc/forms/npc-skills-editor.hbs";
        options.width = "auto";
        options.height = 700;
    ...
    private onClickEditSkill(event: JQuery.ClickEvent): void {
        ...
        if (!item) throw ErrorXPRPG(`Unable to find item for skill ${skillId}.`);
     ...
     */
    private findSkillItem(skillId: string): Embedded<LoreXPRPG> | null {
        const { skills } = this.npc.system;
