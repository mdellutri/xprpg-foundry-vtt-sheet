...
import { NPCXPRPG } from "@actor/index";
...
class NPCSheetXPRPG<TActor extends NPCXPRPG> extends CreatureSheetXPRPG<TActor> {
    ...
    static override get defaultOptions(): ActorSheetOptions {
        ...
        return {
            ...options,
            classes: [...options.classes, "xprpg", "npc"],
            width: 650,
            height: 680,
    ...
    override get template(): string {
        if (this.isLootSheet) {
            return "systems/xprpg/templates/actors/npc/loot-sheet.hbs";
        } else if (this.actor.limited) {
            return "systems/xprpg/templates/actors/limited/npc-sheet.hbs";
        }
        return "systems/xprpg/templates/actors/npc/sheet.hbs";
    }
    ...
    override get title(): string {
        if (this.isLootSheet || this.actor.limited) {
            ...
            if (this.actor.isDead) {
                return `${actorName} [${game.i18n.localize("XPRPG.NPC.Dead")}]`;
            } else {
                return actorName;
            }
        }
    ...
    override async getData(): Promise<NPCSheetData<TActor>> {
        ...
        sheetData.identifySkillAdjustment = CONFIG.XPRPG.dcAdjustments[identifyCreatureData.skill.start];
        ...
        sheetData.identificationSkills = Array.from(sheetData.identifyCreatureData.skills)
            .sort()
            .map((skill) => CONFIG.XPRPG.skillList[skill]);

        sheetData.specificLoreDC = identifyCreatureData.specificLoreDC.dc;
        sheetData.specificLoreAdjustment = CONFIG.XPRPG.dcAdjustments[identifyCreatureData.specificLoreDC.start];
        ...
        sheetData.unspecificLoreAdjustment = CONFIG.XPRPG.dcAdjustments[identifyCreatureData.unspecificLoreDC.start];
        ...
        sheetData.actorSize = CONFIG.XPRPG.actorSizes[sheetData.data.traits.size.value as Size];
        ...
        sheetData.configLootableNpc = game.settings.get("xprpg", "automation.lootableNPCs");
        ...
        sheetData.traitTagifyData = createTagifyTraits(this.actor.system.traits.value, {
            ...
            record: CONFIG.XPRPG.creatureTraits,
        });
    ...
    override activateListeners($html: JQuery): void {
        ...
        if (traitsEl) {
            tagify(traitsEl, { whitelist: CONFIG.XPRPG.monsterTraits });
        }
        ...
        $html.find(".item-control[data-action=generate-attack]").on("click", async (event) => {
            ...
            const existing = actor.itemTypes.melee.filter((m) => m.flags.xprpg.linkedWeapon === itemId).map((m) => m.id);
            if (existing.length > 0) {
                const proceed = await Dialog.confirm({
                    title: game.i18n.localize("XPRPG.Actor.NPC.GenerateAttack.Confirm.Title"),
                    content: game.i18n.localize("XPRPG.Actor.NPC.GenerateAttack.Confirm.Content"),
                    defaultYes: false,
                });
                if (proceed) {
            ...
            ui.notifications.info(
                game.i18n.format("XPRPG.Actor.NPC.GenerateAttack.Notification", { attack: attacks.at(0)?.name ?? "" })
            );
        });
    }
    ...
    #prepareAbilities(abilities: Abilities): void {
        for (const key of ABILITY_ABBREVIATIONS) {
            ...
            const localizedCode = game.i18n.localize(`XPRPG.AbilityId.${key}`);
            const nameKey = CONFIG.XPRPG.abilities[key];
    ...
    #prepareAlignment(sheetSystemData: NPCSystemSheetData): void {
        ...
        const localizedName = game.i18n.localize(`XPRPG.Alignment${alignmentCode}`);
    ...
    #prepareSkills(sheetSystemData: NPCSystemSheetData): void {
        ...
        for (const skillId of sortedSkillsIds) {
            ...
            skill.label = objectHasKey(CONFIG.XPRPG.skillList, skill.expanded)
                ? game.i18n.localize(CONFIG.XPRPG.skillList[skill.expanded])
    ...
    #prepareSaves(systemData: NPCSystemSheetData): void {
        for (const saveType of SAVE_TYPES) {
            ...
            save.labelShort = game.i18n.localize(`XPRPG.Saves${saveType.titleCase()}Short`);
    ...
    async #prepareActions(sheetData: NPCSheetData<TActor>): Promise<void> {
        ...
        const actions: NPCActionSheetData = {
            passive: { label: game.i18n.localize("XPRPG.ActionTypePassive"), actions: [] },
            free: { label: game.i18n.localize("XPRPG.ActionTypeFree"), actions: [] },
            reaction: { label: game.i18n.localize("XPRPG.ActionTypeReaction"), actions: [] },
            action: { label: game.i18n.localize("XPRPG.ActionTypeAction"), actions: [] },
        };
    ...
    #getSizeLocalizedKey(size: string): string {
        const actorSizes = CONFIG.XPRPG.actorSizes;
    ...
    async #rollAbility(event: JQuery.ClickEvent, abilityId: AbilityString): Promise<void> {
        ...
        const parts = ["@bonus"];
        const title = game.i18n.localize(`XPRPG.AbilityCheck.${abilityId}`);
        const data = { bonus };
        const speaker = ChatMessage.getSpeaker({ token: this.token, actor: this.actor });

        await DiceXPRPG.d20Roll({
            event,
            parts,
            data,
            title,
...
type PrePrepSheetData<T extends NPCXPRPG> = Partial<NPCSheetData<T>> & CreatureSheetData<T>;

export { NPCSheetXPRPG };
