import { ActorXPRPG, ActorProxyXPRPG } from "@actor";
...
import { ErrorXPRPG, getActionGlyph, getActionIcon, sluggify } from "@util";
import { ActorSourceXPRPG } from "./data";
import { DamageRollFunction, TraitViewData } from "./data/base";
import { CheckModifier, ModifierXPRPG, MODIFIER_TYPE, StatisticModifier } from "./modifiers";
...
async function resetActors(actors?: Iterable<ActorXPRPG>, { rerender = true } = {}): Promise<void> {
    actors ??= [
    ...
    const refreshScenes =
        game.settings.get("xprpg", "automation.effectExpiration") &&
        !game.settings.get("xprpg", "automation.removeExpiredEffects");
...
async function migrateActorSource(source: PreCreate<ActorSourceXPRPG>): Promise<ActorSourceXPRPG> {
    ...
    const actor = new ActorProxyXPRPG(mergeObject({ prototypeToken: tokenDefaults }, source));
...
async function checkAreaEffects(this: ActorXPRPG): Promise<void> {
    ...
    for (const effect of this.itemTypes.effect) {
        const auraData = effect.flags.xprpg.aura;
        if (!auraData?.removeOnExit) continue;

        const auraToken = await (async (): Promise<TokenDocumentXPRPG | null> => {
            const document = await UUIDUtils.fromUuid(auraData.origin);
            if (document instanceof TokenDocumentXPRPG) {
                return document;
            } else if (document instanceof ActorXPRPG) {
...
function strikeFromMeleeItem(item: Embedded<MeleeXPRPG>): NPCStrike {
    const { ability, isMelee, isThrown } = item;
    const { actor } = item;
    if (!actor.isOfType("npc", "hazard")) {
        throw ErrorXPRPG("Attempted to create melee-item strike statistic for non-NPC/hazard");
    }
    ...
    const attackEffects: Record<string, string | undefined> = CONFIG.XPRPG.attackEffects;
    ...
    const traitObjects = item.system.traits.value.map(
        (t): TraitViewData => ({
            name: t,
            label: CONFIG.XPRPG.npcAttackTraits[t] ?? t,
            description: CONFIG.XPRPG.traitsDescriptions[t],
        })
    );
    ...
    const strike: NPCStrike = mergeObject(statistic, {
        ...
        sourceId: item.id,
        attackRollType: item.isRanged ? "XPRPG.NPCAttackRanged" : "XPRPG.NPCAttackMelee",
        additionalEffects,
        item,
        weapon: item,
        traits: traitObjects,
    ...
    const strikeLabel = game.i18n.localize("XPRPG.WeaponStrikeLabel");
    const multipleAttackPenalty = calculateMAPs(item, { domains, options: baseOptions });
    const sign = strike.totalModifier < 0 ? "" : "+";
    const attackTrait = {
        name: "attack",
        label: CONFIG.XPRPG.featTraits.attack,
        description: CONFIG.XPRPG.traitsDescriptions.attack,
    };

    strike.variants = [
        null,
        new ModifierXPRPG("XPRPG.MultipleAttackPenalty", multipleAttackPenalty.map1, MODIFIER_TYPE.UNTYPED),
        new ModifierXPRPG("XPRPG.MultipleAttackPenalty", multipleAttackPenalty.map2, MODIFIER_TYPE.UNTYPED),
    ].map((map, mapIncreases) => {
        const label = map
            ? game.i18n.format("XPRPG.MAPAbbreviationLabel", { penalty: map.modifier })
        ...
        return {
            ...
            roll: async (params: AttackRollParams = {}): Promise<Rolled<CheckRoll> | null> => {
                ...
                const checkName = game.i18n.format(
                    item.isMelee ? "XPRPG.Action.Strike.MeleeLabel" : "XPRPG.Action.Strike.RangedLabel",
                    { weapon: item.name }
                );
    ...
    const damageRoll =
        ...
        async (params: DamageRollParams = {}): Promise<Rolled<DamageRoll> | string | null> => {
            ...
            const damage = await WeaponDamageXPRPG.fromNPCAttack({
...
function calculateBaseMAP(item: ItemXPRPG): MAPData {
    if (item.isOfType("melee", "weapon")) {
        ...
        switch (alternateMAP) {
            case "1":
                return { label: "XPRPG.MultipleAttackPenalty", map1: -1, map2: -2 };
            case "2":
                return { label: "XPRPG.MultipleAttackPenalty", map1: -2, map2: -4 };
            case "3":
                return { label: "XPRPG.MultipleAttackPenalty", map1: -3, map2: -6 };
            case "4":
                return { label: "XPRPG.MultipleAttackPenalty", map1: -4, map2: -8 };
            case "5":
                return { label: "XPRPG.MultipleAttackPenalty", map1: -5, map2: -10 };
            default: {
                return item.traits.has("agile")
                    ? { label: "XPRPG.MultipleAttackPenalty", map1: -4, map2: -8 }
                    : { label: "XPRPG.MultipleAttackPenalty", map1: -5, map2: -10 };
            }
        }
    }
    return { label: "XPRPG.MultipleAttackPenalty", map1: -5, map2: -10 };
...
/** Whether this actor is of a the "character" type, excluding those from the XPRPG Companion Compendia module */
function isReallyPC(actor: ActorXPRPG): boolean {
