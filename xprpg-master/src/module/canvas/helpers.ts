...
function measureDistanceCuboid(
    ...
    r1: PIXI.Rectangle,
    {
        ...
        reach?: number | null;
        token?: TokenXPRPG | null;
        target?: TokenXPRPG | null;
    } = {}
): number {
...
/**
 * Measure distance using Xenos Paragon: Roleplaying Gamegrid-counting rules
...
/** Highlight grid according to Xenos Paragon: Roleplaying Gameeffect-area shapes */
function highlightGrid({
    ...
    const offsetEmanationOrigin = (destination: Point): Point => {
        if (!(type === "emanation" && object instanceof TokenXPRPG)) {
            return { x: 0, y: 0 };
        }
...
interface HighlightGridParams {
    ...
    object: MeasuredTemplateXPRPG | TokenXPRPG;
