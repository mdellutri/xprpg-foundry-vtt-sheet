...
import { TokenDocumentXPRPG } from "@module/scene";
import { pick } from "@util";
import { CanvasXPRPG, measureDistanceCuboid, TokenLayerXPRPG } from "..";
import { HearingSource } from "../perception/hearing-source";
import { AuraRenderers } from "./aura";

class TokenXPRPG extends Token<TokenDocumentXPRPG> {
    ...
    constructor(document: TokenDocumentXPRPG) {
        super(document);
    ...
    isAdjacentTo(token: TokenXPRPG): boolean {
    ...
    canFlank(flankee: TokenXPRPG, context: { reach?: number } = {}): boolean {
        if (this === flankee || !game.settings.get("xprpg", "automation.flankingDetection")) {
            return false;
        }
    ...
    isFlanking(flankee: TokenXPRPG, { reach }: { reach?: number } = {}): boolean {
        ...
        const onOppositeSides = (flankerA: TokenXPRPG, flankerB: TokenXPRPG, flankee: TokenXPRPG): boolean => {
        ...
        // The Side By Side feat with tie-in to the XPRPG Animal Companion Compendia module
        const sideBySide =
    ...
    /** Overrides _drawBar(k) to also draw xprpg variants of normal resource bars (such as temp health) */
    ...
    protected override _isVisionSource(): boolean {
        const partyVisionEnabled =
            !!this.actor?.hasPlayerOwner && !game.user.isGM && game.settings.get("xprpg", "metagame_partyVision");
    ...
    async showFloatyText(params: ShowFloatyEffectParams): Promise<void> {
        ...
        if (!game.user.isGM && typeof params !== "number") {
            ...
            if (document instanceof EffectXPRPG && document.system.unidentified) return;
        }

        const scrollingTextArgs = ((): Parameters<CanvasXPRPG["interface"]["createScrollingText"]> | null => {
     ...
     */
    distanceTo(target: TokenXPRPG, { reach = null }: { reach?: number | null } = {}): number {
    ...
    protected override _onControl(options: { releaseOthers?: boolean; pan?: boolean } = {}): void {
        if (game.ready) game.xprpg.effectPanel.refresh();
    ...
    override _onDelete(options: DocumentModificationContext<TokenDocumentXPRPG>, userId: string): void {
...
interface TokenXPRPG extends Token<TokenDocumentXPRPG> {
    get layer(): TokenLayerXPRPG<this>;
