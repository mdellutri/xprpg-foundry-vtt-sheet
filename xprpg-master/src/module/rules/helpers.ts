...
import { DamageDiceXPRPG, DeferredValueParams, ModifierAdjustment, ModifierXPRPG } from "@actor/modifiers";
import { AttackItem } from "@actor/types";
import { ConditionSource, EffectSource, ItemSourceXPRPG } from "@item/data";
import { RollNoteXPRPG } from "@module/notes";
import { DegreeOfSuccessAdjustment } from "@system/degree-of-success";
import { RollTwiceOption } from "@system/rolls";
import { isObject, pick } from "@util";
import { RuleElementXPRPG } from "./rule-element";
...
function extractNotes(rollNotes: Record<string, RollNoteXPRPG[]>, selectors: string[]) {
...
async function processPreUpdateActorHooks(
    changed: DocumentUpdateData<ActorXPRPG>,
    ...
    if (!(actor instanceof ActorXPRPG)) return;

    // Run preUpdateActor rule element callbacks
    type WithPreUpdateActor = RuleElementXPRPG & {
        preUpdateActor: NonNullable<RuleElementXPRPG["preUpdateActor"]>;
    };
    const rules = actor.rules.filter((r): r is WithPreUpdateActor => !!r.preUpdateActor);
    if (rules.length === 0) return;

    actor.flags.xprpg.rollOptions = actor.clone(changed, { keepId: true }).flags.xprpg.rollOptions;
    const createDeletes = (
        await Promise.all(
            rules.map(
                (r): Promise<{ create: ItemSourceXPRPG[]; delete: string[] }> =>
