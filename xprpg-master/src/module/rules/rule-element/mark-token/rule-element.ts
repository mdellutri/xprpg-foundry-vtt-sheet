...
class MarkTokenRuleElement extends RuleElementXPRPG {
    ...
    constructor(data: MarkTokenSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
    ...
    override async preCreate({ ruleSource, itemSource, pendingItems }: RuleElementXPRPG.PreCreateParams): Promise<void> {
        ...
        if (!(token instanceof TokenDocumentXPRPG)) {
    ...
    #checkRuleSource(source: RuleElementSource): asserts source is MarkTokenSource {
        if (!(source.key === "MarkToken" && source.slug === this.slug)) {
            throw ErrorXPRPG("Expected incorrect rule element passed");
        }
    }
}
