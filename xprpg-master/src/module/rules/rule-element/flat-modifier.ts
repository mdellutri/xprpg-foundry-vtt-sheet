import { DeferredValueParams, ModifierXPRPG, ModifierType, MODIFIER_TYPES } from "@actor/modifiers";
 ...
 */
class FlatModifierRuleElement extends RuleElementXPRPG<FlatModifierSchema> {
    constructor(source: FlatModifierSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        ...
        if (this.type === "ability") {
            if (setHasElement(ABILITY_ABBREVIATIONS, source.ability)) {
                this.label = typeof source.label === "string" ? source.label : CONFIG.XPRPG.abilities[this.ability];
    ...
    override beforePrepareData(): void {
        ...
        for (const selector of selectors) {
            const construct = (options: DeferredValueParams = {}): ModifierXPRPG | null => {
                ...
                if (game.xprpg.variantRules.AutomaticBonusProgression.suppressRuleElement(this, finalValue)) {
                    return null;
                }
                ...
                if (damageType !== null && !objectHasKey(CONFIG.XPRPG.damageTypes, damageType)) {
...
interface FlatModifierRuleElement
    extends RuleElementXPRPG<FlatModifierSchema>,
