...
/** A Xenos Paragon: Roleplaying Gameaura, capable of transmitting effects and with a visual representation on the canvas */
export class AuraRuleElement extends RuleElementXPRPG {
    ...
    constructor(source: AuraRuleElementSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        source.effects ??= [];
        source.traits ??= [];
        ...
        if (this.#isValid(source)) {
            ...
            this.effects = deepClone(source.effects ?? []).map((e) => ({
                ...e,
                predicate: new PredicateXPRPG(e.predicate ?? []),
            }));
    ...
    #isValid(data: AuraRuleElementSource): data is AuraRuleElementData {
        const validations = {
            traits:
                ...
                data.traits.every((t) => t in CONFIG.XPRPG.spellTraits || t in CONFIG.XPRPG.actionTraits),
            ...
            predicate: !("predicate" in data) || PredicateXPRPG.isValid(data.predicate),
        };
