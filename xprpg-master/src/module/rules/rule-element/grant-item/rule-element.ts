...
import { ErrorXPRPG, isObject, pick, setHasElement, sluggify, tupleHasValue } from "@util";
import { UUIDUtils } from "@util/uuid-utils";
import { RuleElementOptions, RuleElementXPRPG, RuleElementSchema, RuleElementSource } from "..";
...
class GrantItemRuleElement extends RuleElementXPRPG {
    ...
    constructor(data: GrantItemSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        ...
        this.grantedId = this.item.flags.xprpg.itemGrants[this.flag ?? ""]?.id ?? null;
    ...
    override async preCreate(args: RuleElementXPRPG.PreCreateParams): Promise<void> {
        ...
        if (!(grantedItem instanceof ItemXPRPG)) return;
        ruleSource.flag ??=
            typeof ruleSource.flag === "string" && ruleSource.flag.length > 0
                ...
                : ((): string => {
                      ...
                      const itemGrants = itemSource.flags?.xprpg?.itemGrants ?? {};
        ...
        if (!this.allowDuplicate && existingItem) {
            ...
            return ui.notifications.info(
                game.i18n.format("XPRPG.UI.RuleElements.GrantItem.AlreadyHasItem", {
        ...
        const tempGranted = new ItemProxyXPRPG(deepClone(grantedSource), { parent: this.actor }) as Embedded<ItemXPRPG>;
    ...
    override async preUpdateActor(): Promise<{ create: ItemSourceXPRPG[]; delete: string[] }> {
        ...
        const pendingItems: ItemSourceXPRPG[] = [];
        ...
        if (pendingItems.length > 0) {
            const updatedGrants = itemSource.flags.xprpg?.itemGrants ?? {};
            await this.item.update({ "flags.xprpg.itemGrants": updatedGrants }, { render: false });
    ...
    #applyChoiceSelections(grantedItem: Embedded<ItemXPRPG>): void {
    ...
    #setGrantFlags(granter: PreCreate<ItemSourceXPRPG>, grantee: ItemSourceXPRPG | ItemXPRPG): void {
        const flags = mergeObject(granter.flags ?? {}, { xprpg: { itemGrants: {} } });
        if (!this.flag) throw ErrorXPRPG("Unexpected failure looking up RE flag key");
        flags.xprpg.itemGrants[this.flag] = {
            // The granting item records the granted item's ID in an array at `flags.xprpg.itemGrants`
            id: grantee instanceof ItemXPRPG ? grantee.id : grantee._id,
        ...
        // The granted item records its granting item's ID at `flags.xprpg.grantedBy`
        const grantedBy = {
            id: granter._id,
        ...
        if (grantee instanceof ItemXPRPG) {
            ...
            grantee.update({ "flags.xprpg.grantedBy": grantedBy }, { render: false });
        } else {
            grantee.flags = mergeObject(grantee.flags ?? {}, { xprpg: { grantedBy } });
        }
    }

    /** Run the preCreate callbacks of REs from the granted item */
    async #runGrantedItemPreCreates(
        originalArgs: Omit<RuleElementXPRPG.PreCreateParams, "ruleSource">,
        grantedItem: Embedded<ItemXPRPG>,
        grantedSource: ItemSourceXPRPG,
...
interface GrantItemRuleElement extends RuleElementXPRPG, WithItemAlterations<RuleElementSchema> {}
