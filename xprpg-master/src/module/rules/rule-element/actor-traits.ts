...
import { RuleElementXPRPG, RuleElementSource, RuleElementOptions } from "./";

class ActorTraitsRuleElement extends RuleElementXPRPG {
    add: string[] = [];

    remove: string[] = [];

    constructor(data: ActorTraitsSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        data.add ??= [];
        data.remove ??= [];
