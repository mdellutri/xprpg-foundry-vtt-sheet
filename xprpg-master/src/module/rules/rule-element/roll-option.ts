 ...
 */
class RollOptionRuleElement extends RuleElementXPRPG {
    domain: string;
    ...
    private disabledIf?: PredicateXPRPG;
    ...
    constructor(data: RollOptionSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        ...
        if (this.toggleable && Array.isArray(data.disabledIf)) {
            this.disabledIf = new PredicateXPRPG(...data.disabledIf);
            this.disabledValue =
        ...
        if (this.toggleable === "totm" && !game.settings.get("xprpg", "totmToggles")) {
            this.ignored = true;
        }
    }
    ...
    async toggle(newValue = !this.resolveValue()): Promise<boolean | null> {
        if (!this.toggleable) throw ErrorXPRPG("Attempted to toggle non-toggleable roll option");
    ...
    override async afterRoll({ domains, rollOptions }: RuleElementXPRPG.AfterRollParams): Promise<void> {
        if (
            !this.ignored &&
