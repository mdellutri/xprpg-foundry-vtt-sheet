...
class EphemeralEffectRuleElement extends RuleElementXPRPG<EphemeralEffectSchema> {
    ...
    #createDeferredEffect(): (params?: DeferredValueParams) => Promise<EffectSource | ConditionSource | null> {
        return async (params: DeferredValueParams = {}): Promise<EffectSource | ConditionSource | null> => {
            ...
            const effect = game.xprpg.ConditionManager.conditions.get(uuid) ?? (await UUIDUtils.fromUuid(uuid));
            if (!(effect instanceof ItemXPRPG && effect.isOfType("condition", "effect"))) {
...
interface EphemeralEffectRuleElement
    extends RuleElementXPRPG<EphemeralEffectSchema>,
