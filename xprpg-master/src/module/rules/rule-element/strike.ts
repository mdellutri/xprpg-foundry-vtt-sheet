 ...
 */
class StrikeRuleElement extends RuleElementXPRPG<StrikeSchema> {
    protected static override validActorTypes: ActorType[] = ["character", "npc"];

    constructor(source: StrikeSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        super(source, item, options);

        // Force a label of "Fist" if the `fist` shorthand is being used
        if (this.fist) {
            this.label = game.i18n.localize("XPRPG.Weapon.Base.fist");
        }
    ...
    static override defineSchema(): StrikeSchema {
        return {
            ...
            category: new fields.StringField({
                required: true,
                blank: false,
                choices: CONFIG.XPRPG.weaponCategories,
                initial: "unarmed",
            }),
            group: new fields.StringField({
                required: true,
                blank: false,
                choices: CONFIG.XPRPG.weaponGroups,
                initial: "brawling",
            }),
            baseType: new fields.StringField({
                ...
                nullable: true,
                blank: false,
                choices: CONFIG.XPRPG.baseWeaponTypes,
                initial: null,
            }),
            traits: new fields.ArrayField(
                new fields.StringField({ required: true, blank: false, choices: CONFIG.XPRPG.npcAttackTraits })
            ),
            otherTags: new fields.ArrayField(
                new fields.StringField({ required: true, blank: false, choices: CONFIG.XPRPG.otherWeaponTags }),
            ...
            damage: new fields.SchemaField({
                base: new fields.SchemaField({
                    ...
                    die: new fields.StringField({ required: true, choices: CONFIG.XPRPG.damageDie, initial: "d4" }),
            ...
            img: new fields.FilePathField({
                categories: ["IMAGE"],
                nullable: false,
                initial: () => "systems/xprpg/icons/default-icons/melee.svg",
            }),
    ...
    protected override _initialize(options?: Record<string, unknown>): void {
        if (this._source.fist) {
            ...
            this.priority = 100;
            this.slug = "fist";
            this.img = "systems/xprpg/icons/features/classes/powerful-fist.webp";
            ...
            this.predicate = new PredicateXPRPG([{ gt: ["hands-free", 0] }]);
        } else {
    ...
    /**
     * Construct a `WeaponXPRPG` instance for use as the synthetic strike
     * @param damageType The resolved damage type for the strike
     */
    #constructWeapon(damageType: DamageType): Embedded<WeaponXPRPG> {
        const source: PreCreate<WeaponSource> = deepClone({
            _id: this.item.id,
            name: this.label,
            type: "weapon",
            img: this.img,
            flags: { xprpg: { battleForm: this.battleForm } },
            system: {
                slug: this.slug,
                description: { value: "" },
        ...
        return new WeaponXPRPG(source, { parent: this.actor, xprpg: { ready: true } }) as Embedded<WeaponXPRPG>;
    }
}

interface StrikeRuleElement extends RuleElementXPRPG<StrikeSchema>, ModelPropsFromSchema<StrikeSchema> {
    slug: string;
