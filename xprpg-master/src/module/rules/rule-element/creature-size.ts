...
import { ItemXPRPG, TreasureXPRPG } from "@item";
import { Size, SIZES } from "@module/data";
import { isObject, tupleHasValue } from "@util";
import { RuleElementXPRPG, RuleElementData, RuleElementSource, RuleElementOptions, BracketedValue } from "./";

/**
 * @category RuleElement
 * Change a creature's size
 */
class CreatureSizeRuleElement extends RuleElementXPRPG {
    ...
    constructor(data: CreatureSizeSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
    ...
    override beforePrepareData(): void {
        ...
        const { actor } = this;
        const originalSize = new ActorSizeXPRPG({ value: actor.size });
        ...
        if (value === 1) {
            ...
            actor.system.traits.size = new ActorSizeXPRPG({ value: size });
        } else {
        ...
        if (this.resizeEquipment) {
            ...
            for (const item of actor.inventory.filter((i) => !(i instanceof TreasureXPRPG && i.isCoinage))) {
                if (sizeDifference < 0) {
    ...
    #getReach(originalSize: ActorSizeXPRPG): number {
...
interface CreatureSizeRuleElement extends RuleElementXPRPG {
    get actor(): CreatureXPRPG;
...
interface CreatureSizeRuleElementData extends RuleElementData {
    resizeEquipment: boolean;
    minimumSize?: ActorSizeXPRPG;
    maximumSize?: ActorSizeXPRPG;
}
