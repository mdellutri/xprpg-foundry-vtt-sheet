import { RuleElementXPRPG, RuleElementData, RuleElementOptions } from "../";
...
import { DiceModifierXPRPG, ModifierXPRPG, StatisticModifier } from "@actor/modifiers";
import { RollNoteXPRPG } from "@module/notes";
import { PredicateXPRPG } from "@system/predication";
import { ErrorXPRPG, isObject, sluggify, tupleHasValue } from "@util";
import { RuleElementSource } from "../data";
import { CharacterStrike } from "@actor/character/data";

export class BattleFormRuleElement extends RuleElementXPRPG {
    ...
    constructor(data: BattleFormSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        data.value ??= {};
        data.overrides ??= {};
    ...
    static defaultIcons: Record<string, ImageFilePath | undefined> = [
        ...
        const path = `systems/xprpg/icons/unarmed-attacks/${strike}.webp` as const;
    ...
    override async preCreate({ itemSource, ruleSource }: RuleElementXPRPG.PreCreateParams): Promise<void> {
        if (!this.test()) {
    ...
    override beforePrepareData(): void {
        ...
        if (attributes.polymorphed) {
            actor.synthetics.preparationWarnings.add("XPRPG System | You are already under a polymorph effect");
            this.ignored = true;
            return;
        }
    ...
    private prepareAC(): void {
        ...
        armorClass.unshift(new ModifierXPRPG(this.modifierLabel, newModifier, "untyped"));
    ...
    private prepareSpeeds(): void {
        ...
        for (const movementType of MOVEMENT_TYPES) {
            ...
            if (movementType === "land") {
                ...
                const label = game.i18n.format("XPRPG.SpeedBaseLabel", {
                    type: game.i18n.localize("XPRPG.SpeedTypesLand"),
                });
                ...
                const label = game.i18n.localize(CONFIG.XPRPG.speedTypes[movementType]);
                ...
                if (!newSpeed) throw ErrorXPRPG("Unexpected failure retrieving movement type");
    ...
    private prepareSkills(): void {
        for (const key of SKILL_ABBREVIATIONS) {
            ...
            currentSkill.unshift(new ModifierXPRPG(this.modifierLabel, newModifier, "untyped"));
    ...
    private prepareStrikes(): void {
        ...
        const ruleData = Object.entries(this.overrides.strikes).map(([slug, strikeData]) => ({
            key: "Strike",
            label: strikeData.label ?? `XPRPG.BattleForm.Attack.${sluggify(slug, { camel: "bactrian" })}`,
            slug,
            img: strikeData.img,
        ...
        // Repopulate strikes with new WeaponXPRPG instances--unless ownUnarmed is true
        if (this.ownUnarmed) {
            ...
            for (const striking of Object.values(synthetics.striking).flat()) {
                const predicate = (striking.predicate ??= new PredicateXPRPG());
        ...
        for (const action of strikeActions) {
            ...
            if (!this.ownUnarmed && strike && (strike.modifier >= action.totalModifier || !strike.ownIfHigher)) {
                ...
                action.unshift(new ModifierXPRPG(this.modifierLabel, baseModifier, "untyped"));

                // Also replace the label
                const title = game.i18n.localize("XPRPG.RuleElement.Strike");
    ...
    private suppressModifiers(statistic: StatisticModifier): void {
        for (const modifier of statistic.modifiers) {
            if (
                ...
                modifier.adjustments.push({ slug: null, predicate: new PredicateXPRPG(), suppress: true });
    ...
    private suppressNotes(notes: RollNoteXPRPG[]): void {
        for (const note of notes) {
            if (!note.predicate.includes("battle-form")) {
                note.predicate =
                    note.predicate instanceof PredicateXPRPG ? note.predicate : new PredicateXPRPG(note.predicate);
    ...
    override applyDamageExclusion(weapon: WeaponXPRPG, modifiers: (DiceModifierXPRPG | ModifierXPRPG)[]): void {
        ...
        for (const modifier of modifiers) {
            ...
            const isNumericBonus = modifier instanceof ModifierXPRPG && modifier.modifier >= 0;
            const isAbilityModifier = modifier instanceof ModifierXPRPG && modifier.type === "ability";
            const isExtraDice = modifier instanceof DiceModifierXPRPG;
    ...
    private async resolveStrikeQueries(ruleSource: RuleElementSource & { overrides?: unknown }): Promise<void> {
        ...
        for (const [slug, strike] of Object.entries(value.strikes)) {
            ...
            strike.pack = String(strike.pack ?? "xprpg.equipment-srd");
...
export interface BattleFormRuleElement extends RuleElementXPRPG {
