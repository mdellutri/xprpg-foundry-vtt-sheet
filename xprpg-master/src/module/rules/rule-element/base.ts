...
import { DiceModifierXPRPG, ModifierXPRPG } from "@actor/modifiers";
import { ItemXPRPG, PhysicalItemXPRPG, WeaponXPRPG } from "@item";
 ...
 */
abstract class RuleElementXPRPG<TSchema extends RuleElementSchema = RuleElementSchema> extends DataModel<null, TSchema> {
     ...
     */
    constructor(source: RuleElementSource, public item: Embedded<ItemXPRPG>, options: RuleElementOptions = {}) {
        ...
        if (!validActorType) {
            const ruleName = game.i18n.localize(`XPRPG.RuleElement.${this.key}`);
            const actorType = game.i18n.localize(`ACTOR.Type${item.actor.type.titleCase()}`);
            console.warn(`XPRPG System | A ${ruleName} rules element may not be applied to a ${actorType}`);
            source.ignored = true;
        }
    ...
    failValidation(...message: string[]): void {
        ...
        if (!this.suppressWarnings) {
            ...
            this.actor.synthetics.preparationWarnings.add(
                `XPRPG System | ${ruleName} rules element on item ${name} (${uuid}) failed to validate: ${fullMessage}`
            );
        }
        this.ignored = true;
    }
...
interface RuleElementXPRPG<TSchema extends RuleElementSchema>
     ...
     * @param rollOptions Currently accumulated roll options for the pending check
     */
    afterRoll?(params: RuleElementXPRPG.AfterRollParams): Promise<void>;

    /** Runs before the rule's parent item's owning actor is updated */
    preUpdateActor?(): Promise<{ create: ItemSourceXPRPG[]; delete: string[] }>;
     ...
     * manner.
     */
    preCreate?({ ruleSource, itemSource, pendingItems, context }: RuleElementXPRPG.PreCreateParams): Promise<void>;
     ...
     * manner.
     */
    preDelete?({ pendingItems, context }: RuleElementXPRPG.PreDeleteParams): Promise<void>;

    /**
     * Runs before this rules element's parent item is updated */
    preUpdate?(changes: DeepPartial<ItemSourceXPRPG>): Promise<void>;
    ...
    applyDamageExclusion?(weapon: WeaponXPRPG, modifiers: (DiceModifierXPRPG | ModifierXPRPG)[]): void;
}

// eslint-disable-next-line @typescript-eslint/no-namespace
namespace RuleElementXPRPG {
    ...
    export interface PreCreateParams<T extends RuleElementSource = RuleElementSource> {
        ...
        itemSource: PreCreate<ItemSourceXPRPG>;
        /** The source of the rule in `itemSource`'s `system.rules` array */
        ruleSource: T;
        /** All items pending creation in a `ItemXPRPG.createDocuments` call */
        pendingItems: PreCreate<ItemSourceXPRPG>[];
        /** The context object from the `ItemXPRPG.createDocuments` call */
    ...
    export interface PreDeleteParams {
        /** All items pending deletion in a `ItemXPRPG.deleteDocuments` call */
        pendingItems: Embedded<ItemXPRPG>[];
        /** The context object from the `ItemXPRPG.deleteDocuments` call */
        context: DocumentModificationContext<ItemXPRPG>;
    }
...
export { RuleElementXPRPG, RuleElementOptions };
