...
import { RuleElementOptions, RuleElementXPRPG } from "../";
import {
    ChoiceSetData,
    ChoiceSetOwnedItems,
    ChoiceSetPackQuery,
 ...
 */
class ChoiceSetRuleElement extends RuleElementXPRPG<ChoiceSetSchema> {
    ...
    constructor(data: ChoiceSetSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
        ...
        if (isObject(this.choices) && !Array.isArray(this.choices) && !("query" in this.choices)) {
            this.choices.predicate = new PredicateXPRPG(this.choices.predicate ?? []);
        ...
        if (this.selection !== null) {
            item.flags.xprpg.rulesSelections[this.flag] = this.selection;
    ...
    static override defineSchema(): ChoiceSetSchema {
        return {
            ...
            prompt: new fields.StringField({
                ...
                blank: false,
                nullable: false,
                initial: "XPRPG.UI.RuleElements.ChoiceSet.Prompt",
            }),
    ...
    override async preCreate({
        itemSource,
        ruleSource,
    }: RuleElementXPRPG.PreCreateParams<ChoiceSetSource>): Promise<void> {
        ...
        if (isObject(this.choices)) {
            ...
            if ("ownedItems" in choices && choices.ownedItems && !choices.types?.length) {
                console.warn(
                    "XPRPG System | Failure during ChoiceSet preCreate, types is required if ownedItems is set"
                );
        ...
        if (selection) {
            ...
            this.item.flags.xprpg.rulesSelections[this.flag] = selection.value;
    ...
    /**
     * If an array was passed, localize & sort the labels and return. If a string, look it up in CONFIG.XPRPG and
    ...
    private async inflateChoices(): Promise<PickableThing[]> {
        ...
        try {
            const choiceData = choices.map((c) => ({
                ...
                img: c.img,
                predicate: c.predicate ? new PredicateXPRPG(c.predicate) : undefined,
            }));
    ...
    private choicesFromPath(path: string): PickableThing<string>[] {
        const choiceObject: unknown = getProperty(CONFIG.XPRPG, path) ?? getProperty(this.actor, path) ?? {};
        if (
    ...
    private choicesFromOwnedItems(options: ChoiceSetOwnedItems): PickableThing<string>[] {
        const predicate = options.predicate ?? new PredicateXPRPG();
    ...
    private choicesFromAttacks(predicate: PredicateXPRPG): PickableThing<string>[] {
    ...
    private async queryCompendium(choices: ChoiceSetPackQuery): Promise<PickableThing<ItemUUID>[]> {
        const pack = game.packs.get(choices.pack ?? "xprpg.feats-srd");
        if (choices.postFilter) choices.postFilter = new PredicateXPRPG(choices.postFilter);
        ...
        try {
            ...
            const itemType = objectHasKey(CONFIG.XPRPG.Item.documentClasses, choices.itemType)
                ? choices.itemType
                : "feat";
            ...
            if (!items.every((i): i is ItemXPRPG => i instanceof ItemXPRPG)) {
                return [];
            }
            ...
            return filtered
                .filter((i) =>
                    i instanceof FeatXPRPG ? (existing.get(i.slug ?? sluggify(i.name)) ?? 0) < i.maxTakeable : true
                )
...
interface ChoiceSetRuleElement extends RuleElementXPRPG<ChoiceSetSchema>, ModelPropsFromSchema<ChoiceSetSchema> {
    data: ChoiceSetData;
    flag: string;
