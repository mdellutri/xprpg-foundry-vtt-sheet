...
import { DropCanvasDataXPRPG } from "@module/canvas/drop-canvas-data";
import {
    PickableThing,
...
export class ChoiceSetPrompt extends PickAThingPrompt<string | number | object> {
    ...
    allowedDrops: { label: string | null; predicate: PredicateXPRPG } | null;
    ...
    override get template(): string {
        return "systems/xprpg/templates/system/rules-elements/choice-set-prompt.hbs";
    }
    ...
    override async _onDrop(event: ElementDragEvent): Promise<void> {
        ...
        const dropData: DropCanvasDataXPRPG<"Item"> | undefined = JSON.parse(dataString ?? "");
        if (dropData?.type !== "Item") {
            ui.notifications.error("Only an item can be dropped here.");
            return;
        }
        const droppedItem = await ItemXPRPG.fromDropData(dropData);
        if (!droppedItem) throw ErrorXPRPG("Unexpected error resolving drop");
        ...
        if (this.allowedDrops && !isAllowedDrop) {
            ui.notifications.error(
                game.i18n.format("XPRPG.Item.ABC.InvalidDrop", {
        ...
        if (!prompt) throw ErrorXPRPG("Unexpected error retrieving ChoiceSet dialog");
