 ...
 */
class TempHPRuleElement extends RuleElementXPRPG {
    static override validActorTypes: ActorType[] = ["character", "npc", "familiar"];

    constructor(data: TempHPSource, item: Embedded<ItemXPRPG>, options?: RuleElementOptions) {
    ...
    broadcast(newQuantity: number, oldQuantity: number): void {
        const singularOrPlural =
            newQuantity === 1
                ? "XPRPG.Encounter.Broadcast.TempHP.SingleNew"
                : "XPRPG.Encounter.Broadcast.TempHP.PluralNew";
        const wasAt = oldQuantity > 0 ? game.i18n.format("XPRPG.Encounter.Broadcast.TempHP.WasAt", { oldQuantity }) : "";
        ...
        ChatMessageXPRPG.create({ content, speaker, whisper: recipients });
    }
}

interface TempHPRuleElement extends RuleElementXPRPG {
    data: TempHPData;
}
