...
import { PhysicalItemXPRPG } from "./physical";
 ...
 * @param itemData
 */
function getMagicTraditions(item: PhysicalItemXPRPG): Set<MagicTradition> {
...
function getDcRarity(item: PhysicalItemXPRPG): Rarity {
...
function identifyMagic(item: PhysicalItemXPRPG, baseDc: number, notMatchingTraditionModifier: number) {
    const result = {
        occult: baseDc,
        primal: baseDc,
        divine: baseDc,
...
export function isMagical(item: PhysicalItemXPRPG): boolean {
    const { traits } = item;
...
export function getUnidentifiedPlaceholderImage(item: PhysicalItemXPRPG): string {
    ...
    return `systems/xprpg/icons/unidentified_item_icons/${iconName}.webp`;
}
