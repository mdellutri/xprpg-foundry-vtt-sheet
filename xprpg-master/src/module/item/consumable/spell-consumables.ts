import { ConsumableXPRPG, SpellXPRPG } from "@item";
...
const SPELL_CONSUMABLE_NAME_TEMPLATES = {
    cantripDeck5: "XPRPG.Item.Physical.FromSpell.CantripDeck5",
    scroll: "XPRPG.Item.Physical.FromSpell.Scroll",
    wand: "XPRPG.Item.Physical.FromSpell.Wand",
};
...
async function createConsumableFromSpell(
    ...
    const pack = game.packs.find((p) => p.collection === "xprpg.equipment-srd");
    const itemId = getIdForSpellConsumable(type, heightenedLevel);
    const consumable = await pack?.getDocument(itemId ?? "");
    if (!(consumable instanceof ConsumableXPRPG)) {
        throw ErrorXPRPG("Failed to retrieve consumable item");
    }
