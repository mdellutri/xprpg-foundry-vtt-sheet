...
import { RuleElementOptions, RuleElementXPRPG } from "@module/rules";
...
class ConditionXPRPG extends AbstractEffectXPRPG {
    active!: boolean;
    ...
    override get origin(): ActorXPRPG | null {
        const grantingItem = this.actor?.items.get(this.flags.xprpg.grantedBy?.id ?? "");
    ...
    get appliedBy(): ItemXPRPG | null {
        return this.actor?.items.get(this.system.references.parent?.id ?? this.flags.xprpg.grantedBy?.id ?? "") ?? null;
    }
    ...
    get isLocked(): boolean {
        ...
        const granter = this.actor?.items.get(this.flags.xprpg.grantedBy?.id ?? "");
        const grants = Object.values(granter?.flags.xprpg.itemGrants ?? {});
        return grants.find((g) => g.id === this.id)?.onDelete === "restrict";
    }

    /** Is the condition found in the token HUD menu? */
    get isInHUD(): boolean {
        return this.slug in CONFIG.XPRPG.statusEffects.conditions;
    }
    ...
    override async increase(): Promise<void> {
        await this.actor?.increaseCondition(this as Embedded<ConditionXPRPG>);
    }

    override async decrease(): Promise<void> {
        await this.actor?.decreaseCondition(this as Embedded<ConditionXPRPG>);
    }

    async onEndTurn(options: { token?: TokenDocumentXPRPG | null } = {}): Promise<void> {
        ...
        if (this.system.persistent) {
            ...
            roll.toMessage(
                {
                    speaker: ChatMessageXPRPG.getSpeaker({ actor: actor, token }),
    ...
    async rollRecovery(): Promise<void> {
        ...
        if (this.system.persistent) {
            ...
            const result = await new Statistic(this.actor, {
                slug: "recovery",
                label: game.i18n.format("XPRPG.Item.Condition.PersistentDamage.Chat.RecoverLabel", {
                    name: this.name,
                }),
    ...
    override prepareBaseData(): void {
        ...
        const folder = CONFIG.XPRPG.statusEffects.iconDir;
        ...
        if (systemData.persistent) {
            ...
            const localizationKey = `XPRPG.Item.Condition.PersistentDamage.${dc !== null ? "NameWithDC" : "Name"}`;
            this.name = game.i18n.format(localizationKey, {
                formula,
                damageType: game.i18n.localize(CONFIG.XPRPG.damageRollFlavors[damageType] ?? damageType),
                dc,
            });
    ...
    override prepareSiblingData(): void {
        if (!this.actor) throw ErrorXPRPG("prepareSiblingData may only be called from an embedded item");

        // Inactive conditions shouldn't deactivate others
        if (!this.active) return;

        const deactivate = (condition: ConditionXPRPG): void => {
    ...
    override prepareRuleElements(options?: RuleElementOptions): RuleElementXPRPG[] {
    ...
    protected override _onCreate(
        ...
        if (!game.user.isGM && !this.actor?.hasPlayerOwner && game.settings.get("xprpg", "metagame_secretCondition")) {
            return;
        }
        ...
        game.xprpg.StatusEffects.refresh();
    }
...
interface ConditionXPRPG extends AbstractEffectXPRPG {
    readonly data: ConditionData;

    get slug(): ConditionSlug;
}

interface PersistentDamageXPRPG extends ConditionXPRPG {
    system: Omit<ConditionSystemData, "persistent"> & { persistent: PersistentDamageData };
}

interface ConditionModificationContext<T extends ConditionXPRPG> extends DocumentModificationContext<T> {
    conditionValue?: number | null;
}

export { ConditionXPRPG, ConditionModificationContext, PersistentDamageXPRPG };
