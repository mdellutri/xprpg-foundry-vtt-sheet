...
import { PersistentDamageXPRPG } from "./document";

class PersistentDamageDialog extends Application {
    constructor(private actor: ActorXPRPG, options: Partial<ApplicationOptions> = {}) {
        super(options);
    ...
    static override get defaultOptions(): ApplicationOptions {
        return {
            ...
            template: "systems/xprpg/templates/items/persistent-damage-dialog.hbs",
            width: 380,
            height: "auto",
        };
    }
    ...
    override get title(): string {
        return game.i18n.format("XPRPG.Item.Condition.PersistentDamage.Dialog.Title", { actor: this.actor.name });
    }

    override async getData(): Promise<PersistentDialogData> {
        const existing = this.actor.itemTypes.condition
            .filter((c): c is Embedded<PersistentDamageXPRPG> => c.slug === "persistent-damage")
            .map((c) => ({
                id: c.id,
    ...
    #prepareDamageTypes(): DamageTypeData[] {
        const types = Object.keys(CONFIG.XPRPG.damageTypes).map((type) => {
            const labels: Record<string, string | undefined> = CONFIG.XPRPG.damageTypes;
    ...
    override activateListeners($html: JQuery<HTMLElement>): void {
        ...
        html.querySelector("[data-action=add]")?.addEventListener("click", (event) => {
            ...
            if (!DamageRoll.validate(`(${formula})[${damageType}]`)) {
                ...
                const baseConditionSource = game.xprpg.ConditionManager.getCondition("persistent-damage").toObject();
        ...
        html.querySelector("[data-action=roll-persistent]")?.addEventListener("click", () => {
            const existing = this.actor.itemTypes.condition.filter(
                (c): c is Embedded<PersistentDamageXPRPG> => c.slug === "persistent-damage"
            );
