...
import { ConditionXPRPG } from "@item/condition";
import { EffectXPRPG } from "@item/effect";
import { ItemSheetXPRPG } from "@item/sheet";
...
class AfflictionSheetXPRPG extends ItemSheetXPRPG<AfflictionXPRPG> {
    ...
    override async getData(options?: Partial<DocumentSheetOptions>): Promise<AfflictionSheetData> {
        ...
        return {
            ...
            hasDetails: true,
            hasSidebar: true,
            itemType: game.i18n.localize(definingTrait ? CONFIG.XPRPG.actionTraits[definingTrait] : "XPRPG.LevelLabel"),
            conditionTypes: omit(CONFIG.XPRPG.conditionTypes, ["persistent-damage"]),
            damageTypes: CONFIG.XPRPG.damageTypes,
            damageCategories: pick(CONFIG.XPRPG.damageCategories, DAMAGE_CATEGORIES_UNIQUE),
            durationUnits: omit(CONFIG.XPRPG.timeUnits, ["encounter"]),
            onsetUnits: omit(CONFIG.XPRPG.timeUnits, ["encounter", "unlimited"]),
    ...
    protected async prepareStages(): Promise<Record<string, AfflictionStageSheetData>> {
        ...
        for (const [idx, [id, stage]] of Object.entries(Object.entries(this.item.system.stages))) {
            ...
            const effects = stage.effects.map((effect) => {
                ...
                if (!(document instanceof ItemXPRPG)) return effect;
    ...
    override async _onDrop(event: ElementDragEvent): Promise<void> {
        ...
        if (item instanceof EffectXPRPG) {
            const effects = [...stage.effects, { uuid: item.uuid }];
            this.item.update({ system: { stages: { [stageId]: { effects } } } });
        } else {
            ui.notifications.error("XPRPG.Item.Affliction.Error.RestrictedStageItem", { localize: true });
        }
    }
}

interface AfflictionSheetData extends ItemSheetDataXPRPG<AfflictionXPRPG> {
    conditionTypes: Omit<ConfigXPRPG["XPRPG"]["conditionTypes"], "persistent-damage">;
    damageTypes: ConfigXPRPG["XPRPG"]["damageTypes"];
    damageCategories: Pick<ConfigXPRPG["XPRPG"]["damageCategories"], DamageCategoryUnique>;
    durationUnits: Omit<ConfigXPRPG["XPRPG"]["timeUnits"], "encounter">;
    onsetUnits: Omit<ConfigXPRPG["XPRPG"]["timeUnits"], "unlimited" | "encounter">;
...
interface AfflictionConditionSheetData extends AfflictionConditionData {
    document: ConditionXPRPG | null;
}

export { AfflictionSheetXPRPG };
