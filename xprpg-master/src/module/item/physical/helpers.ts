...
class CoinsXPRPG implements Coins {
    cp: number;
    sp: number;
    gp: number;
    pp: number;
    ...
    add(coins: Coins): CoinsXPRPG {
        const other = new CoinsXPRPG(coins);
        return new CoinsXPRPG({
            pp: this.pp + other.pp,
            gp: this.gp + other.gp,
            sp: this.sp + other.sp,
            cp: this.cp + other.cp,
    ...
    scale(factor: number): CoinsXPRPG {
        const result = new CoinsXPRPG(this);
        result.pp *= factor;
        result.gp *= factor;
        result.sp *= factor;
        result.cp *= factor;
    ...
    /** Parses a price string such as "5 gp" and returns a new CoinsXPRPG object */
    static fromString(coinString: string, quantity = 1): CoinsXPRPG {
    ...
    static fromPrice(price: PartialPrice, factor: number): CoinsXPRPG {
        const per = Math.max(1, price.per ?? 1);
        return new CoinsXPRPG(price.value).scale(factor / per);
    }
...
export { CoinsXPRPG, coinCompendiumIds };
