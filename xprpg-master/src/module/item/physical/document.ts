import { type ContainerXPRPG, ItemXPRPG } from "@item";
...
abstract class PhysicalItemXPRPG extends ItemXPRPG {
    // The cached container of this item, if in a container, or null
    private _container: Embedded<ContainerXPRPG> | null = null;
    ...
    get assetValue(): CoinsXPRPG {
        return CoinsXPRPG.fromPrice(this.price, this.quantity);
    }
    ...
    get container(): Embedded<ContainerXPRPG> | null {
    ...
    get activations() {
        return Object.values(this.system.activations ?? {}).map((action) => {
            ...
            if (action.components.cast) components.push(game.i18n.localize("XPRPG.Item.Activation.Cast"));
            if (action.components.command) components.push(game.i18n.localize("XPRPG.Item.Activation.Command"));
            if (action.components.envision) components.push(game.i18n.localize("XPRPG.Item.Activation.Envision"));
            if (action.components.interact) components.push(game.i18n.localize("XPRPG.Item.Activation.Interact"));
    ...
    override prepareBaseData(): void {
        ...
        if (typeof systemData.price.value === "string") {
            systemData.price.value = CoinsXPRPG.fromString(systemData.price.value);
        }
        ...
        systemData.price.value = new CoinsXPRPG(systemData.temporary ? {} : systemData.price.value);
    ...
    override prepareSiblingData(this: Embedded<PhysicalItemXPRPG>): void {
        if (this.isStowed) {
    ...
    isStackableWith(item: PhysicalItemXPRPG): boolean {
        const preCheck =
            this !== item &&
            this.type === item.type &&
            this.name === item.name &&
    ...
    getMystifiedData(status: IdentificationStatus, _options?: Record<string, boolean>): MystifiedData {
        ...
        const description =
            ...
            (() => {
                ...
                const formatString = LocalizeXPRPG.translations.XPRPG.identification.UnidentifiedDescription;
    ...
    override async getChatData(): Promise<ItemSummaryData> {
        ...
        const material = precious
            ? game.i18n.format("XPRPG.Item.Weapon.MaterialAndRunes.MaterialOption", {
                  type: game.i18n.localize(CONFIG.XPRPG.preciousMaterials[precious.type]),
                  grade: game.i18n.localize(CONFIG.XPRPG.preciousMaterialGrades[precious.grade]),
              })
            : null;
        ...
        return {
            rarity: {
                name: this.rarity,
                label: CONFIG.XPRPG.rarityTraits[this.rarity],
                description: CONFIG.XPRPG.traitsDescriptions[this.rarity],
            },
            description: { value: this.description },
            material,
        };
    ...
    protected override traitChatData(dictionary: Record<string, string>): TraitChatData[] {
        ...
        for (const trait of traitData) {
            ...
            if (trait.excluded) {
                ...
                const gmNote = LocalizeXPRPG.translations.XPRPG.identification.TraitGMNote;
...
interface PhysicalItemXPRPG extends ItemXPRPG {
    readonly data: PhysicalItemData;

    computeAdjustedPrice?(): CoinsXPRPG | null;
}

export { PhysicalItemXPRPG };
