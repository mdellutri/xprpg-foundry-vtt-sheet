...
import { ConsumableXPRPG, MeleeXPRPG, PhysicalItemXPRPG } from "@item";
...
import { ErrorXPRPG, objectHasKey, setHasElement, sluggify } from "@util";
...
class WeaponXPRPG extends PhysicalItemXPRPG {
    ...
    override isStackableWith(item: PhysicalItemXPRPG): boolean {
    ...
    get ammo(): Embedded<ConsumableXPRPG> | null {
        const ammo = this.actor?.items.get(this.system.selectedAmmoId ?? "");
        return ammo instanceof ConsumableXPRPG && ammo.quantity > 0 ? ammo : null;
    }
    ...
    override getRollOptions(prefix = this.type): string[] {
        ...
        const baseTypeRollOptions = ((): Record<string, boolean> => {
            const equivalentBases: Record<string, string | undefined> = CONFIG.XPRPG.equivalentWeapons;
    ...
    override prepareBaseData(): void {
        ...
        this.flags.xprpg.comboMeleeUsage ??= false;
    ...
    private prepareMaterialAndRunes(): void {
        ...
        this.system.damage.dice =
            inherentDiceNumber === 1 && !this.flags.xprpg.battleForm
    ...
    override computeAdjustedPrice(): CoinsXPRPG | null {
        const materialData = this.getMaterialValuationData();
        ...
        const modifiedPrice = new CoinsXPRPG({ gp: runeValue + materialValue });

        return modifiedPrice;
    }

    private getRunesValuationData(): RuneValuationData[] {
        const propertyRuneData: Record<string, WeaponPropertyRuneData | undefined> = CONFIG.XPRPG.runes.weapon.property;
        return [
    ...
    override async getChatData(
        this: Embedded<WeaponXPRPG>,
        htmlOptions: EnrichHTMLOptions = {}
    ): Promise<ItemSummaryData> {
        const traits = this.traitChatData(CONFIG.XPRPG.weaponTraits);
        const chatData = await super.getChatData();

        const rangeIncrement =
            this.rangeIncrement && this.maxRange === this.rangeIncrement * 6
                ? `XPRPG.TraitRangeIncrement${this.rangeIncrement}`
                : null;
        const maxRange =
            this.maxRange && this.maxRange === this.rangeIncrement ? `XPRPG.TraitRange${this.maxRange}` : null;

        return this.processChatData(htmlOptions, {
            ...chatData,
            traits,
            properties: [CONFIG.XPRPG.weaponCategories[this.category], rangeIncrement, maxRange].filter((p) => !!p),
        });
    }
    ...
    generateMagicName(): string {
        ...
        const strikingRune = ((): keyof ConfigXPRPG["XPRPG"]["weaponStrikingRunes"] | null => {
        ...
        const params = {
            ...
            material: material.precious && game.i18n.localize(CONFIG.XPRPG.preciousMaterials[material.precious.type]),
            potency: potencyRune,
            striking: strikingRune && game.i18n.localize(CONFIG.XPRPG.weaponStrikingRunes[strikingRune]),
            property1: runes.property[0] && game.i18n.localize(CONFIG.XPRPG.weaponPropertyRunes[runes.property[0]]),
            property2: runes.property[1] && game.i18n.localize(CONFIG.XPRPG.weaponPropertyRunes[runes.property[1]]),
            property3: runes.property[2] && game.i18n.localize(CONFIG.XPRPG.weaponPropertyRunes[runes.property[2]]),
            property4: runes.property[3] && game.i18n.localize(CONFIG.XPRPG.weaponPropertyRunes[runes.property[3]]),
        };
    ...
    override generateUnidentifiedName({ typeOnly = false }: { typeOnly?: boolean } = { typeOnly: false }): string {
        ...
        const group = this.group ? CONFIG.XPRPG.weaponGroups[this.group] : null;
        const fallback = "ITEM.TypeWeapon";
        const itemType = game.i18n.localize(base ?? group ?? fallback);

        if (typeOnly) return itemType;

        const formatString = LocalizeXPRPG.translations.XPRPG.identification.UnidentifiedItem;
    ...
    getAltUsages({ recurse = true } = {}): WeaponXPRPG[] {
    ...
    override clone(data?: DocumentUpdateData<this>, options?: WeaponCloneOptions): this | Promise<this> {
        ...
        if (options?.altUsage && clone instanceof WeaponXPRPG) {
    ...
    private toMeleeUsage(): this | null {
        ...
        if (!meleeUsage || this.flags.xprpg.comboMeleeUsage) return null;
    ...
    toNPCAttacks(this: Embedded<WeaponXPRPG>): Embedded<MeleeXPRPG>[] {
        const { actor } = this;
        if (!actor.isOfType("npc")) throw ErrorXPRPG("Melee items can only be generated for NPCs");
        ...
        const toAttackTraits = (traits: WeaponTrait[]): NPCAttackTrait[] => {
            const newTraits: NPCAttackTrait[] = traits
                ...
                .filter(
                    ...
                    (t) =>
                        // Creature traits
                        !(t in CONFIG.XPRPG.creatureTraits) &&
            ...
            const actorSize = new ActorSizeXPRPG({ value: actor.size });
            ...
            if (objectHasKey(CONFIG.XPRPG.npcAttackTraits, reloadTrait)) {
        ...
        const source: PreCreate<MeleeSource> = {
            ...
            flags: { xprpg: { linkedWeapon: this.id } },
        };

        const attack = new MeleeXPRPG(source, { parent: this.actor }) as Embedded<MeleeXPRPG>;
    ...
    protected override _preUpdate(
        ...
        if ("value" in traits && Array.isArray(traits.value)) {
            traits.value = traits.value.filter((t) => t in CONFIG.XPRPG.weaponTraits);
        }
        ...
        if (game.user.id === userId) {
            const updates =
                this.actor?.itemTypes.melee
                    .filter((a) => a.flags.xprpg.linkedWeapon === this.id)
                    .map((a) => ({ _id: a.id, "flags.xprpg.-=linkedWeapon": null })) ?? [];
            this.actor?.updateEmbeddedDocuments("Item", updates);
        }
    }
}

interface WeaponXPRPG extends PhysicalItemXPRPG {
