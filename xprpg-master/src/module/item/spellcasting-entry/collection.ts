...
import { ErrorXPRPG, groupBy, ordinal } from "@util";
import { SpellcastingEntryXPRPG } from ".";
import { ActiveSpell, SlotKey, SpellcastingSlotLevel, SpellPrepEntry } from "./data";

export class SpellCollection extends Collection<Embedded<SpellXPRPG>> {
    constructor(public entry: Embedded<SpellcastingEntryXPRPG>) {
        super();
    }
    ...
    async addSpell(spell: SpellXPRPG, options: { slotLevel?: number } = {}): Promise<SpellXPRPG | null> {
        ...
        if (!actor.isOfType("creature")) {
            throw ErrorXPRPG("Spellcasting entries can only exist on creatures");
        }
        ...
        if (spell.isFocusSpell && !this.entry.isFocusPool) {
            const focusTypeLabel = game.i18n.format("XPRPG.SpellFocusLabel");
            ui.notifications.warn(
                game.i18n.format("XPRPG.Item.Spell.Warning.WrongSpellType", {
        ...
        if (spell.baseLevel > heightenLevel && this.id === spell.system.location?.value) {
            const targetLevelLabel = game.i18n.format("XPRPG.SpellLevel", { level: ordinal(heightenLevel) });
            const baseLabel = game.i18n.format("XPRPG.SpellLevel", { level: ordinal(spell.baseLevel) });
            ui.notifications.warn(
                game.i18n.format("XPRPG.Item.Spell.Warning.InvalidLevel", {
                    name: spell.name,
        ...
        if (spell.actor === actor) {
            ...
            return created instanceof SpellXPRPG ? created : null;
        }
    }

    /** Saves the prepared spell slot data to the spellcasting entry  */
    async prepareSpell(spell: SpellXPRPG, slotLevel: number, spellSlot: number): Promise<SpellcastingEntryXPRPG> {
        if (spell.baseLevel > slotLevel && !(slotLevel === 0 && spell.isCantrip)) {
            const targetLevelLabel = game.i18n.format("XPRPG.SpellLevel", { level: ordinal(slotLevel) });
            const baseLabel = game.i18n.format("XPRPG.SpellLevel", { level: ordinal(spell.baseLevel) });
            ui.notifications.warn(
                game.i18n.format("XPRPG.Item.Spell.Warning.InvalidLevel", {
                    name: spell.name,
        ...
        if (CONFIG.debug.hooks) {
            console.debug(
                `XPRPG System | Updating location for spell ${spell.name} to match spellcasting entry ${this.id}`
            );
        }
    ...
    unprepareSpell(slotLevel: number, spellSlot: number): Promise<SpellcastingEntryXPRPG> {
        if (CONFIG.debug.hooks === true) {
            console.debug(
                `XPRPG System | Updating spellcasting entry ${this.id} to remove spellslot ${spellSlot} for spell level ${slotLevel}`
            );
        }
        ...
        return this.entry.update({
            [key]: {
                name: game.i18n.localize("XPRPG.SpellSlotEmpty"),
                id: null,
                prepared: false,
                expended: false,
            },
    ...
    setSlotExpendedState(slotLevel: number, spellSlot: number, isExpended: boolean): Promise<SpellcastingEntryXPRPG> {
    ...
    async getSpellData(): Promise<SpellCollectionData> {
        ...
        if (this.entry.isPrepared) {
            ...
            for (let level = 0; level <= this.highestLevel; level++) {
                ...
                results.push({
                    label: level === 0 ? "XPRPG.TraitCantrip" : CONFIG.XPRPG.spellLevels[level as OneToTen],
            ...
            if (cantrips.length) {
                ...
                results.push({
                    label: "XPRPG.Actor.Creature.Spellcasting.Cantrips",
                    level: 0,
                    isCantrip: true,
                    active,
                });
            ...
            if (leveled.length) {
                ...
                results.push({
                    label: actor.isOfType("character") ? "XPRPG.Focus.Spells" : "XPRPG.Focus.Pool",
    ...
    private getSpellPrepList(spells: Embedded<SpellXPRPG>[]) {
