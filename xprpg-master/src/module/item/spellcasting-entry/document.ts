...
import { ItemXPRPG, PhysicalItemXPRPG, SpellXPRPG } from "@item";
...
import { ErrorXPRPG, setHasElement, sluggify } from "@util";
...
class SpellcastingEntryXPRPG extends ItemXPRPG implements SpellcastingEntry {
    ...
    override prepareBaseData(): void {
        ...
        if (this.actor) {
            this.statistic = new Statistic(this.actor, {
                ...
                label: "XPRPG.Actor.Creature.Spellcasting.InvalidProficiency",
                check: { type: "check" },
            });
        }
    }
    ...
    override prepareSiblingData(): void {
        if (!this.actor || this.system.prepared.value === "items") {
            ...
            this.spells = new SpellCollection(this as Embedded<SpellcastingEntryXPRPG>);
    ...
    override prepareActorData(this: Embedded<SpellcastingEntryXPRPG>): void {
    ...
    override getLinkedItems(): Embedded<SpellXPRPG>[] {
    ...
    canCastSpell(spell: SpellXPRPG, options: { origin?: PhysicalItemXPRPG } = {}): boolean {
    ...
    async cast(spell: Embedded<SpellXPRPG>, options: SpellcastingEntryXPRPGCastOptions = {}): Promise<void> {
    ...
    async consume(spell: SpellXPRPG, level: number, slot?: number): Promise<boolean> {
        const actor = this.actor;
        if (!(actor instanceof CharacterXPRPG || actor instanceof NPCXPRPG)) {
            throw ErrorXPRPG("Spellcasting entries require an actor");
        }
        ...
        if (this.isFocusPool && actor.isOfType("character", "npc")) {
            ...
            if (currentPoints > 0) {
                ...
                ui.notifications.warn(game.i18n.localize("XPRPG.Focus.NotEnoughFocusPointsError"));
                return false;
            }
        }

        const levelLabel = game.i18n.localize(CONFIG.XPRPG.spellLevels[level as OneToTen]);
        ...
        if (this.isPrepared && !this.isFlexible) {
            ...
            if (!Number.isInteger(slot)) {
                throw ErrorXPRPG("Slot not given for prepared spell, and no alternative slot was found");
            }

            const isExpended = preparedData[slot].expended ?? false;
            if (isExpended) {
                ui.notifications.warn(game.i18n.format("XPRPG.SpellSlotExpendedError", { name: spell.name }));
                return false;
            }
        ...
        if (slots.value > 0) {
            ...
            ui.notifications.warn(
                game.i18n.format("XPRPG.SpellSlotNotEnoughError", { name: spell.name, level: levelLabel })
            );
            return false;
        }
    }
     ...
     */
    async addSpell(spell: SpellXPRPG, options?: { slotLevel?: number }): Promise<SpellXPRPG | null> {
        return this.spells?.addSpell(spell, options) ?? null;
    }

    /** Saves the prepared spell slot data to the spellcasting entry  */
    async prepareSpell(spell: SpellXPRPG, slotLevel: number, spellSlot: number): Promise<SpellcastingEntryXPRPG | null> {
        return this.spells?.prepareSpell(spell, slotLevel, spellSlot) ?? null;
    }

    /** Removes the spell slot and updates the spellcasting entry */
    async unprepareSpell(spellLevel: number, slotLevel: number): Promise<SpellcastingEntryXPRPG | null> {
    ...
    async setSlotExpendedState(
        ...
        spellSlot: number,
        isExpended: boolean
    ): Promise<SpellcastingEntryXPRPG | null> {
    ...
    async getSpellData(): Promise<SpellcastingAbilityData | SpellcastingEntryListData> {
        if (!this.actor?.isOfType("character", "npc")) {
            throw ErrorXPRPG("Spellcasting entries can only exist on characters and npcs");
        }
...
interface SpellcastingEntryXPRPG extends ItemXPRPG {
    readonly data: SpellcastingEntryData;
}

export { SpellcastingEntryXPRPG };
