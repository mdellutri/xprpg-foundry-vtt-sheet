import { ItemXPRPG, FeatXPRPG } from "@item";
...
/** Abstract base class representing a Xenos Paragon (A)ncestry, (B)ackground, or (C)lass */
abstract class ABCItemXPRPG extends ItemXPRPG {
    /** Returns all items that should also be deleted should this item be deleted */
    override getLinkedItems(): Embedded<FeatXPRPG>[] {
    ...
    override async createGrantedItems(options: { level?: number } = {}): Promise<FeatXPRPG[]> {
        ...
        return items.flatMap((item): FeatXPRPG | never[] => {
            if (item instanceof FeatXPRPG) {
                ...
                console.error("XPRPG System | Missing or invalid ABC item");
                return [];
            }
        });
    }

    protected logAutoChange(this: Embedded<ABCItemXPRPG>, path: string, value: string | number): void {
        if (value === 0) return;
...
interface ABCItemXPRPG extends ItemXPRPG {
