import { SpellXPRPG } from "@item/spell";
import { ItemSheetXPRPG } from "../sheet/base";
import { ItemSheetDataXPRPG } from "../sheet/data-types";
import { SpellDamage, SpellHeighteningInterval, SpellSystemData } from "./data";
import { ErrorXPRPG, fontAwesomeIcon, getActionGlyph, objectHasKey, pick, tagify, tupleHasValue } from "@util";
...
const spellOverridable: Partial<Record<keyof SpellSystemData, string>> = {
    time: "XPRPG.SpellTimeLabel",
    components: "XPRPG.SpellComponentsLabel",
    target: "XPRPG.SpellTargetLabel",
    area: "XPRPG.AreaLabel",
    range: "XPRPG.SpellRangeLabel",
    damage: "XPRPG.DamageLabel",
};
...
export class SpellSheetXPRPG extends ItemSheetXPRPG<SpellXPRPG> {
    ...
    override async getData(options?: Partial<DocumentSheetOptions>): Promise<SpellSheetData> {
        ...
        const itemType =
            isCantrip && isFocusSpell
                ? game.i18n.localize("XPRPG.SpellCategoryFocusCantrip")
                : this.item.isCantrip
                ? game.i18n.localize("XPRPG.TraitCantrip")
                : game.i18n.localize(CONFIG.XPRPG.spellCategories[this.item.system.category.value]);

        const damageTypes = Object.fromEntries(
            Object.entries(CONFIG.XPRPG.damageTypes)
        ...
        return {
            ...
            spellCategories: CONFIG.XPRPG.spellCategories,
            spellTypes: CONFIG.XPRPG.spellTypes,
            saves: CONFIG.XPRPG.saves,
            magicSchools: CONFIG.XPRPG.magicSchools,
            spellLevels: CONFIG.XPRPG.spellLevels,
            damageTypes,
            damageSubtypes: pick(CONFIG.XPRPG.damageCategories, DAMAGE_CATEGORIES_UNIQUE),
            damageCategories: CONFIG.XPRPG.damageCategories,
            spellComponents: this.formatSpellComponents(sheetData.data),
            areaSizes: CONFIG.XPRPG.areaSizes,
            areaTypes: CONFIG.XPRPG.areaTypes,
    ...
    override get title(): string {
        return this.item.isVariant
            ? game.i18n.format("XPRPG.Item.Spell.Variants.SheetTitle", { originalName: this.item.original!.name })
            : super.title;
    }

    override get validTraits() {
        const traits = deepClone(CONFIG.XPRPG.spellTraits);
    ...
    override activateListeners($html: JQuery): void {
        ...
        tagify(html.querySelector('input[name="system.traditions.value"]'), { whitelist: CONFIG.XPRPG.magicTraditions });

        $html.find(".toggle-trait").on("change", (evt) => {
            const target = evt.target as HTMLInputElement;
            const trait = target.dataset.trait ?? "";
            if (!objectHasKey(CONFIG.XPRPG.spellTraits, trait)) {
        ...
        $html.find("[data-action=overlay-add-property]").on("click", (event) => {
            ...
            if (overlay && overlay.data && property && !(property in overlay.data)) {
                ...
                if (typeof value !== "undefined") {
                    ...
                    ui.notifications.warn(`XPRPG System | Failed to initialize property ${property} for overlay`);
                }
            }
        });
        ...
        $html.find("[data-action=variant-delete]").on("click", (event) => {
            ...
            if (id) {
                ...
                new Dialog({
                    title: game.i18n.localize("XPRPG.Item.Spell.Variants.DeleteDialogTitle"),
                    content: `<p>${game.i18n.format("XPRPG.Item.Spell.Variants.DeleteDialogText", {
                        variantName: variant.name,
                    })}</p>`,
                    buttons: {
                        delete: {
                            icon: fontAwesomeIcon("fa-trash").outerHTML,
                            label: game.i18n.localize("XPRPG.DeleteShortLabel"),
                            callback: () => {
    ...
    protected override async _updateObject(event: Event, formData: Record<string, unknown>): Promise<void> {
        ...
        if (this.item.original && this.item.appliedOverlays!.has("override") && !this.rendered) {
            await this.item.original.overlays.updateOverride(this.item as Embedded<SpellXPRPG>, formData);
            return;
        }
    ...
    private formatSpellComponents(data: SpellSystemData): string[] {
        ...
        const comps: string[] = [];
        if (data.components.focus) comps.push(game.i18n.localize(CONFIG.XPRPG.spellComponents.F));
        if (data.components.material) comps.push(game.i18n.localize(CONFIG.XPRPG.spellComponents.M));
        if (data.components.somatic) comps.push(game.i18n.localize(CONFIG.XPRPG.spellComponents.S));
        if (data.components.verbal) comps.push(game.i18n.localize(CONFIG.XPRPG.spellComponents.V));
...
interface SpellSheetData extends ItemSheetDataXPRPG<SpellXPRPG> {
    isCantrip: boolean;
    ...
    magicSchools: ConfigXPRPG["XPRPG"]["magicSchools"];
    spellCategories: ConfigXPRPG["XPRPG"]["spellCategories"];
    spellLevels: ConfigXPRPG["XPRPG"]["spellLevels"];
    spellTypes: ConfigXPRPG["XPRPG"]["spellTypes"];
    saves: ConfigXPRPG["XPRPG"]["saves"];
    damageCategories: ConfigXPRPG["XPRPG"]["damageCategories"];
    damageTypes: Record<string, string>;
    damageSubtypes: Pick<ConfigXPRPG["XPRPG"]["damageCategories"], DamageCategoryUnique>;
    spellComponents: string[];
    areaSizes: ConfigXPRPG["XPRPG"]["areaSizes"];
    areaTypes: ConfigXPRPG["XPRPG"]["areaTypes"];
