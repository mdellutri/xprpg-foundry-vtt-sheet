...
import { ItemXPRPG, SpellcastingEntryXPRPG } from "@item";
import { ActionTrait } from "@item/action/data";
import { ItemSourceXPRPG, ItemSummaryData } from "@item/data";
...
import { CheckXPRPG, CheckRoll } from "@system/check";
import { DamageXPRPG } from "@system/damage/damage";
...
import { EnrichHTMLOptionsXPRPG } from "@system/text-editor";
import { ErrorXPRPG, getActionIcon, groupBy, htmlClosest, ordinal, sortBy, traitSlugToObject } from "@util";
import {
    SpellData,
    SpellHeightenLayer,
    SpellOverlayType,
...
interface SpellConstructionContext extends DocumentConstructionContext<ActorXPRPG | null> {
    fromConsumable?: boolean;
}

class SpellXPRPG extends ItemXPRPG {
    ...
    constructor(data: PreCreate<ItemSourceXPRPG>, context: SpellConstructionContext = {}) {
        super(data, context);
    ...
    get spellcasting(): SpellcastingEntryXPRPG | undefined {
    ...
    get components(): Record<SpellComponent, boolean> & { value: string } {
        ...
        if (components.focus) results.push(game.i18n.localize("XPRPG.SpellComponentShortF"));
        if (components.material) results.push(game.i18n.localize("XPRPG.SpellComponentShortM"));
        if (components.somatic) results.push(game.i18n.localize("XPRPG.SpellComponentShortS"));
        if (components.verbal) results.push(game.i18n.localize("XPRPG.SpellComponentShortV"));
        return {
            ...components,
            value: results.join(""),
        };
    ...
    async getDamage(damageOptions: SpellDamageOptions = { skipDialog: true }): Promise<SpellDamage | null> {
        ...
        const damageDice: DamageDiceXPRPG[] = [];
        if (actor?.isOfType("character", "npc")) {
            const abilityModifiers = Object.entries(this.system.damage.value)
                ...
                .map(
                    ([k, d]) =>
                        new ModifierXPRPG({
                            label: CONFIG.XPRPG.abilities[ability],
                            slug: `ability-${k}`,
        ...
        for (const [damageType, group] of groupBy(notPersistent, (f) => f.damageType).entries()) {
            ...
            if (mainGroup) {
                const damageTypeLabel = game.i18n.localize(CONFIG.XPRPG.damageTypes[damageType] ?? damageType);
        ...
        for (const partial of allPartials.filter((p) => p.damageCategory === "persistent")) {
            ...
            const typeLabel = game.i18n.localize(CONFIG.XPRPG.damageTypes[damageType] ?? damageType);
            const flavorLabel = game.i18n.format("XPRPG.Damage.PersistentTooltip", { damageType: typeLabel });
        ...
        try {
            if (combinedInstanceData.length) {
                ...
                if (invalid.length) {
                    throw ErrorXPRPG(`Invalid damage formulas on ${this.name}: ${invalid.join(", ")}`);
                }

                // Create the damage roll
                const critRule =
                    game.settings.get("xprpg", "critRule") === "doubledamage" ? "double-damage" : "double-dice";
     ...
     */
    loadVariant(options: { castLevel?: number; overlayIds?: string[] } = {}): Embedded<SpellXPRPG> | null {
        if (this.original) {
        ...
        const variant = new SpellXPRPG(override, { parent: this.actor, fromConsumable }) as Embedded<SpellXPRPG>;
    ...
    createTemplate(): MeasuredTemplateXPRPG {
        ...
        if (!area) throw ErrorXPRPG("Attempted to create template with non-area spell");
        ...
        return new MeasuredTemplateXPRPG(templateDoc);
    }
    ...
    override prepareSiblingData(this: Embedded<SpellXPRPG>): void {
    ...
    override async toMessage(
    ...
    ): Promise<ChatMessageXPRPG | undefined> {
        ...
        const flags = messageSource.flags.xprpg;
        ...
        return ChatMessageXPRPG.create(messageSource, { renderSheet: false });
    }

    override async getChatData(
        this: Embedded<SpellXPRPG>,
        htmlOptions: EnrichHTMLOptionsXPRPG = {},
        rollOptions: { castLevel?: number | string; slotLevel?: number | string } = {}
    ): Promise<Omit<ItemSummaryData, "traits">> {
        if (!this.actor) throw ErrorXPRPG(`Cannot retrieve chat data for unowned spell ${this.name}`);
        ...
        if (!spellcasting && !trickData) {
            console.warn(
                `XPRPG System | Orphaned spell ${this.name} (${this.id}) on actor ${this.actor.name} (${this.actor.id})`
            );
            return { ...systemData };
        }
        ...
        if (!statistic) {
            console.warn(
                `XPRPG System | Spell ${this.name} is missing a statistic to cast with (${this.id}) on actor ${this.actor.name} (${this.actor.id})`
            );
            return { ...systemData };
        }
        ...
        // Spell save label
        const saveType = systemData.save.value ? game.i18n.localize(CONFIG.XPRPG.saves[systemData.save.value]) : "";
        const saveKey = systemData.save.basic ? "XPRPG.SaveDCLabelBasic" : "XPRPG.SaveDCLabel";
        const saveLabel = game.i18n.format(saveKey, { dc: spellDC, type: saveType });

        // Spell attack labels
        const isHeal = systemData.spellType.value === "heal";
        const damageLabel = isHeal ? localize("XPRPG.SpellTypeHeal") : localize("XPRPG.DamageLabel");

        const [areaSize, areaType, areaUnit] = systemData.area
            ? [
                  Number(systemData.area.value),
                  game.i18n.localize(CONFIG.XPRPG.areaTypes[systemData.area.type]),
                  game.i18n.localize("XPRPG.Foot"),
              ]
            : [null, null, null];
        const area =
            areaSize && areaType && areaUnit
                ? game.i18n.format("XPRPG.SpellArea", { areaSize, areaUnit, areaType }).trim()
                : null;
        ...
        const levelLabel = (() => {
            const type = this.isCantrip
                ? localize("XPRPG.TraitCantrip")
                : localize(CONFIG.XPRPG.spellCategories[this.system.category.value]);
            return game.i18n.format("XPRPG.ItemLevel", { type, level: castLevel });
        })();

        // Combine properties
        const properties: string[] = [
            heightened ? game.i18n.format("XPRPG.SpellLevelBase", { level: ordinal(baseLevel) }) : null,
            heightened ? game.i18n.format("XPRPG.SpellLevelHeightened", { heightened }) : null,
            `${localize("XPRPG.SpellComponentsLabel")}: ${this.components.value}`,
            systemData.range.value ? `${localize("XPRPG.SpellRangeLabel")}: ${systemData.range.value}` : null,
            systemData.target.value ? `${localize("XPRPG.SpellTargetLabel")}: ${systemData.target.value}` : null,
            area,
            systemData.time.value ? `${localize("XPRPG.SpellTimeLabel")}: ${systemData.time.value}` : null,
            systemData.duration.value ? `${localize("XPRPG.SpellDurationLabel")}: ${systemData.duration.value}` : null,
        ].filter((p): p is string => p !== null);

        const spellTraits = this.traitChatData({
            ...CONFIG.XPRPG.spellTraits,
            ...CONFIG.XPRPG.magicTraditions,
        });
        ...
        return {
            ...
            spellTraits,
            traits: spellTraits,
            actionTraits: this.castingTraits.map((t) => traitSlugToObject(t, CONFIG.XPRPG.actionTraits)),
            areaSize,
            areaType,
            areaUnit,
            item,
    ...
    async rollAttack(
        ...
        if (statistic) {
            ...
            throw ErrorXPRPG("Spell points to location that is not a spellcasting type");
        }
    }
    ...
    async rollDamage(
        ...
        return DamageXPRPG.roll(template, context);
    }
    ...
    async rollCounteract(event: JQuery.ClickEvent): Promise<Rolled<CheckRoll> | null> {
        ...
        if (!(spellcastingEntry instanceof SpellcastingEntryXPRPG)) {
            throw ErrorXPRPG("Spell points to location that is not a spellcasting type");
        }
        ...
        let flavor = "<hr>";
        flavor += `<h3>${game.i18n.localize("XPRPG.Counteract")}</h3>`;
        flavor += `<hr>`;
        ...
        const addFlavor = (success: string, level: number) => {
            const title = game.i18n.localize(`XPRPG.${success}`);
            const description = game.i18n.format(`XPRPG.CounteractDescription.${success}`, { level });
            flavor += `<strong>${title}</strong> ${description}<br />`;
        };
        flavor += `<p>${game.i18n.localize("XPRPG.CounteractDescription.Hint")}</p>`;
        flavor += "<p>";
        ...
        const spellTraits = { ...CONFIG.XPRPG.spellTraits, ...CONFIG.XPRPG.magicSchools, ...CONFIG.XPRPG.magicTraditions };
        const traitObjects = traits.map((trait) => ({
            name: trait,
            label: spellTraits[trait],
        }));

        return CheckXPRPG.roll(
            check,
            {
                actor: this.actor,
                type: "counteract-check",
                options: finalOptions,
                title: game.i18n.localize("XPRPG.Counteract"),
                traits: traitObjects,
            },
            event
        );
    ...
    override async update(data: DocumentUpdateData<this>, options?: DocumentModificationContext<this>): Promise<this> {
        ...
        if (this.original && this.appliedOverlays!.has("override") && this.sheet.rendered) {
            return this.original.overlays.updateOverride(this as Embedded<SpellXPRPG>, data, options) as Promise<this>;
        }
...
interface SpellXPRPG extends ItemXPRPG {
...
export { SpellXPRPG, SpellToMessageOptions };
