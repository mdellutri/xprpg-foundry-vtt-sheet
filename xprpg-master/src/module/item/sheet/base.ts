...
import { ItemSheetDataXPRPG } from "./data-types";
import { RuleElementForm, RULE_ELEMENT_FORMS } from "./rule-elements";

export class ItemSheetXPRPG<TItem extends ItemXPRPG> extends ItemSheet<TItem> {
    static override get defaultOptions(): DocumentSheetOptions {
        const options = super.defaultOptions;
        options.width = 691;
        options.height = 460;
        options.classes = options.classes.concat(["xprpg", "item"]);
        options.template = "systems/xprpg/templates/items/item-sheet.hbs";
    ...
    get validTraits(): Record<string, string> | null {
        if (objectHasKey(CONFIG.XPRPG.Item.traits, this.item.type)) {
            return CONFIG.XPRPG.Item.traits[this.item.type];
        }
        return null;
    }

    /** An alternative to super.getData() for subclasses that don't need this class's `getData` */
    override async getData(options: Partial<DocumentSheetOptions> = {}): Promise<ItemSheetDataXPRPG<TItem>> {
        options.id = this.id;
        ...
        return {
            ...
            hasDetails: true,
            sidebarTitle: game.i18n.format("XPRPG.Item.SidebarSummary", {
            ...
            rarities: CONFIG.XPRPG.rarityTraits,
            traits,
            traitTagifyData,
            enabledRulesUI: game.settings.get("xprpg", "enabledRulesUI"),
            ruleEditing: !!this.editingRuleElement,
            rules: {
                labels: rules.map((ruleData: RuleElementSource) => {
                    const translations: Record<string, string> = LocalizeXPRPG.translations.XPRPG.RuleElement;
                    const key = String(ruleData.key).replace(/^XPRPG\.RuleElement\./, "");
                    ...
                    types: sortStringRecord(
                        Object.keys(RuleElements.all).reduce(
                            (result: Record<string, string>, key) =>
                                mergeObject(result, { [key]: `XPRPG.RuleElement.${key}` }),
                            {}
                        )
                    ),
                },
            ...
            sidebarTemplate: () => `systems/xprpg/templates/items/${item.type}-sidebar.hbs`,
            detailsTemplate: () => `systems/xprpg/templates/items/${item.type}-details.hbs`,
            proficiencies: CONFIG.XPRPG.proficiencyLevels, // lore only, will be removed later
        };
    }
    ...
    protected onTagSelector(anchor: HTMLAnchorElement): void {
        ...
        if (selectorType !== "basic") {
            throw ErrorXPRPG("Item sheets can only use the basic tag selector");
        }
    ...
    override activateListeners($html: JQuery): void {
        ...
        for (const anchor of htmlQueryAll<HTMLAnchorElement>(rulesPanel, "a[data-clipboard]")) {
            anchor.addEventListener("click", () => {
                ...
                if (clipText) {
                    ...
                    ui.notifications.info(game.i18n.format("XPRPG.ClipboardNotification", { clipText }));
                }
            });
        }
                    ...
                    try {
                        ...
                        if (error instanceof Error) {
                            ui.notifications.error(
                                game.i18n.format("XPRPG.ErrorMessage.RuleElementSyntax", { message: error.message })
                            );
        ...
        if (this.isEditable && game.user.isGM && !this.item.system.description.gm) {
            ...
            if (!mainEditor) throw ErrorXPRPG("Unexpected error retrieving description editor");
            ...
            addGMNotesLink.dataset.tooltip = "XPRPG.Item.GMNotes.Add";
    ...
    protected override async _updateObject(event: Event, formData: Record<string, unknown>): Promise<void> {
        ...
        const expanded = expandObject(formData) as DeepPartial<ItemSourceXPRPG>;
