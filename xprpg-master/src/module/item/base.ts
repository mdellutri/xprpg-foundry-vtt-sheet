...
import { RuleElementOptions, RuleElementXPRPG, RuleElements, RuleElementSource } from "@module/rules";
...
import { ErrorXPRPG, isObject, setHasElement, sluggify } from "@util";
import { AfflictionSource } from "./affliction";
import { ContainerXPRPG } from "./container";
import {
    ConditionSource,
    EffectSource,
    FeatSource,
...
class ItemXPRPG extends Item<ActorXPRPG> {
    ...
    get grantedBy(): Embedded<ItemXPRPG> | null {
        return this.actor?.items.get(this.flags.xprpg.grantedBy?.id ?? "") ?? null;
    }

    /** Check this item's type (or whether it's one among multiple types) without a call to `instanceof` */
    isOfType<T extends ItemType>(...types: T[]): this is InstanceType<ConfigXPRPG["XPRPG"]["Item"]["documentClasses"][T]>;
    isOfType(type: "physical"): this is PhysicalItemXPRPG;
    isOfType<T extends "physical" | ItemType>(
        ...types: T[]
    ): this is PhysicalItemXPRPG | InstanceType<ConfigXPRPG["XPRPG"]["Item"]["documentClasses"][Exclude<T, "physical">]>;
    isOfType(...types: string[]): boolean {
        return types.some((t) => (t === "physical" ? setHasElement(PHYSICAL_ITEM_TYPES, this.type) : this.type === t));
    }

    /** Redirect the deletion of any owned items to ActorXPRPG#deleteEmbeddedDocuments for a single workflow */
    ...
    getRollOptions(prefix = this.type): string[] {
        if (prefix.length === 0) throw ErrorXPRPG("`prefix` must be at least one character long");
        ...
        const traitOptions = ((): string[] => {
            ...
            const damageType = Object.keys(CONFIG.XPRPG.damageTypes).join("|");
    ...
    override getRollData(): NonNullable<EnrichHTMLOptionsXPRPG["rollData"]> {
    ...
    async toMessage(
        ...
        if (!this.actor) throw ErrorXPRPG(`Cannot create message for unowned item ${this.name}`);

        // Basic template rendering data
        const template = `systems/xprpg/templates/chat/${this.type}-card.hbs`;
        ...
        const chatData: PreCreate<foundry.data.ChatMessageSource> = {
            speaker: ChatMessageXPRPG.getSpeaker({
                actor: this.actor,
        ...
        if (["gmroll", "blindroll"].includes(rollMode))
            chatData.whisper = ChatMessageXPRPG.getWhisperRecipients("GM").map((u) => u.id);
        ...
        return create ? ChatMessageXPRPG.create(chatData, { renderSheet: false }) : new ChatMessageXPRPG(chatData);
    }

    /** A shortcut to `item.toMessage(..., { create: true })`, kept for backward compatibility */
    async toChat(event?: JQuery.TriggeredEvent): Promise<ChatMessageXPRPG | undefined> {
    ...
    /** Ensure the presence of the xprpg flag scope with default properties and values */
    override prepareBaseData(): void {
        super.prepareBaseData();

        const { flags } = this;
        flags.xprpg = mergeObject(flags.xprpg ?? {}, { rulesSelections: {} });

        // Set item grant default values: pre-migration values will be strings, so temporarily check for objectness
        if (isObject(flags.xprpg.grantedBy)) {
            flags.xprpg.grantedBy.onDelete ??= this.isOfType("physical") ? "detach" : "cascade";
        }
        const grants = (flags.xprpg.itemGrants ??= {});
    ...
    prepareRuleElements(options?: RuleElementOptions): RuleElementXPRPG[] {
        if (!this.actor) throw ErrorXPRPG("Rule elements may only be prepared from embedded items");

        return (this.rules = this.actor.canHostRuleElements
            ? RuleElements.fromOwnedItem(this as Embedded<ItemXPRPG>, options)
            : []);
    }
    ...
    async getChatData(
        ...
        if (!this.actor) throw ErrorXPRPG(`Cannot retrieve chat data for unowned item ${this.name}`);
    ...
    static override async createDialog(
        ...
        const newItem = super.createDialog(data, options) as Promise<ItemXPRPG | undefined>;
    ...
    static override async createDocuments(
        data: (ItemXPRPG | PreCreate<ItemSourceXPRPG>)[] = [],
        context: DocumentModificationContext<ItemXPRPG> = {}
    ): Promise<Item[]> {
        // Convert all `ItemXPRPG`s to source objects
        const sources = data.map((d) => (d instanceof ItemXPRPG ? d.toObject() : d));
        ...
        for (const source of sources) {
            if (!validTypes.includes(source.type)) {
                ui.notifications.error(
                    game.i18n.format("XPRPG.Item.CannotAddType", {
        ...
        for (const source of effectSources) {
            const effect = new CONFIG.XPRPG.Item.documentClasses[source.type](deepClone(source), { parent: actor });
            ...
            if (isUnaffected || isImmune) {
                ...
                if (!(effect.isOfType("effect") && effect.fromAura)) {
                    const locKey = isUnaffected ? "XPRPG.Damage.IWR.ActorIsUnaffected" : "XPRPG.Damage.IWR.ActorIsImmune";
        ...
        const preCreateDeletions = singularTypesToDelete.flatMap((type): Embedded<ItemXPRPG>[] => actor.itemTypes[type]);
        ...
        const items = await (async () => {
            ...
            async function getSimpleGrants(item: Embedded<ItemXPRPG>): Promise<Embedded<ItemXPRPG>[]> {
                const granted = (await item.createGrantedItems?.()) ?? [];
                if (!granted.length) return [];
                const reparented = granted.map(
                    (i): Embedded<ItemXPRPG> =>
                        (i.parent
                            ? i
                            : new CONFIG.Item.documentClass(i._source, { parent: actor })) as Embedded<ItemXPRPG>
                );
                return [...reparented, ...(await Promise.all(reparented.map(getSimpleGrants))).flat()];
            }

            const items = sources.map((source): Embedded<ItemXPRPG> => {
                if (!(context.keepId || context.keepEmbeddedIds)) {
                    source._id = randomID();
                }
                return new CONFIG.Item.documentClass(source, { parent: actor }) as Embedded<ItemXPRPG>;
            });
    ...
    static override async deleteDocuments<T extends ConstructorOf<ItemXPRPG>>(
        this: T,
        ids: string[] = [],
        ...
        if (actor) {
            ...
            const containers = items.filter((i): i is Embedded<ContainerXPRPG> => i.isOfType("backpack"));
    ...
    protected override async _preCreate(
        ...
        // Set default icon
        if (this._source.img === ItemXPRPG.DEFAULT_ICON) {
            this._source.img = data.img = `systems/xprpg/icons/default-icons/${data.type}.svg`;
        }
    ...
    protected override _onDelete(options: DocumentModificationContext, userId: string): void {
        ...
        const actorUpdates: DocumentUpdateData<ActorXPRPG> = {};
        ...
        if (this.actor.isOfType("npc") && ["action", "consumable"].includes(this.type)) {
            ...
            if (!this.actor.isToken) {
                const itemUpdates: DocumentUpdateData<ItemXPRPG>[] = [];
                ...
                const promises: Promise<ItemXPRPG>[] = [];
...
interface ItemXPRPG extends Item<ActorXPRPG> {
    readonly data: ItemDataXPRPG;

    readonly parent: ActorXPRPG | null;

    _sheet: ItemSheetXPRPG<this> | null;

    get sheet(): ItemSheetXPRPG<this>;

    prepareSiblingData?(this: Embedded<ItemXPRPG>): void;

    prepareActorData?(this: Embedded<ItemXPRPG>): void;

    /** Returns items that should also be added when this item is created */
    createGrantedItems(): Promise<ItemXPRPG[]>;

    /** Returns items that should also be deleted should this item be deleted */
    getLinkedItems?(): Embedded<ItemXPRPG>[];
}

/** A `Proxy` to to get Foundry to construct `ItemXPRPG` subclasses */
const ItemProxyXPRPG = new Proxy(ItemXPRPG, {
    construct(
        _target,
        args: [source: PreCreate<ItemSourceXPRPG>, context: DocumentConstructionContext<ItemXPRPG["parent"]>]
    ) {
        return new CONFIG.XPRPG.Item.documentClasses[args[0].type](...args);
    },
});

export { ItemXPRPG, ItemProxyXPRPG };
