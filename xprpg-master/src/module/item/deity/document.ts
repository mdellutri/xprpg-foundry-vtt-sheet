...
class DeityXPRPG extends ItemXPRPG {
    ...
    override prepareActorData(this: Embedded<DeityXPRPG>): void {
        ...
        for (const domain of this.system.domains.primary) {
            const label = CONFIG.XPRPG.deityDomains[domain]?.label;
    ...
    setFavoredWeaponRank(this: Embedded<DeityXPRPG>): void {
        if (!this.actor.isOfType("character")) return;

        const favoredWeaponRank = this.actor.flags.xprpg.favoredWeaponRank;
        if (favoredWeaponRank > 0) {
            const proficiencies = this.actor.system.martial;
            for (const baseType of this.favoredWeapons) {
                mergeObject(proficiencies, {
                    [`weapon-base-${baseType}`]: {
                        label: CONFIG.XPRPG.baseWeaponTypes[baseType],
...
interface DeityXPRPG extends ItemXPRPG {
