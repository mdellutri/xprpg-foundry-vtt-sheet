...
import { DeityXPRPG, ItemXPRPG, SpellXPRPG } from "@item";
...
import { ErrorXPRPG, htmlClosest, htmlQuery, htmlQueryAll, tagify } from "@util";
import { UUIDUtils } from "@util/uuid-utils";

export class DeitySheetXPRPG extends ItemSheetXPRPG<DeityXPRPG> {
    ...
    override async getData(options?: Partial<DocumentSheetOptions>): Promise<DeitySheetData> {
        ...
        const spells = (await UUIDUtils.fromUUIDs(Object.values(sheetData.data.spells)))
            .filter((i): i is SpellXPRPG => i instanceof SpellXPRPG)
            .map((spell) => {
        ...
        return {
            ...sheetData,
            hasDetails: true,
            detailsTemplate: () => "systems/xprpg/templates/items/deity-details.hbs",
            alignments: CONFIG.XPRPG.alignments,
            atheistic: this.item.category === "philosophy",
            skills: CONFIG.XPRPG.skills,
            divineFonts: createSheetOptions(
                { harm: "XPRPG.Item.Deity.DivineFont.Harm", heal: "XPRPG.Item.Deity.DivineFont.Heal" },
                sheetData.data.font
            ),
            spells,
        };
    ...
    override activateListeners($html: JQuery): void {
        ...
        tagify(getInput("system.ability"), { whitelist: CONFIG.XPRPG.abilities, maxTags: 2 });
        tagify(getInput("system.alignment.follower"), { whitelist: CONFIG.XPRPG.alignments, maxTags: 9 });

        // Everything past this point requires a deity or pantheon
        if (this.item.category === "philosophy") return;

        tagify(getInput("system.weapons"), { whitelist: CONFIG.XPRPG.baseWeaponTypes, maxTags: 2 });
        tagify(getInput("system.domains.primary"), { whitelist: CONFIG.XPRPG.deityDomains, maxTags: 4 });
        tagify(getInput("system.domains.alternate"), { whitelist: CONFIG.XPRPG.deityDomains, maxTags: 4 });
    ...
    override async _onDrop(event: ElementDragEvent): Promise<void> {
        ...
        if (!(item instanceof SpellXPRPG)) throw ErrorXPRPG("Invalid item drop on deity sheet");

        if (item.isCantrip || item.isFocusSpell || item.isRitual) {
            ui.notifications.error("XPRPG.Item.Deity.ClericSpells.DropError", { localize: true });
            return;
        }
...
interface DeitySheetData extends ItemSheetDataXPRPG<DeityXPRPG> {
