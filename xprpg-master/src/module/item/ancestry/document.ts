...
class AncestryXPRPG extends ABCItemXPRPG {
    ...
    override prepareActorData(this: Embedded<AncestryXPRPG>): void {
        const { actor } = this;
        if (!(actor instanceof CharacterXPRPG)) {
            console.error("XPRPG System | Only a character can have an ancestry");
            return;
        }
        ...
        if (!(vision === "normal" || senses.some((sense) => sense.type === vision))) {
            senses.push(new CreatureSenseXPRPG({ type: vision, value: "", source: this.name }));
...
interface AncestryXPRPG extends ABCItemXPRPG {
    readonly data: AncestryData;
}

export { AncestryXPRPG };
