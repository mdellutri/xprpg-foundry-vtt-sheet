...
import { ABCItemXPRPG, FeatXPRPG } from "@item";
...
class ClassXPRPG extends ABCItemXPRPG {
    ...
    get grantedFeatSlots(): { ancestry: FeatSlotLevel[]; class: number[]; skill: number[]; general: number[] } {
        ...
        if (game.settings.get("xprpg", "ancestryParagonVariant")) {
            ...
            new Set([
                ...
                ...this.actor.itemTypes.feat.filter(
                    (f) =>
                        ...
                        !(f.flags.xprpg.grantedBy && this.actor?.items.has(f.flags.xprpg.grantedBy.id))
                ),
            ])
        );
    }
    ...
    override async createGrantedItems(options: { level?: number } = {}): Promise<FeatXPRPG[]> {
        const hasChoiceSet = (f: FeatXPRPG) => f.system.rules.some((re) => re.key === "ChoiceSet");
    ...
    override prepareActorData(this: Embedded<ClassXPRPG>): void {
...
interface ClassXPRPG extends ABCItemXPRPG {
