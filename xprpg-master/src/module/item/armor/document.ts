...
import { getResilientBonus, PhysicalItemHitPoints, PhysicalItemXPRPG } from "@item/physical";
import { MAGIC_TRADITIONS } from "@item/spell/values";
import { LocalizeXPRPG } from "@module/system/localize";
import { addSign, ErrorXPRPG, setHasElement, sluggify } from "@util";
import { ArmorCategory, ArmorData, ArmorGroup, BaseArmorType } from ".";

class ArmorXPRPG extends PhysicalItemXPRPG {
    ...
    override prepareActorData(): void {
        const { actor } = this;
        if (!actor) throw ErrorXPRPG("This method may only be called from embedded items");
    ...
    override async getChatData(
        this: Embedded<ArmorXPRPG>,
        htmlOptions: EnrichHTMLOptions = {}
    ): Promise<ItemSummaryData> {
        const systemData = this.system;
        const translations = LocalizeXPRPG.translations.XPRPG;
        const properties = [
            this.isArmor ? CONFIG.XPRPG.armorTypes[this.category] : CONFIG.XPRPG.weaponCategories.martial,
        ...
        return this.processChatData(htmlOptions, {
            ...super.getChatData(),
            traits: this.traitChatData(CONFIG.XPRPG.armorTraits),
            properties,
        });
    }
    ...
    override generateUnidentifiedName({ typeOnly = false }: { typeOnly?: boolean } = { typeOnly: false }): string {
        ...
        const group = this.group ? CONFIG.XPRPG.armorGroups[this.group] : null;
        const fallback = this.isShield ? "XPRPG.ArmorTypeShield" : "ITEM.TypeArmor";
...
interface ArmorXPRPG extends PhysicalItemXPRPG {
