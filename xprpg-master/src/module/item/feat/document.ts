...
class FeatXPRPG extends ItemXPRPG {
    ...
    override prepareActorData(this: Embedded<FeatXPRPG>): void {
    ...
    override async getChatData(
        this: Embedded<FeatXPRPG>,
        htmlOptions: EnrichHTMLOptions = {}
    ): Promise<ItemSummaryData> {
        const systemData = this.system;
        const actionType = this.actionCost?.type ?? "passive";
        const levelLabel = game.i18n.format("XPRPG.LevelN", { level: this.level });
        const properties = [levelLabel, CONFIG.XPRPG.actionTypes[actionType]];
    ...
    protected override _onCreate(data: FeatSource, options: DocumentModificationContext<this>, userId: string): void {
        ...
        if (this.onlyLevel1 && this.actor.level > 1) {
            ...
            const warning = game.i18n.format("XPRPG.Item.Feat.Warning.TakenAfterLevel1", formatParams);
            ui.notifications.warn(warning);
        }

        // Skip subsequent warnings if this feat is from a grant
        if (this.flags.xprpg.grantedBy) return;
        ...
        if (maxTakeable === 1 && timesTaken > 1) {
            ui.notifications.warn(game.i18n.format("XPRPG.Item.Feat.Warning.TakenMoreThanOnce", actorItemNames));
        } else if (timesTaken > maxTakeable) {
            const formatParams = { ...actorItemNames, maxTakeable, timesTaken };
            ui.notifications.warn(game.i18n.format("XPRPG.Item.Feat.Warning.TakenMoreThanMax", formatParams));
        }
    }
}

interface FeatXPRPG extends ItemXPRPG {
