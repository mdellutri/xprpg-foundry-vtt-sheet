import { AbstractEffectXPRPG } from "@item/abstract-effect";
...
class EffectXPRPG extends AbstractEffectXPRPG {
    ...
    get totalDuration(): number {
        ...
        if (["unlimited", "encounter"].includes(duration.unit)) {
            ...
            return duration.value * (EffectXPRPG.DURATION_UNITS[duration.unit] ?? 0);
        }
    }
    ...
    override prepareRuleElements(options?: RuleElementOptions): RuleElementXPRPG[] {
        const autoExpireEffects = game.settings.get("xprpg", "automation.effectExpiration");
    ...
    protected override async _preCreate(
        ...
        if (this.actor && badge?.type === "formula" && badge.evaluate) {
            ...
            const speaker = ChatMessageXPRPG.getSpeaker({ actor: this.actor, token: this.actor.token });
    ...
    protected override _onCreate(
        ...
        if (!this.flags.xprpg.aura || game.combat?.started) {
    ...
    protected override _onDelete(options: DocumentModificationContext, userId: string): void {
        if (this.actor) {
            game.xprpg.effectTracker.unregister(this as Embedded<EffectXPRPG>);
        }
...
interface EffectXPRPG extends AbstractEffectXPRPG {
