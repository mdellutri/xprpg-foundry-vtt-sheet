...
import { SceneXPRPG } from "@scene/document";
import { TokenAuraData } from "./types";
import { TokenDocumentXPRPG } from "../document";
...
class TokenAura implements TokenAuraData {
    ...
    token: Embedded<TokenDocumentXPRPG>;
    ...
    containsToken(token: Embedded<TokenDocumentXPRPG>): boolean {
     ...
     */
    async notifyActors(specific?: TokenDocumentXPRPG[]): Promise<void> {
        ...
        const tokensToCheck = (specific ? specific : this.token.scene?.tokens.contents ?? []).filter(
            (t): t is Embedded<TokenDocumentXPRPG> & { actor: ActorXPRPG } =>
