...
import { SceneXPRPG, TokenConfigXPRPG } from "@module/scene";
import { TokenDataXPRPG } from "./data";
import { ChatMessageXPRPG } from "@module/chat-message";
import { CombatantXPRPG, EncounterXPRPG } from "@module/encounter";
import { PrototypeTokenXPRPG } from "@actor/data/base";
import { TokenAura } from "./aura";
import { objectHasKey, sluggify } from "@util";
import { LightLevels } from "@scene/data";

class TokenDocumentXPRPG<TActor extends ActorXPRPG = ActorXPRPG> extends TokenDocument<TActor> {
    ...
    override hasStatusEffect(statusId: string): boolean {
        ...
        const { actor } = this;
        if (!actor || !game.settings.get("xprpg", "automation.rulesBasedVision")) {
            return false;
        }

        const hasCondition = objectHasKey(CONFIG.XPRPG.conditionTypes, statusId) && actor.hasCondition(statusId);
    ...
    protected override _initialize(): void {
        this.auras = new Map();
        this._source.flags.xprpg ??= {};
        this._source.flags.xprpg.linkToActorSize ??= true;
        this._source.flags.xprpg.autoscale = this._source.flags.xprpg.linkToActorSize
            ? this._source.flags.xprpg.autoscale ?? game.settings.get("xprpg", "tokens.autoscale")
            : false;
    ...
    get linkToActorSize(): boolean {
        return this.flags.xprpg.linkToActorSize;
    }

    /** Is this token's scale locked at 1 or (for small creatures) 0.8? */
    get autoscale(): boolean {
        return this.flags.xprpg.autoscale;
    }
    ...
    override prepareBaseData(): void {
        ...
        const tokenImgIsDefault = [
            ActorXPRPG.DEFAULT_ICON,
            `systems/xprpg/icons/default-icons/${this.actor.type}.svg`,
        ...
        for (const [key, data] of this.actor.auras.entries()) {
            this.auras.set(
                ...
                new TokenAura({
                    slug: key,
                    radius: data.radius,
                    token: this as Embedded<TokenDocumentXPRPG>,
        ...
        const linkToActorSize = this.flags.xprpg?.linkToActorSize ?? linkDefault;

        const autoscaleDefault = game.settings.get("xprpg", "tokens.autoscale");
        // Autoscaling is a secondary feature of linking to actor size
        const autoscale = linkToActorSize ? this.flags.xprpg.autoscale ?? autoscaleDefault : false;
        this.flags.xprpg = mergeObject(this.flags.xprpg ?? {}, { linkToActorSize, autoscale });

        // Nath mode
        const defaultIcons = [ActorXPRPG.DEFAULT_ICON, `systems/xprpg/icons/default-icons/${this.actor.type}.svg`];
        if (game.settings.get("xprpg", "nathMode") && defaultIcons.includes(this.texture.src)) {
            this.texture.src = ((): VideoFilePath => {
                switch (this.actor.alliance) {
                    case "party":
                        return "systems/xprpg/icons/default-icons/alternatives/nath/ally.webp";
                    case "opposition":
                        return "systems/xprpg/icons/default-icons/alternatives/nath/enemy.webp";
                    default:
    ...
    override prepareDerivedData(): void {
        ...
        if (tokenOverrides.texture) {
            ...
            if ("scaleX" in tokenOverrides.texture) {
                ...
                this.flags.xprpg.autoscale = false;
            }
        }
        ...
        TokenDocumentXPRPG.prepareSize(this, this.actor);
    ...
    #prepareDerivedPerception(): void {
        ...
        if (visionMode === "darkvision" || this.scene.lightLevel > LightLevels.DARKNESS) {
            ...
            if (this.actor.isOfType("character") && this.actor.flags.xprpg.colorDarkvision) {
    ...
    static prepareSize(token: TokenDocumentXPRPG | PrototypeTokenXPRPG, actor: ActorXPRPG | null): void {
        if (!(actor && token.flags.xprpg.linkToActorSize)) return;
        ...
        if (actor.isOfType("vehicle")) {
            ...
            if (game.settings.get("xprpg", "tokens.autoscale") && token.flags.xprpg.autoscale !== false) {
    ...
    async setInitiative({
        ...
        if (!game.combat) {
            ui.notifications.error("XPRPG.Encounter.NoActiveEncounter");
            return;
        }
        ...
        if (sendMessage) {
            await ChatMessageXPRPG.createDocuments([
                {
                    ...
                    content: game.i18n.format("XPRPG.InitativeIsNow", { name: this.name, value: initiative }),
                },
            ]);
        }
    }
...
interface TokenDocumentXPRPG<TActor extends ActorXPRPG = ActorXPRPG> extends TokenDocument<TActor> {
    readonly data: TokenDataXPRPG<this>;

    readonly _object: TokenXPRPG | null;

    get object(): TokenXPRPG;

    readonly parent: SceneXPRPG | null;

    get combatant(): CombatantXPRPG<EncounterXPRPG> | null;

    _sheet: TokenConfigXPRPG<this> | null;

    get sheet(): TokenConfigXPRPG<this>;

    overlayEffect: ImageFilePath;
}

export { TokenDocumentXPRPG };
