...
import { UserVisibility, UserVisibilityXPRPG } from "@scripts/ui/user-visibility";
...
class TextEditorXPRPG extends TextEditor {
    static override enrichHTML(content: string | null, options?: EnrichHTMLOptionsXPRPG & { async?: false }): string;
    static override enrichHTML(
        content: string | null,
        options?: EnrichHTMLOptionsXPRPG & { async: true }
    ): Promise<string>;
    static override enrichHTML(content: string | null, options?: EnrichHTMLOptionsXPRPG): string;
    static override enrichHTML(
        this: typeof TextEditor,
        content: string | null,
        options: EnrichHTMLOptionsXPRPG = {}
        ...
        if (typeof enriched === "string") {
            return TextEditorXPRPG.processUserVisibility(enriched, options);
        }

        return Promise.resolve().then(async () => TextEditorXPRPG.processUserVisibility(await enriched, options));
    }
    ...
    static override async _onClickInlineRoll(event: MouseEvent): Promise<ChatMessage> {
        ...
        const [actor, rollData]: [ActorXPRPG | null, Record<string, unknown>] =
            app instanceof ActorSheetXPRPG
                ? [app.actor, app.actor.getRollData()]
                : app instanceof ItemSheetXPRPG
        ...
        const speaker = ChatMessageXPRPG.getSpeaker({ actor });
    ...
    static processUserVisibility(content: string, options: EnrichHTMLOptionsXPRPG): string {
        const $html = $("<div>").html(content);
        const document = options.rollData?.actor ?? null;
        UserVisibilityXPRPG.process($html, { document });
    ...
    static async #localize(paramString: string, options: EnrichHTMLOptionsXPRPG = {}): Promise<HTMLElement | null> {
    ...
    static #createTemplate(paramString: string, label?: string, itemData?: ItemSystemData): HTMLSpanElement | null {
        ...
        if (!params.type) {
            ui.notifications.error(game.i18n.localize("XPRPG.InlineTemplateErrors.TypeMissing"));
        } else if (!params.distance) {
            ui.notifications.error(game.i18n.localize("XPRPG.InlineTemplateErrors.DistanceMissing"));
            return null;
        } else if (!objectHasKey(CONFIG.XPRPG.areaTypes, params.type)) {
            ui.notifications.error(
                game.i18n.format("XPRPG.InlineTemplateErrors.TypeUnsupported", { type: params.type })
            );
            return null;
        } else if (isNaN(+params.distance)) {
            ui.notifications.error(
                game.i18n.format("XPRPG.InlineTemplateErrors.DistanceNoNumber", { distance: params.distance })
            );
            return null;
        } else if (params.width && isNaN(+params.width)) {
            ui.notifications.error(
                game.i18n.format("XPRPG.InlineTemplateErrors.WidthNoNumber", { width: params.width })
            );
            return null;
        } else {
            ...
            if (!label) {
                label = game.i18n.format("XPRPG.TemplateLabel", {
                    size: params.distance,
                    unit: game.i18n.localize("XPRPG.Foot"),
                    shape: game.i18n.localize(CONFIG.XPRPG.areaTypes[params.type]),
                });
            }
            ...
            html.setAttribute("data-xprpg-effect-area", params.type);
            html.setAttribute("data-xprpg-distance", params.distance);
            if (params.traits !== "") html.setAttribute("data-xprpg-traits", params.traits);
            if (params.type === "line") html.setAttribute("data-xprpg-width", params.width ?? "5");
            return html;
        }
        return null;
    }
    ...
    static #createCheck({
        ...
        if (!params.type) {
            ui.notifications.warn(game.i18n.localize("XPRPG.InlineCheck.Errors.TypeMissing"));
            return null;
        }
        ...
        const html = document.createElement("span");
        html.setAttribute("data-xprpg-traits", `${allTraits}`);
        const name = params.name ?? item?.name ?? params.type;
        html.setAttribute("data-xprpg-label", game.i18n.format("XPRPG.InlineCheck.DCWithName", { name }));
        html.setAttribute("data-xprpg-repost-flavor", name);
        const role = params.showDC ?? "owner";
        html.setAttribute("data-xprpg-show-dc", params.showDC ?? role);
        html.setAttribute("data-xprpg-adjustment", params.adjustment ?? "");

        switch (params.type) {
            case "flat":
                html.innerHTML = inlineLabel ?? game.i18n.localize("XPRPG.FlatCheck");
                html.setAttribute("data-xprpg-check", "flat");
                break;
            case "perception":
                html.innerHTML = inlineLabel ?? game.i18n.localize("XPRPG.PerceptionLabel");
                html.setAttribute("data-xprpg-check", "perception");
                break;
            case "fortitude":
            case "reflex":
            case "will": {
                const saveName = game.i18n.localize(CONFIG.XPRPG.saves[params.type]);
                const saveLabel =
                    params.basic === "true"
                        ? game.i18n.format("XPRPG.InlineCheck.BasicWithSave", { save: saveName })
                        : saveName;
                html.innerHTML = inlineLabel ?? saveLabel;
                html.setAttribute("data-xprpg-check", params.type);
                break;
            }
            default: {
                // Skill or Lore
                ...
                const skillLabel = shortForm
                    ? game.i18n.localize(CONFIG.XPRPG.skills[shortForm])
                    : params.type
                          .split("-")
                          .map((word) => {
        ...
        if (params.type && params.dc) {
            ...
            html.setAttribute("data-xprpg-dc", checkDC);
            const text = html.innerHTML;
            if (checkDC !== "@self.level") {
                html.innerHTML = game.i18n.format("XPRPG.DCWithValueAndVisibility", { role, dc: checkDC, text });
            }
        }
        return html;
    }
...
interface EnrichHTMLOptionsXPRPG extends EnrichHTMLOptions {
    rollData?: {
        actor?: ActorXPRPG | null;
        item?: ItemXPRPG | null;
        mod?: number;
...
interface ConvertXMLNodeOptions {
    ...
    visible?: boolean;
    /**
     * Whether this piece of data belongs to the "self" actor or the target: used by UserVisibilityXPRPG to
...
export { TextEditorXPRPG, EnrichHTMLOptionsXPRPG };
