...
abstract class SettingsMenuXPRPG extends FormApplication {
    ...
    static override get defaultOptions() {
        ...
        return mergeObject(options, {
            title: `XPRPG.SETTINGS.${this.namespace.titleCase()}.Name`,
            id: `${this.namespace}-settings`,
            template: `systems/xprpg/templates/system/settings/menu.hbs`,
            width: 550,
            height: "auto",
    ...
    static registerSettings(): void {
        ...
        for (const setting of this.SETTINGS) {
            game.settings.register("xprpg", `${this.prefix}${setting}`, {
                ...settings[setting],
                scope: "world",
                config: false,
            });
    ...
    protected override _injectHTML($html: JQuery<HTMLElement>): void {
        ...
        for (const key of (this.constructor as typeof SettingsMenuXPRPG).SETTINGS) {
            const settingKey = `${this.prefix}${key}`;
            this.cache[key] = game.settings.get("xprpg", settingKey);
        }
    }
}

interface SettingsMenuXPRPG extends FormApplication {
    constructor: typeof SettingsMenuXPRPG;
}
...
function settingsToSheetData(
    ...
    return Object.entries(settings).reduce((result: Record<string, SettingsTemplateData>, [key, setting]) => {
        ...
        const value = key in cache ? cache[key] : game.settings.get("xprpg", lookupKey);
        cache[key] = value;
        result[key] = {
            ...setting,
            key,
...
export { MenuTemplateData, SettingsMenuXPRPG, SettingsTemplateData, settingsToSheetData };
