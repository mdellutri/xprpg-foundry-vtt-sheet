import { PredicateXPRPG, PredicateStatement, RawPredicate, StatementValidator } from "@system/predication";
...
class PredicateField<
    ...
    THasInitial extends boolean = true
> extends fields.ArrayField<PredicateStatementField, RawPredicate, PredicateXPRPG, TRequired, TNullable, THasInitial> {
    ...
    /** Construct a `PredicateXPRPG` from the initialized `PredicateStatement[]` */
    override initialize(
        value: RawPredicate,
        model: ConstructorOf<DataModel>,
        options?: ArrayFieldOptions<PredicateStatementField, TRequired, TNullable, THasInitial>
    ): MaybeSchemaProp<PredicateXPRPG, TRequired, TNullable, THasInitial>;
    override initialize(
        value: RawPredicate,
        model: ConstructorOf<DataModel>,
        options: ArrayFieldOptions<PredicateStatementField, TRequired, TNullable, THasInitial>
    ): PredicateXPRPG | null | undefined {
        const statements = super.initialize(value, model, options);
        return statements ? new PredicateXPRPG(...statements) : statements;
    }
}
