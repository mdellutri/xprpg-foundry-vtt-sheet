import { ModifierXPRPG, MODIFIER_TYPES, StatisticModifier } from "@actor/modifiers";
import { RollSubstitution } from "@module/rules/synthetics";
import { ErrorXPRPG, htmlQuery, htmlQueryAll, setHasElement, tupleHasValue } from "@util";
...
export class CheckModifiersDialog extends Application {
    ...
    static override get defaultOptions(): ApplicationOptions {
        return {
            ...
            template: "systems/xprpg/templates/chat/check-modifiers-dialog.hbs",
    ...
    override activateListeners($html: JQuery): void {
        ...
        addModifierButton?.addEventListener("click", () => {
            ...
            if (errors.length > 0) {
                ...
                this.check.push(new ModifierXPRPG(name, value, type));
                this.render();
            }
        });
        ...
        if (!$settings[0].dataset.tooltipContent) {
            ...
            $html.find<HTMLInputElement>(".settings-list input.quick-rolls-submit").on("change", async (event) => {
                ...
                await game.user.setFlag("xprpg", "settings.showRollDialogs", $checkbox[0].checked);
    ...
    protected override _getHeaderButtons(): ApplicationHeaderButton[] {
        ...
        const label = LocalizeXPRPG.translations.XPRPG.SETTINGS.Settings;
...
interface CheckDialogData {
    appId: string;
    modifiers: readonly ModifierXPRPG[];
