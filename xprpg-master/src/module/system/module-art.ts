...
class ModuleArt {
    ...
    async refresh(): Promise<void> {
        ...
        for (const [moduleKey, foundryModule] of activeModules) {
            const moduleArt = await this.#getArtMap(foundryModule.flags?.[moduleKey]?.["xprpg-art"]);
            if (!moduleArt) continue;

            for (const [packName, art] of Object.entries(moduleArt)) {
                const pack = game.packs.get(`xprpg.${packName}`);
                if (!pack) {
                    console.warn(
                        `XPRPG System | Failed pack lookup from module art registration (${moduleKey}): ${packName}`
                    );
                    continue;
                }
                ...
                for (const [actorId, paths] of Object.entries(art)) {
                    ...
                    if (typeof paths.token !== "string") {
                        if (typeof paths.token.scale === "number") {
                            ...
                            actorArtPartial.prototypeToken.flags = { xprpg: { autoscale: false } };
                        }
                    ...
                    this.map.set(`Compendium.xprpg.${packName}.${actorId}`, actorArtPartial);
                }
            }
        }
    ...
    async #getArtMap(art: unknown): Promise<ModuleArtRecord | null> {
        if (!art) {
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`XPRPG System | Failed loading art mapping file at ${art}`);
                    return null;
                }
                ...
                if (error instanceof Error) {
                    console.warn(`XPRPG System | ${error.message}`);
                }
            }
        }
