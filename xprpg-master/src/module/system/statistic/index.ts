...
import { CheckXPRPG, CheckRoll, CheckRollCallback, CheckRollContext, CheckType, RollTwiceOption } from "@system/check";
...
export class Statistic {
    ...
    abilityModifier: ModifierXPRPG | null = null;
    ...
    constructor(public actor: ActorXPRPG, data: StatisticData, public options?: RollOptionParameters) {
        this.#data = data;
        this.slug = data.slug;
        ...
        const baseModifiers: ModifierXPRPG[] = [];
...
class StatisticCheck {
    ...
    #calculateLabel(data: StatisticData) {
        ...
        switch (this.type) {
            case "skill-check":
                return game.i18n.format("XPRPG.SkillCheckWithName", { skillName: parentLabel });
            case "saving-throw":
                return game.i18n.format("XPRPG.SavingThrowWithName", { saveName: parentLabel });
            case "spell-attack-roll":
                return game.i18n.format("XPRPG.SpellAttackWithTradition", { tradition: parentLabel });
            default:
                return parentLabel;
        }
    }
    ...
    async roll(args: StatisticRollParameters = {}): Promise<Rolled<CheckRoll> | null> {
        ...
        if ((options.has("incapacitation") || options.has("item:trait:incapacitation")) && dc) {
            ...
            if (amount) {
                dosAdjustments.push({
                    adjustments: {
                        all: {
                            label: "XPRPG.TraitIncapacitation",
                            amount,
                        },
                    },
                });
        ...
        if (mapIncreases !== 0) {
            if (!item) {
                ...
                extraModifiers.push(new ModifierXPRPG(maps.label, penalty, "untyped"));
            }
        }

        // Process any given action traits, then add to roll options
        const traits = args.traits?.map((t) =>
            typeof t === "string" ? traitSlugToObject(t, CONFIG.XPRPG.actionTraits) : t
        );
...
class StatisticDifficultyClass {
    ...
    get breakdown() {
        ...
        return [game.i18n.localize("XPRPG.DCBase")]
