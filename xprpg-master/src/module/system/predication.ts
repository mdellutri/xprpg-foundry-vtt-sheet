 ...
 * @category XPRPG
 */
class PredicateXPRPG extends Array<PredicateStatement> {
    ...
    constructor(...statements: PredicateStatement[] | [PredicateStatement[]]) {
        ...
        this.isValid = PredicateXPRPG.isValid(this);
    }
    ...
    static test(predicate: PredicateStatement[] = [], options: Set<string> | string[]): boolean {
        return predicate instanceof PredicateXPRPG
            ? predicate.test(options)
            : new PredicateXPRPG(...predicate).test(options);
    }

    /** Create a predicate from unknown data, with deprecation support for legacy objects */
    static create(data: unknown, warn = false): PredicateXPRPG {
        if (data instanceof PredicateXPRPG) return data.clone();
        if (Array.isArray(data)) return new PredicateXPRPG(data);
        if (isObject<OldRawPredicate>(data)) {
            ...
            return new PredicateXPRPG(convertLegacyData(data));
        }

        return new PredicateXPRPG();
    }
    ...
    test(options: Set<string> | string[]): boolean {
        if (this.length === 0) {
            ...
            console.error("XPRPG System | The provided predicate set is malformed.");
            return false;
        }
    ...
    clone(): PredicateXPRPG {
        return new PredicateXPRPG(this.toObject());
    }
        ...
        if ("eq" in statement) {
            ...
            switch (operator) {
                ...
                case "lte":
                    return leftValues.some((l) => rightValues.every((r) => l <= r));
                default:
                    console.warn("XPRPG System | Malformed binary operation encounter");
                    return false;
            }
        }
    }
...
export { PredicateXPRPG, PredicateStatement, RawPredicate, StatementValidator, convertLegacyData };
