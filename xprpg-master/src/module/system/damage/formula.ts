...
function createDamageFormula(
    ...
    if (degree === DEGREE_OF_SUCCESS.CRITICAL_FAILURE) {
        ...
        damage.dice = damage.dice.filter((d): d is DamageDiceXPRPG => d.category === "splash");
    ...
    const modifiers = damage.modifiers
        .filter((m) => m.enabled)
        .flatMap((modifier): ModifierXPRPG | never[] => {
    ...
    return Array.from(typeMap.entries()).flatMap(([damageType, typePartials]): AssembledFormula | never[] => {
        ...
        const nonCriticalDamage = ((): string | null => {
            ...
            const doubleDice =
                ...
                game.settings.get("xprpg", "critRule") === "doubledice";
        ...
        const breakdown = (() => {
            ...
            const damageTypeLabel =
                breakdownDamage[0].category === "persistent"
                    ? game.i18n.format("XPRPG.Damage.PersistentTooltip", {
                          damageType: game.i18n.localize(CONFIG.XPRPG.damageTypes[damageType] ?? damageType),
                      })
                    : game.i18n.localize(CONFIG.XPRPG.damageTypes[damageType] ?? damageType);
