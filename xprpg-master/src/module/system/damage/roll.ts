...
import { ErrorXPRPG, fontAwesomeIcon, isObject, objectHasKey, setHasElement, tupleHasValue } from "@util";
...
abstract class AbstractDamageRoll extends Roll {
    ...
    constructor(formula: string, data = {}, options: DamageInstanceData = {}) {
        options.critRule = ((): CriticalDoublingRule => {
            ...
            return game.settings.get("xprpg", "critRule") === "doubledamage" ? "double-damage" : "double-dice";
        })();
    ...
    protected override _evaluateSync(): never {
        throw ErrorXPRPG("Damage rolls must be evaluated asynchronously");
    }
}
...
class DamageRoll extends AbstractDamageRoll {
    ...
    constructor(formula: string, data = {}, options: DamageRollDataXPRPG = {}) {
    ...
    static override CHAT_TEMPLATE = "systems/xprpg/templates/dice/damage-roll.hbs";

    static override TOOLTIP_TEMPLATE = "systems/xprpg/templates/dice/damage-tooltip.hbs";
    ...
    static override parse(formula: string, data: Record<string, unknown>): InstancePool[] {
        ...
        if (!poolData) {
            ...
            throw ErrorXPRPG("A damage roll must consist of a single InstancePool");
        }
    ...
    static classifyDice(data: RollTermData): void {
        ...
        for (const term of diceTerms) {
            if (typeof term.faces === "number" || term.faces instanceof Object) {
                ...
                if (!termClassName) throw ErrorXPRPG(`No matching DiceTerm class for "${term.faces}"`);
    ...
    override async render({
        ...
        const chatData = {
            ...
            showTripleDamage: game.settings.get("xprpg", "critFumbleButtons"),
        };
...
class DamageInstance extends AbstractDamageRoll {
    ...
    constructor(formula: string, data = {}, options: DamageInstanceData = {}) {
        ...
        this.materials = flavorIdentifiers.filter((i): i is MaterialDamageEffect =>
            objectHasKey(CONFIG.XPRPG.materialDamageEffects, i)
        );
    }
    ...
    override get formula(): string {
        const typeFlavor = game.i18n.localize(CONFIG.XPRPG.damageRollFlavors[this.type] ?? this.type);
        const damageType =
            this.persistent && this.type !== "bleed"
                ? game.i18n.format("XPRPG.Damage.RollFlavor.persistent", { damageType: typeFlavor })
                : this.type !== "untyped"
                ? typeFlavor
                : "";
    ...
    get typeLabel(): string {
        const damageType = game.i18n.localize(CONFIG.XPRPG.damageTypes[this.type]);
        return this.persistent && this.type !== "bleed"
            ? game.i18n.format("XPRPG.Damage.PersistentTooltip", { damageType })
            : damageType;
    }
    ...
    componentTotal(component: "precision" | "splash"): number {
        if (!this._evaluated) {
            throw ErrorXPRPG("Component totals may only be accessed from an evaluated damage instance");
        }
...
interface DamageRollDataXPRPG extends RollDataXPRPG, AbstractDamageRollData {
...
export { DamageInstance, DamageRoll, DamageRollDataXPRPG };
