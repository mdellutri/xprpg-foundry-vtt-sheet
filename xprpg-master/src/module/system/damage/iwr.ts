...
function applyIWR(actor: ActorXPRPG, roll: Rolled<DamageRoll>, rollOptions: Set<string>): IWRApplicationData {
    // Skip the whole exercise if the actor is dead
    if (actor.isDead) {
        return { finalDamage: 0, applications: [], persistent: [] };
    }

    if (!game.settings.get("xprpg", "automation.iwr")) {
        return {
...
async function maxPersistentAfterIWR(
    ...
    const ConditionXPRPG = CONFIG.XPRPG.Item.documentClasses.condition;
    const { damage, damageType } = new ConditionXPRPG(data, { ready: true }).system.persistent!;
