import { ActorXPRPG, CharacterXPRPG, HazardXPRPG, NPCXPRPG } from "@actor";
...
class WeaponDamageXPRPG {
    static async fromNPCAttack({
        ...
        const labelFromCategory = {
            ...
            splash: "XPRPG.WeaponSplashDamageLabel",
        };
        ...
        return WeaponDamageXPRPG.calculate({
            weapon: attack,
            actor,
            damageDice,
            modifiers,
    ...
    static async calculate({
        ...
        const materialTraits = weapon.isOfType("melee")
            ? weapon.system.traits.value.filter(
                  (t): t is MaterialDamageEffect => t in CONFIG.XPRPG.materialDamageEffects
              )
            : [];
        ...
        if (actor.isOfType("character", "npc")) {
            ...
            const modifierValue = WeaponDamageXPRPG.#strengthModToDamage(weapon)
                ? strengthModValue
            ...
            if (weapon.isOfType("weapon") && typeof modifierValue === "number") {
                const strModifier = new ModifierXPRPG({
                    slug: "str",
                    label: CONFIG.XPRPG.abilities.str,
                    ability: "str",
        ...
        if (weapon.isOfType("weapon")) {
            ...
            if (weaponTraits.includes("kickback")) {
                ...
                modifiers.push(new ModifierXPRPG({ label: CONFIG.XPRPG.weaponTraits.kickback, modifier: 1 }));
            }
            ...
            if (splashDamage > 0) {
                const modifier = new ModifierXPRPG({
                    slug: "splash",
                    label: "XPRPG.WeaponSplashDamageLabel",
            ...
            if (bonusDamage > 0) {
                modifiers.push(
                    new ModifierXPRPG({
                        label: "XPRPG.WeaponBonusDamageLabel",
            ...
            if (normalDice > 0) {
                ...
                damageDice.push(
                    new DamageDiceXPRPG({
                        ...
                        slug: "custom",
                        label: "XPRPG.WeaponCustomDamageLabel",
        ...
        if (baseDamage.persistent?.faces) {
            damageDice.push(
                new DamageDiceXPRPG({
                    ...
                    label: "XPRPG.ConditionTypePersistent",
        ...
        modifiers.push(...critSpecEffect.filter((e): e is ModifierXPRPG => e instanceof ModifierXPRPG));
        damageDice.push(...critSpecEffect.filter((e): e is DamageDiceXPRPG => e instanceof DamageDiceXPRPG));
        ...
        const ignoredResistances = propertyRunes.flatMap(
            (r) => CONFIG.XPRPG.runes.weapon.property[r].damage?.ignoredResistances ?? []
        );

        // Backstabber trait
        if (weaponTraits.some((t) => t === "backstabber") && options.has("target:condition:flat-footed")) {
            const modifier = new ModifierXPRPG({
                label: CONFIG.XPRPG.weaponTraits.backstabber,
        ...
        // Deadly trait
        const traitLabels: Record<string, string> = CONFIG.XPRPG.weaponTraits;
        ...
        if (weaponSpecializationDamage > 0) {
            ...
            if (has("greater-weapon-specialization", "Greater Weapon Specialization")) {
                modifiers.push(
                    new ModifierXPRPG({
                        label: "XPRPG.GreaterWeaponSpecialization",
                ...
                modifiers.push(
                    new ModifierXPRPG({
                        label: "XPRPG.WeaponSpecialization",
        ...
        const runeNotes = propertyRunes.flatMap((r) => {
            const data = CONFIG.XPRPG.runes.weapon.property[r].damage?.notes ?? [];
            return data.map((d) => new RollNoteXPRPG({ selector: "strike-damage", ...d }));
        });

        const notes = [
            runeNotes,
            extractNotes(actor.synthetics.rollNotes, selectors),
            critSpecEffect.filter((e): e is RollNoteXPRPG => e instanceof RollNoteXPRPG),
        ]
            .flat()
            .filter((n) => n.predicate.test(options));

        // Accumulate damage-affecting precious materials
        const material = objectHasKey(CONFIG.XPRPG.materialDamageEffects, weapon.system.material.precious?.type)
        ...
        return {
            name: `${game.i18n.localize("XPRPG.DamageRoll")}: ${weapon.name}`,
            notes,
    ...
    static #finalizeDamage(damage: DamageFormulaData, degree: DegreeOfSuccessIndex): AssembledFormula | null {
        ...
        const damageOverrides = damage.dice.filter(
            (d): d is DamageDiceXPRPG & { override: DamageDiceOverride } => !!(d.enabled && d.override)
        );
    ...
    static #strengthBasedDamage(weapon: WeaponXPRPG | MeleeXPRPG): boolean {
    ...
    static #strengthModToDamage(weapon: WeaponXPRPG | MeleeXPRPG): boolean {
...
interface WeaponDamageCalculateParams {
    weapon: WeaponXPRPG | MeleeXPRPG;
    actor: CharacterXPRPG | NPCXPRPG | HazardXPRPG;
    actionTraits: TraitViewData[];
    proficiencyRank: number;
    weaponPotency?: PotencySynthetic | null;
    damageDice?: DamageDiceXPRPG[];
...
interface ExcludeDamageParams {
    actor: ActorXPRPG;
    modifiers: (DiceModifierXPRPG | ModifierXPRPG)[];
    weapon: WeaponXPRPG | null;
    options: Set<string>;
}

export { ConvertedNPCDamage, WeaponDamageXPRPG };
