    ...
    const { notes, title, variant } = (() => {
        switch (options?.variant) {
            case "stabilize":
                return {
                    notes: {
                        criticalFailure: "XPRPG.Actions.AdministerFirstAid.Stabilize.Notes.criticalFailure",
                        success: "XPRPG.Actions.AdministerFirstAid.Stabilize.Notes.success",
                    },
                    title: "XPRPG.Actions.AdministerFirstAid.Stabilize.Title",
                    variant: options.variant,
                };
            case "stop-bleeding":
                return {
                    notes: {
                        criticalFailure: "XPRPG.Actions.AdministerFirstAid.StopBleeding.Notes.criticalFailure",
                        success: "XPRPG.Actions.AdministerFirstAid.StopBleeding.Notes.success",
                    },
                    title: "XPRPG.Actions.AdministerFirstAid.StopBleeding.Title",
                    variant: options.variant,
                };
            default: {
                const variant = options?.variant ? `'${options.variant}'` : "null";
                const variants = ADMINISTER_FIRST_AID_VARIANTS.map((v) => `'${v}'`).join(", ");
                const error = "XPRPG.Actions.AdministerFirstAid.Warning.UnknownVariant";
            ...
            if (variant === "stabilize" && target) {
                ...
                if (dying?.value) {
                    ...
                    return new Statistic(target, {
                        ...
                        label: "XPRPG.Actions.AdministerFirstAid.Stabilize.Title",
                        dc: { base: dc, label: "XPRPG.Actions.AdministerFirstAid.Stabilize.DifficultyClass.Label" },
                    });
                }
                ui.notifications.warn(game.i18n.localize("XPRPG.Actions.AdministerFirstAid.Warning.TargetNotDying"));
            }
            return null;
        },
