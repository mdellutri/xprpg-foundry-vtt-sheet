...
import type { EffectXPRPG } from "@item/index";
import { EncounterXPRPG } from "@module/encounter";

export class EffectTracker {
    effects: Embedded<EffectXPRPG>[] = [];

    /** A separate collection of aura effects, including ones with unlimited duration */
    auraEffects: Collection<Embedded<EffectXPRPG>> = new Collection();

    private insert(effect: Embedded<EffectXPRPG>, duration: { expired: boolean; remaining: number }): void {
    ...
    register(effect: Embedded<EffectXPRPG>): void {
    ...
    unregister(toRemove: Embedded<EffectXPRPG>): void {
    ...
    async #removeExpired(actor: ActorXPRPG): Promise<void> {
    ...
    async onEncounterEnd(encounter: EncounterXPRPG): Promise<void> {
        const autoRemoveExpired = game.settings.get("xprpg", "automation.removeExpiredEffects");
        const autoExpireEffects = !autoRemoveExpired && game.settings.get("xprpg", "automation.effectExpiration");
