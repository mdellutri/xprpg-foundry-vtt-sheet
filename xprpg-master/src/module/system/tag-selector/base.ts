...
abstract class BaseTagSelector<
    TDocument extends ActorXPRPG | ItemXPRPG = ActorXPRPG | ItemXPRPG
> extends FormApplication<TDocument> {
    ...
    /**
     * Builds an object of all keys of this.configTypes from CONFIG.XPRPG
    ...
    private getChoices(): Record<string, string> {
        const choices = this.configTypes.reduce(
            (types, key) => mergeObject(types, CONFIG.XPRPG[key]),
