...
import { ItemXPRPG } from "@item/index";
...
export interface BasicSelectorOptions extends TagSelectorOptions {
    ...
    /* An array of keys from CONFIG.XPRPG */
...
class TagSelectorBasic<TDocument extends ActorXPRPG | ItemXPRPG> extends BaseTagSelector<TDocument> {
    allowCustom: boolean;
    ...
    constructor(object: TDocument, options: BasicConstructorOptions) {
        ...
        this.options.title = options.title ?? "XPRPG.TraitsLabel";
    }
    ...
    static override get defaultOptions(): TagSelectorOptions {
        return mergeObject(super.defaultOptions, {
            template: "systems/xprpg/templates/system/tag-selector/basic.hbs",
            height: 710,
        });
    }
    ...
    override async getData(): Promise<TagSelectorBasicData<TDocument>> {
        const { chosen, custom, flat, disabled } = (() => {
            const document: { toObject(): ActorSourceXPRPG | ItemSourceXPRPG } = this.object;
...
interface TagSelectorBasic<TDocument extends ActorXPRPG | ItemXPRPG> extends BaseTagSelector<TDocument> {
    options: BasicSelectorOptions;
}

interface TagSelectorBasicData<TDocument extends ActorXPRPG | ItemXPRPG> extends FormApplicationData<TDocument> {
