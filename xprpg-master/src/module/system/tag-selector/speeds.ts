...
export class SpeedSelector<TActor extends ActorXPRPG> extends BaseTagSelector<TActor> {
    protected objectProperty = "system.attributes.speed.otherSpeeds";

    static override get defaultOptions(): TagSelectorOptions {
        return mergeObject(super.defaultOptions, {
            template: "systems/xprpg/templates/system/tag-selector/speeds.hbs",
            title: "XPRPG.SpeedTypes",
        });
    }
    ...
    override async getData(): Promise<SpeedSelectorData<TActor>> {
        if (!this.object.isOfType("creature")) {
            throw ErrorXPRPG("The Speed selector is usable only with creature-type actors");
        }

        const speeds = this.object.system.attributes.speed.otherSpeeds;
        const speedLabels: Record<string, string> = CONFIG.XPRPG.speedTypes;
...
interface SpeedSelectorData<TActor extends ActorXPRPG> extends FormApplicationData<TActor> {
