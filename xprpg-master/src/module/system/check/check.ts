...
import { ChatMessageSourceXPRPG, CheckRollContextFlag, TargetFlag } from "@module/chat-message/data";
import { RollNoteXPRPG } from "@module/notes";
import { TokenDocumentXPRPG } from "@scene";
import { eventToRollParams } from "@scripts/sheet-util";
import { ErrorXPRPG, fontAwesomeIcon, objectHasKey, parseHTML, signedInteger, sluggify, traitSlugToObject } from "@util";
import { CheckModifier, StatisticModifier } from "@actor/modifiers";
import { CheckModifiersDialog } from "../check-modifiers-dialog";
import { CheckRoll, CheckRollDataXPRPG } from "./roll";
import {
...
import { LocalizeXPRPG } from "../localize";
import { TextEditorXPRPG } from "../text-editor";
...
class CheckXPRPG {
    ...
    static async roll(
        ...
        const [dice, tagsFromDice] = ((): [string, string[]] => {
            ...
            if (rollOptions.has("fortune") && rollOptions.has("misfortune")) {
                return ["1d20", ["XPRPG.TraitFortune", "XPRPG.TraitMisfortune"]];
            } else if (substitution) {
                const effectType = {
                    fortune: "XPRPG.TraitFortune",
                    misfortune: "XPRPG.TraitMisfortune",
                }[substitution.effectType];
                const extraTag = game.i18n.format("XPRPG.SpecificRule.SubstituteRoll.EffectType", {
                ...
                return ["2d20kl", ["XPRPG.TraitMisfortune"]];
            } else if (context.rollTwice === "keep-higher") {
                return ["2d20kh", ["XPRPG.TraitFortune"]];
            } else {
                return ["1d20", []];
            }
        })();
        ...
        const options: CheckRollDataXPRPG = { rollerId: game.userId, isReroll, totalModifier: check.totalModifier };
        ...
        const notes = context.notes?.map((n) => (n instanceof RollNoteXPRPG ? n : new RollNoteXPRPG(n))) ?? [];
        const notesText =
            notes
                .filter((note) => {
        ...
        const contextFlag: CheckRollContextFlag = {
            ...
            title: context.title ?? "XPRPG.Check.Label",
        ...
        type MessagePromise = Promise<ChatMessageXPRPG | ChatMessageSourceXPRPG>;
        const message = await ((): MessagePromise => {
            ...
            const speaker = ChatMessageXPRPG.getSpeaker({ actor: context.actor, token: context.token });
        ...
        if (callback) {
            const msg = message instanceof ChatMessageXPRPG ? message : new ChatMessageXPRPG(message);
        ...
        const isConsumableWeapon = item instanceof WeaponXPRPG && item.traits.has("consumable");
    ...
    private static createTagFlavor({ check, context, extraTags }: CreateTagFlavorParams): HTMLElement[] {
        ...
        const itemTraits = item?.isOfType("weapon", "melee")
            ? Array.from(item.traits)
                  .map((t): TraitViewData => {
                      const obj = traitSlugToObject(t, CONFIG.XPRPG.npcAttackTraits);
            ...
            if (item?.isOfType("weapon") && item.isRanged) {
                ...
                const locKey =
                    item.maxRange === rangeIncrement
                        ? `XPRPG.TraitRange${rangeIncrement}`
                        : "XPRPG.Item.Weapon.RangeIncrementN.Label";
                    ...
                    toTagElement(
                        {
                            ...
                            label,
                            description: "XPRPG.Item.Weapon.RangeIncrementN.Hint",
                        },
                        "secondary"
                    ),
                ];
    ...
    static async rerollFromMessage(message: ChatMessageXPRPG, { heroPoint = false, keep = "new" }: RerollOptions = {}) {
        if (!(message.isAuthor || game.user.isGM)) {
            ui.notifications.error(game.i18n.localize("XPRPG.RerollMenu.ErrorCantDelete"));
            return;
        }

        const actor = game.actors.get(message.speaker.actor ?? "");
        let rerollFlavor = game.i18n.localize(`XPRPG.RerollMenu.MessageKeep.${keep}`);
        if (heroPoint) {
            // If the reroll costs a hero point, first check if the actor has one to spare and spend it
            if (actor instanceof CharacterXPRPG) {
                const heroPointCount = actor.heroPoints.value;
                if (heroPointCount) {
                    await actor.update({
                        "system.resources.heroPoints.value": Math.clamped(heroPointCount - 1, 0, 3),
                    });
                    rerollFlavor = game.i18n.format("XPRPG.RerollMenu.MessageHeroPoint", { name: actor.name });
                } else {
                    ui.notifications.warn(game.i18n.format("XPRPG.RerollMenu.WarnNoHeroPoint", { name: actor.name }));
                    return;
                }
            } else {
                ui.notifications.error(game.i18n.localize("XPRPG.RerollMenu.ErrorNoActor"));
                return;
            }
        }

        const systemFlags = deepClone(message.flags.xprpg);
        ...
        if (!(oldRoll instanceof CheckRoll)) throw ErrorXPRPG("Unexpected error retrieving prior roll");
        ...
        let [oldRollClass, newRollClass] = ["xprpg-reroll-discard", ""];
        ...
        const renders = {
            old: await CheckXPRPG.renderReroll(oldRoll, { isOld: true }),
            new: await CheckXPRPG.renderReroll(newRoll, { isOld: false }),
        };

        const rerollIcon = fontAwesomeIcon(heroPoint ? "hospital-symbol" : "dice");
        rerollIcon.classList.add("xprpg-reroll-indicator");
        ...
        await keptRoll.toMessage(
            {
                content: `<div class="${oldRollClass}">${renders.old}</div><div class="xprpg-reroll-second ${newRollClass}">${renders.new}</div>`,
    ...
    static async renderReroll(roll: Rolled<Roll>, { isOld }: { isOld: boolean }): Promise<string> {
        ...
        if (typeof die?.total !== "number") throw ErrorXPRPG("Unexpected error inspecting d20 term");
    ...
    private static async createResultFlavor({ degree, target }: CreateResultFlavorParams): Promise<HTMLElement | null> {
        ...
        const targetActor = await (async (): Promise<ActorXPRPG | null> => {
            if (!target?.actor) return null;
            if (target.actor instanceof ActorXPRPG) return target.actor;

            // This is a context flag: get the actor via UUID
            const maybeActor = await fromUuid(target.actor);
            return maybeActor instanceof ActorXPRPG
                ? maybeActor
                : maybeActor instanceof TokenDocumentXPRPG
                ? maybeActor.actor
                : null;
        })();
        ...
        const targetData = await (async (): Promise<{ name: string; visible: boolean } | null> => {
            ...
            const token = await (async (): Promise<TokenDocumentXPRPG | null> => {
                if (!target.token) return null;
                if (target.token instanceof TokenDocumentXPRPG) return target.token;
            ...
            const canSeeTokenName = (token ?? new TokenDocumentXPRPG(targetActor?.prototypeToken.toObject() ?? {}))
                .playersCanSeeName;
            const canSeeName = canSeeTokenName || !game.settings.get("xprpg", "metagame_tokenSetsNameVisibility");
        ...
        const translations = LocalizeXPRPG.translations.XPRPG.Check;
        ...
        const dcData = ((): ResultFlavorTemplateData["dc"] => {
            ...
            const visible = targetActor?.hasPlayerOwner || dc.visible || game.settings.get("xprpg", "metagame_showDC");
        ...
        const resultData = ((): ResultFlavorTemplateData["result"] => {
            ...
            const locPath = (checkOrAttack: string, dosKey: DegreeOfSuccessString) =>
                `XPRPG.Check.Result.Degree.${checkOrAttack}.${dosKey}`;
            ...
            const markup = game.i18n.format(`XPRPG.Check.Result.${locKey}`, {
                adjusted,
                unadjusted,
                offset: offset.value,
            });
        ...
        const rendered = await renderTemplate("systems/xprpg/templates/chat/check/target-dc-result.hbs", {
            target: targetData,
            dc: dcData,
            result: resultData,
        });
        ...
        const { convertXMLNode } = TextEditorXPRPG;
        ...
        if (dcData.adjustment) {
            ...
            if (!adjustedNode) throw ErrorXPRPG("Unexpected error processing roll template");
...
export { CheckXPRPG, CheckRollCallback };
