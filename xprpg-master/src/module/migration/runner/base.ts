...
interface CollectionDiff<T extends foundry.data.ActiveEffectSource | ItemSourceXPRPG> {
    inserted: T[];
    deleted: string[];
    updated: T[];
}
...
export class MigrationRunnerBase {
    ...
    diffCollection<T extends ItemSourceXPRPG>(orig: T[], updated: T[]): CollectionDiff<T>;
    diffCollection<T extends foundry.data.ActiveEffectSource | ItemSourceXPRPG>(
        orig: T[],
        updated: T[]
    ): CollectionDiff<T>;
    diffCollection<TSource extends foundry.data.ActiveEffectSource | ItemSourceXPRPG>(
        orig: TSource[],
        updated: TSource[]
    ...
    async getUpdatedActor(actor: ActorSourceXPRPG, migrations: MigrationBase[]): Promise<ActorSourceXPRPG> {
    ...
    async getUpdatedItem(item: ItemSourceXPRPG, migrations: MigrationBase[]): Promise<ItemSourceXPRPG> {
    ...
    async getUpdatedToken(token: TokenDocumentXPRPG, migrations: MigrationBase[]): Promise<foundry.data.TokenSource> {
