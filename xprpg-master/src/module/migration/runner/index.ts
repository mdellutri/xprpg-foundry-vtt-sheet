...
import type { MacroXPRPG } from "@module/macro";
import { MigrationRunnerBase } from "@module/migration/runner/base";
import { MigrationBase } from "@module/migration/base";
import type { UserXPRPG } from "@module/user";
...
export class MigrationRunner extends MigrationRunnerBase {
    override needsMigration(): boolean {
        return super.needsMigration(game.settings.get("xprpg", "worldSchemaVersion"));
    }

    /** Ensure that an actor or item reflects the current data schema before it is created */
    static async ensureSchemaVersion(document: ActorXPRPG | ItemXPRPG, migrations: MigrationBase[]): Promise<void> {
    ...
    private async migrateDocuments<TDocument extends ActorXPRPG | ItemXPRPG>(
    ...
    private async migrateWorldMacro(macro: MacroXPRPG, migrations: MigrationBase[]): Promise<void> {
    ...
    private async migrateUser(user: UserXPRPG, migrations: MigrationBase[]): Promise<void> {
    ...
    async runCompendiumMigration<T extends ActorXPRPG | ItemXPRPG>(compendium: CompendiumCollection<T>) {
        ui.notifications.info(game.i18n.format("XPRPG.Migrations.Starting", { version: game.system.version }), {
            permanent: true,
        });
        ...
        ui.notifications.info(game.i18n.format("XPRPG.Migrations.Finished", { version: game.system.version }), {
            permanent: true,
        });
    }
    ...
    async runMigrations(migrations: MigrationBase[]): Promise<void> {
        ...
        await this.migrateDocuments(game.actors as WorldCollection<ActorXPRPG>, migrations);
        ...
        for (const scene of game.scenes) {
            for (const token of scene.tokens) {
                ...
                const hasMigratableData =
                    !!token._source.actorData.flags?.xprpg ||
    ...
    async runMigration(force = false) {
        const schemaVersion = {
            ...
            current: game.settings.get("xprpg", "worldSchemaVersion"),
        };
        const systemVersion = game.system.version;

        ui.notifications.info(game.i18n.format("XPRPG.Migrations.Starting", { version: systemVersion }), {
            permanent: true,
        });
        ...
        await game.settings.set("xprpg", "worldSchemaVersion", schemaVersion.latest);
    }
}
