    ...
    #cleanText(text: string): string {
        return (
            text
                ...
                .replace(/^XPRPG\.WeaponDescription([A-Z][a-z]+)$/, (substring, group) =>
                    typeof group === "string"
                        ? `XPRPG.Item.Weapon.CriticalSpecialization.${group.toLowerCase()}`
                        : substring
                )
        );
    }
    ...
    override async updateItem(source: ItemSourceXPRPG): Promise<void> {
        ...
        for (const note of notes) {
            ...
            if (newTitle === source.name) {
                ...
                note.title = "XPRPG.Actor.Creature.CriticalSpecialization";
