...
export class Migration738UpdateLaughingShadow extends MigrationBase {
    ...
    #shadowPromise = fromUuid("Compendium.xprpg.classfeatures.3gVDqDPSz4fB5T9G");

    #cascadePromise = fromUuid("Compendium.xprpg.feature-effects.fsjO5oTKttsbpaKl");

    override async updateActor(source: ActorSourceXPRPG): Promise<void> {
        const rollOptionsAll = source.flags.xprpg?.rollOptions?.all;
    ...
    override async updateItem(source: ItemSourceXPRPG): Promise<void> {
        if (source.type === "feat" && source.system.slug === "laughing-shadow") {
            ...
            if (!(laughingShadow instanceof ItemXPRPG)) return;
            ...
            if (!(arcaneCascade instanceof ItemXPRPG)) return;
