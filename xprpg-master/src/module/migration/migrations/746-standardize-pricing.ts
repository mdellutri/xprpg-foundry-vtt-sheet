...
export class Migration746StandardizePricing extends MigrationBase {
    ...
    override async updateItem(item: ItemSourceXPRPG): Promise<void> {
        ...
        if (!isObject(item.system.price)) {
            item.system.price = { value: CoinsXPRPG.fromString(String(item.system.price)).strip() };
        }
        ...
        if (item.type === "treasure") {
            ...
            item.system.price.value = CoinsXPRPG.fromString(String(item.system.price.value)).strip();
        }
    }
}
