...
import { EffectSource, ItemSourceXPRPG } from "@item/data";
...
export class Migration723CumulativeItemBonuses extends MigrationBase {
    ...
    private stanceFeats = (async (): Promise<Record<string, FeatXPRPG | undefined>> => {
        const documents: ClientDocument[] = await UUIDUtils.fromUUIDs([
            "Compendium.xprpg.feats-srd.ZPclfDmiHzEqblry", // Animal Skin
            "Compendium.xprpg.feats-srd.ZL5UU9quCTvcWzfY", // Mountain Stance
            "Compendium.xprpg.feats-srd.n2hawNmzW7DBn1Lm", // Mountain Stronghold
            "Compendium.xprpg.feats-srd.hO4sKslTrSQMLbGx", // Mountain Quake
        ]);

        const feats = documents.filter((d): d is FeatXPRPG & { slug: string } => d instanceof FeatXPRPG && !!d.slug);
        return feats.reduce((record: Record<string, FeatXPRPG>, f) => ({ ...record, [f.slug]: f }), {});
    })();
...
const ITEM_UUID = "Compendium.xprpg.feat-effects.gYpy9XBPScIlY93p"; // Stance: Mountain Stance
