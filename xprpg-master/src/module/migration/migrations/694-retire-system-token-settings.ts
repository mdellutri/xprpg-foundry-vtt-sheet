...
export class Migration694RetireSystemTokenSettings extends MigrationBase {
    ...
    override async migrate(): Promise<void> {
        const systemNameHover =
            (Number(game.settings.storage.get("world").getItem("xprpg.defaultTokenSettingsName")) as TokenDisplayMode) ||
            CONST.TOKEN_DISPLAY_MODES.OWNER_HOVER;
        const systemBarHover =
            (Number(game.settings.storage.get("world").getItem("xprpg.defaultTokenSettingsBar")) as TokenDisplayMode) ||
            CONST.TOKEN_DISPLAY_MODES.OWNER_HOVER;
