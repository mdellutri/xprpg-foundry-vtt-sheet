...
import { ErrorXPRPG, fontAwesomeIcon, htmlQueryAll } from "@util";
import { UUIDUtils } from "@util/uuid-utils";
import MiniSearch from "minisearch";

/** Extend CompendiumDirectory to support a search bar */
export class CompendiumDirectoryXPRPG extends CompendiumDirectory {
    ...
    static override get defaultOptions(): ApplicationOptions {
        ...
        return {
            ...
            template: "systems/xprpg/templates/sidebar/compendium-directory.hbs",
        };
    }
    ...
    override async getData(options?: Partial<ApplicationOptions>): Promise<CompendiumDirectoryDataXPRPG> {
        return {
    ...
    override activateListeners($html: JQuery): void {
        ...
        $html[0]!.querySelector("footer > button")?.addEventListener("click", () => {
            game.xprpg.compendiumBrowser.render(true);
        });
    }
        ...
        ContextMenu.create(this, $html, "ol.document-matches > li", [
            {
                ...
                condition: ($li) => {
                    ...
                    if (!uuid) throw ErrorXPRPG("Unexpected missing uuid");
                ...
                callback: ($li) => {
                    ...
                    if (!("_id" in indexData)) throw ErrorXPRPG("Unexpected missing document _id");
    ...
    protected override _onSearchFilter(_event: KeyboardEvent, query: string): void {
        ...
        if (!matchTemplate) throw ErrorXPRPG("Match template not found");
        ...
        for (const compendiumTypeList of htmlQueryAll(this.element[0]!, "li.compendium-type")) {
            ...
            const listElements = typedMatches.map((match): HTMLLIElement => {
                ...
                if (typeof match.img === "string") {
                    thumbnail.src = game.xprpg.system.moduleArt.map.get(matchUUID)?.img ?? match.img;
    ...
    protected override _onDragStart(event: ElementDragEvent): void {
        ...
        if (!indexEntry) throw ErrorXPRPG("Unexpected error retrieving index data");
    ...
    #compileSearchIndex(): void {
        console.debug("XPRPG System | compiling search index");
        ...
        console.debug("XPRPG System | Finished compiling search index");
    }
}

interface CompendiumDirectoryDataXPRPG extends CompendiumDirectoryData {
