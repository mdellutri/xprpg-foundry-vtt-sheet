...
import { ErrorXPRPG, ordinal, tupleHasValue } from "@util";
...
export class WorldClock extends Application {
    /** Localization keys */
    private readonly translations = LocalizeXPRPG.translations.XPRPG.WorldClock;
    ...
    constructor() {
        ...
        const settingValue = game.settings.get("xprpg", "worldClock.worldCreatedOn");
        const defaultValue = game.settings.settings.get("xprpg.worldClock.worldCreatedOn")?.default;
        if (typeof settingValue === "string" && settingValue === defaultValue) {
            game.settings.set("xprpg", "worldClock.worldCreatedOn", settingValue);
        } else if (!DateTime.fromISO(settingValue).isValid) {
            game.settings.set("xprpg", "worldClock.worldCreatedOn", defaultValue);
        }
    }

    /** Setting: the date theme (Imperial Calendar not yet supported) */
    get dateTheme(): "AR" | "IC" | "AD" | "CE" {
        return game.settings.get("xprpg", "worldClock.dateTheme");
    }

    /** Setting: display either a 24-hour or 12-hour clock */
    get timeConvention(): 24 | 12 {
        const setting = game.settings.get("xprpg", "worldClock.timeConvention");
        if (setting !== 24 && setting !== 12) {
            throw Error("XPRPG System | Unrecognized time convention");
        }
        return setting;
    }
    ...
    get syncDarkness(): boolean {
        const sceneSetting = canvas.scene?.flags.xprpg.syncDarkness ?? "default";
        return {
            enabled: true,
            disabled: false,
            default: game.settings.get("xprpg", "worldClock.syncDarkness"),
        }[sceneSetting];
    }

    /** Setting: Date and time of the Foundry world's creation date */
    get worldCreatedOn(): DateTime {
        const value = game.settings.get("xprpg", "worldClock.worldCreatedOn");
    ...
    static override get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            id: "world-clock",
            width: 400,
            template: "systems/xprpg/templates/system/world-clock.hbs",
            title: "XPRPG.WorldClock.Title",
        });
    }
    ...
    private get year(): number {
        const yearOffset = (() => {
            switch (this.dateTheme) {
                case "AR":
                    return CONFIG.XPRPG.worldClock.AR.yearOffset;
                case "AD":
                    return CONFIG.XPRPG.worldClock.AD.yearOffset;
                default:
                    // 'CE'
                    return 0;
            }
    ...
    protected override _getHeaderButtons(): ApplicationHeaderButton[] {
        const settingsButton: ApplicationHeaderButton[] = game.user.isGM
            ? [
                  {
                      label: "XPRPG.SETTINGS.Settings",
                      class: "configure-settings",
                      icon: "fas fa-cog",
                      onclick: (): void => {
                          const menu = game.settings.menus.get("xprpg.worldClock");
                          if (!menu) throw ErrorXPRPG("XPRPG System | World Clock Settings application not found");
    ...
    override activateListeners($html: JQuery): void {
        ...
        for (const eventName of ["keydown.xprpg.world-clock", "keyup.xprpg.world-clock"]) {
    ...
    override async close(options?: { force?: boolean }): Promise<void> {
        $(document).off("keydown.xprpg.world-clock").off("keyup.xprpg.world-clock");
    ...
    static createSyncedMessage(): HTMLSpanElement {
        ...
        managedBy.innerHTML = " ".concat(game.i18n.localize("XPRPG.SETTINGS.WorldClock.SyncDarknessScene.ManagedBy"));
        ...
        anchor.addEventListener("click", (event) => {
            ...
            const menu = game.settings.menus.get("xprpg.worldClock");
            if (!menu) throw ErrorXPRPG("World Clock Settings application not found");
            const app = new menu.type();
            app.render(true);
        });
