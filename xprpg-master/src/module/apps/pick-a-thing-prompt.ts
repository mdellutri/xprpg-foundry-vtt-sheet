...
abstract class PickAThingPrompt<T> extends Application {
    protected item: Embedded<ItemXPRPG>;
    ...
    protected predicate: PredicateXPRPG;
    ...
    constructor(data: PickAThingConstructorArgs<T>) {
        super();
        this.item = data.item;
        this.predicate = data.predicate ?? new PredicateXPRPG();
    ...
    protected getSelection(event: MouseEvent): PickableThing<T> | null {
        if (!(event.currentTarget instanceof HTMLElement)) {
            throw ErrorXPRPG("Unexpected error retrieving form data");
        }
        ...
        if (!this.selection) {
            if (force) {
                ui.notifications.warn(
                    game.i18n.format("XPRPG.UI.RuleElements.Prompt.NoValidOptions", {
                ...
                ui.notifications.warn(
                    game.i18n.format("XPRPG.UI.RuleElements.Prompt.NoSelectionMade", { item: this.item.name })
                );
            }
        }
