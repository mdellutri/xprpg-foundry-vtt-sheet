...
import { KitXPRPG } from "@item/kit";
import { ErrorXPRPG, htmlQueryAll, isObject, objectHasKey } from "@util";
...
import { SpellcastingEntryXPRPG } from "@item";
...
class PackLoader {
    ...
    async *loadPacks(documentType: "Actor" | "Item", packs: string[], indexFields: string[]) {
        ...
        const translations = LocalizeXPRPG.translations.XPRPG.CompendiumBrowser.ProgressBar;
                ...
                if (pack.documentName === documentType) {
                    ...
                    if (firstResult.system) {
                        ...
                        ui.notifications.warn(
                            game.i18n.format("XPRPG.BrowserWarnPackNotLoaded", { pack: pack.collection })
                        );
                        continue;
                    }
                } else {
    ...
    private setModuleArt(packName: string, index: CompendiumIndex): void {
        if (!packName.startsWith("xprpg.")) return;
        for (const record of index) {
            const uuid: CompendiumUUID = `Compendium.${packName}.${record._id}`;
            const actorArt = game.xprpg.system.moduleArt.map.get(uuid)?.img;
...
class CompendiumBrowser extends Application {
    ...
    dataTabsList = ["action", "equipment", "feat", "hazard", "spell"] as const;
    ...
    constructor(options = {}) {
        ...
        this.settings = game.settings.get("xprpg", "compendiumBrowserPacks");
    ...
    override get title(): string {
        return game.i18n.localize("XPRPG.CompendiumBrowser.Title");
    }
    ...
    static override get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            classes: [],
            template: "systems/xprpg/templates/compendium-browser/compendium-browser.hbs",
            width: 800,
            height: 700,
            resizable: true,
    ...
    initCompendiumList(): void {
        ...
        const loadDefault: Record<string, boolean | undefined> = {
            "xprpg.actionsxprpg": true,
            "xprpg.equipment-srd": true,
            "xprpg.ancestryfeatures": true,
            "xprpg.classfeatures": true,
            "xprpg.feats-srd": true,
            "xprpg.spells-srd": true,
        };
    ...
    async openSpellTab(entry: SpellcastingEntryXPRPG, level = 10): Promise<void> {
    ...
    async loadTab(tab: TabName): Promise<void> {
        ...
        if (!this.dataTabsList.includes(tab)) {
            throw ErrorXPRPG(`Unknown tab "${tab}"`);
        }
                    ...
                    if (filterType === "priceRange") {
                        if (filterValue.min !== null && filterValue.min !== undefined) {
                            if (typeof filterValue.min === "number") {
                                ...
                                range.values.min = new CoinsXPRPG({ gp: filterValue.min }).copperValue;
                                range.values.inputMin = filterValue.min + "gp";
                            } else if (typeof filterValue.min === "string") {
                                range.values.min = CoinsXPRPG.fromString(filterValue.min).copperValue;
                        ...
                        if (filterValue.max !== null && filterValue.max !== undefined) {
                            if (typeof filterValue.max === "number") {
                                // Number values are handled as a price in gold pieces
                                range.values.max = new CoinsXPRPG({ gp: filterValue.max }).copperValue;
                                range.values.inputMax = filterValue.max + "gp";
                            } else if (typeof filterValue.max === "string") {
                                range.values.max = CoinsXPRPG.fromString(filterValue.max).copperValue;
        ...
        if (activeTabName === "settings") {
            ...
            if (form) {
                form.querySelector("button.save-settings")?.addEventListener("click", async () => {
                    ...
                    await game.settings.set("xprpg", "compendiumBrowserPacks", this.settings);
                    for (const tab of Object.values(this.tabs)) {
    ...
    private async takePhysicalItem(uuid: string): Promise<void> {
        ...
        if (actors.length === 0) {
            ui.notifications.error(game.i18n.format("XPRPG.ErrorMessage.NoTokenSelected"));
            return;
        }
        ...
        if (actors.length === 1 && game.user.character && actors[0] === game.user.character) {
            ui.notifications.info(
                game.i18n.format("XPRPG.CompendiumBrowser.AddedItemToCharacter", {
                    item: item.name,
                    character: game.user.character.name,
                })
            );
        } else {
            ui.notifications.info(game.i18n.format("XPRPG.CompendiumBrowser.AddedItem", { item: item.name }));
        }
    }
    ...
    private async buyPhysicalItem(uuid: string): Promise<void> {
        ...
        if (actors.length === 1) {
            if (purchasesSucceeded === 1) {
                ui.notifications.info(
                    game.i18n.format("XPRPG.CompendiumBrowser.BoughtItemWithCharacter", {
                        item: item.name,
                ...
                ui.notifications.warn(
                    game.i18n.format("XPRPG.CompendiumBrowser.FailedToBuyItemWithCharacter", {
                        item: item.name,
                        character: actors[0].name,
                    })
                );
            ...
            if (purchasesSucceeded === actors.length) {
                ui.notifications.info(
                    game.i18n.format("XPRPG.CompendiumBrowser.BoughtItemWithAllCharacters", {
                        item: item.name,
                    })
                );
            } else {
                ui.notifications.warn(
                    game.i18n.format("XPRPG.CompendiumBrowser.FailedToBuyItemWithSomeCharacters", {
                        item: item.name,
                    })
                );
            }
    ...
    private async getPhysicalItem(uuid: string): Promise<PhysicalItemXPRPG | KitXPRPG> {
        const item = await UUIDUtils.fromUuid(uuid);
        if (!(item instanceof PhysicalItemXPRPG || item instanceof KitXPRPG)) {
            throw ErrorXPRPG("Unexpected failure retrieving compendium item");
        }
