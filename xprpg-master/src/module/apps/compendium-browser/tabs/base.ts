...
export abstract class CompendiumBrowserTab {
    ...
    getIndexData(start: number): CompendiumBrowserIndexData[] {
        if (!this.isInitialized) {
            throw ErrorXPRPG(`Compendium Browser Tab "${this.tabName}" is not initialized!`);
        }
    ...
    async renderResults(start: number): Promise<HTMLLIElement[]> {
        if (!this.templatePath) {
            throw ErrorXPRPG(`Tab "${this.tabName}" has no valid template path.`);
        }
    ...
    /** Provide a best-effort sort of an object (e.g. CONFIG.XPRPG.monsterTraits) */
