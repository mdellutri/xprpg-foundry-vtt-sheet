...
import { ErrorXPRPG, htmlQuery, tupleHasValue } from "@util";
...
async function applyDamageFromMessage({
    ...
    if (tokens.length === 0) {
        const errorMsg = LocalizeXPRPG.translations.XPRPG.UI.errorTargetToken;
        return ui.notifications.error(errorMsg);
    }

    const shieldBlockRequest = CONFIG.XPRPG.chatDamageButtonShieldToggle;
    const roll = message.rolls.at(rollIndex);
    if (!(roll instanceof DamageRoll)) throw ErrorXPRPG("Unexpected error retrieving damage roll");
    ...
    const originRollOptions = (message.flags.xprpg.context?.options ?? [])
    ...
    for (const token of tokens) {
        await token.actor?.getContextualClone(originRollOptions).applyDamage({
            ...
            rollOptions: new Set(message.flags.xprpg.context?.options ?? []),
            shieldBlockRequest,
        });
    }
...
function shiftAdjustDamage(message: ChatMessageXPRPG, multiplier: number, rollIndex: number): void {
    new Dialog({
        title: game.i18n.localize("XPRPG.UI.shiftModifyDamageTitle"),
        content: `<form>
                <div class="form-group">
                    <label>${game.i18n.localize("XPRPG.UI.shiftModifyDamageLabel")}</label>
...
function toggleOffShieldBlock(messageId: string): void {
    ...
    CONFIG.XPRPG.chatDamageButtonShieldToggle = false;
}
