...
import { ItemXPRPG, PhysicalItemXPRPG } from "@item";
...
import { ErrorXPRPG, sluggify, tupleHasValue } from "@util";
    ...
    listen: ($html: JQuery): void => {
        ...
        $html.find(selectors).on("click", async (event) => {
            ...
            if (strikeAction && action?.startsWith("strike-")) {
                const context = message.flags.xprpg.context;
            ...
            if (item) {
                ...
                // Consumable usage
                    ...
                    if (item.isOfType("consumable")) {
                        ...
                        if (consumable?.isOfType("consumable")) {
                            ...
                            const toReplace = `${consumable.name} - ${LocalizeXPRPG.translations.ITEM.TypeConsumable} (${oldQuant})`;
                            await consumable.consume();
                            const currentQuant = oldQuant === 1 ? 0 : consumable.quantity;
                            let flavor = message.flavor.replace(
                                toReplace,
                                `${consumable.name} - ${LocalizeXPRPG.translations.ITEM.TypeConsumable} (${currentQuant})`
                            );
                            if (currentQuant === 0) {
                                const buttonStr = `>${LocalizeXPRPG.translations.XPRPG.ConsumableUseLabel}</button>`;
                ...
                if (action === "repair-item") {
                    ...
                    const craftingCost = CoinsXPRPG.fromPrice(item.price, quantity);
                    ...
                    if (!(await actor.inventory.removeCoins(coinsToRemove))) {
                        ui.notifications.warn(game.i18n.localize("XPRPG.Actions.Craft.Warning.InsufficientCoins"));
                        return;
                    }
                    ...
                    if (isSpellConsumable(item.id) && item.isOfType("consumable")) {
                        ...
                        ChatMessageXPRPG.create({
                            user: game.user.id,
                            content: game.i18n.format("XPRPG.Actions.Craft.Information.PayAndReceive", {
                    ...
                    ChatMessageXPRPG.create({
                        user: game.user.id,
                        content: game.i18n.format("XPRPG.Actions.Craft.Information.LoseMaterials", {
                            actorName: actor.name,
                            cost: coinsToRemove.toString(),
                            quantity: quantity,
                            itemName: item.name,
