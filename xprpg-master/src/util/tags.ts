...
import { ErrorXPRPG, objectHasKey } from "./misc";
...
function traitSlugToObject(trait: string, dictionary: Record<string, string | undefined>): TraitViewData {
    ...
    if (objectHasKey(CONFIG.XPRPG.traitsDescriptions, trait)) {
        traitObject.description = CONFIG.XPRPG.traitsDescriptions[trait];
    }
...
function tagify(
    ...
    if (input?.hasAttribute("name") && input.dataset.dtype !== "JSON") {
        throw ErrorXPRPG("Usable only on input elements with JSON data-dtype");
    } else if (!input) {
        return null;
    }
...
interface TagifyOptions {
    ...
    maxTags?: number;
    /** A whitelist record, typically pulled from `CONFIG.XPRPG` */
