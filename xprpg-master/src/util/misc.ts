    ...
    switch (camel) {
        ...
        case "dromedary":
            return text
                ...
                .replace(/\s+/g, "");
        default:
            throw ErrorXPRPG("I don't think that's a real camel.");
    }
}
...
function parseHTML(unparsed: string): HTMLElement {
    ...
    if (!(element instanceof HTMLElement)) throw ErrorXPRPG("Unexpected error parsing HTML");

    return element;
}

const actionImgMap: Record<string, ImageFilePath> = {
    0: "systems/xprpg/icons/actions/FreeAction.webp",
    free: "systems/xprpg/icons/actions/FreeAction.webp",
    1: "systems/xprpg/icons/actions/OneAction.webp",
    2: "systems/xprpg/icons/actions/TwoActions.webp",
    3: "systems/xprpg/icons/actions/ThreeActions.webp",
    "1 or 2": "systems/xprpg/icons/actions/OneTwoActions.webp",
    "1 to 3": "systems/xprpg/icons/actions/OneThreeActions.webp",
    "2 or 3": "systems/xprpg/icons/actions/TwoThreeActions.webp",
    reaction: "systems/xprpg/icons/actions/Reaction.webp",
    passive: "systems/xprpg/icons/actions/Passive.webp",
};
...
function getActionIcon(
    ...
    fallback: ImageFilePath | null = "systems/xprpg/icons/actions/Empty.webp"
): ImageFilePath | null {
...
/**
 * Returns a character that can be used with the Xenos Paragon action font
...
function ErrorXPRPG(message: string): Error {
    return Error(`XPRPG System | ${message}`);
}

/** Returns the number in an ordinal format, like 1st, 2nd, 3rd, 4th, etc */
function ordinal(value: number) {
    const suffixes = LocalizeXPRPG.translations.XPRPG.OrdinalSuffixes;
    const pluralRules = new Intl.PluralRules(game.i18n.lang, { type: "ordinal" });
    const suffix = suffixes[pluralRules.select(value)];
    return game.i18n.format("XPRPG.OrdinalNumber", { value, suffix });
}

/** Localizes a list of strings into a comma delimited list for the current language */
function localizeList(items: string[]) {
    const parts = LocalizeXPRPG.translations.XPRPG.ListPartsOr;
