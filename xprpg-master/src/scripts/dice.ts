...
class DiceXPRPG {
    ...
    /**
     * A standardized helper function for managing core XPRPG "d20 rolls"
     *
    ...
    static async d20Roll({
        ...
        const _roll = async (
            ...
            if (adv === 1) {
                rollParts[0] = ["2d20kh"];
                flav = game.i18n.format("XPRPG.Roll.MisfortuneTitle", { title: title });
            } else if (adv === -1) {
                rollParts[0] = ["2d20kl"];
                flav = game.i18n.format("XPRPG.Roll.MisfortuneTitle", { title: title });
            }
        ...
        if (
            ...
            // Render modal dialog
            template = template || "systems/xprpg/templates/chat/roll-dialog.hbs";
            const dialogData = {
                data,
                rollMode,
                formula: parts.join(" + "),
                ...
                new Dialog(
                    {
                        ...
                        buttons: {
                            advantage: {
                                label: game.i18n.localize("XPRPG.Roll.Fortune"),
                                callback: async (html) => {
                                    roll = await _roll(parts, 1, html);
                                },
                            },
                            normal: {
                                label: game.i18n.localize("XPRPG.Roll.Normal"),
                                callback: async (html) => {
                                    roll = await _roll(parts, 0, html);
                                },
                            },
                            disadvantage: {
                                label: game.i18n.localize("XPRPG.Roll.Misfortune"),
                                callback: async (html) => {
                                    roll = await _roll(parts, -1, html);
                                },
                            },
                        },
                        default: game.i18n.localize("XPRPG.Roll.Normal"),
                        close: (html) => {
    ...
    alter(add: number, multiply: number): this {
        ...
        if (this._rolled) throw ErrorXPRPG("You may not alter a Roll which has already been rolled");
