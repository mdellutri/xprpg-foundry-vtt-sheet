...
function CheckFeat(actor: ActorXPRPG, slug: string) {
...
export async function treatWounds(options: ActionDefaultOptions) {
    ...
    if (!actor || !actor.isOfType("creature")) {
        ui.notifications.error("XPRPG.ErrorMessage.NoPCTokenSelected");
        return;
    }
    ...
    const dialog = new Dialog({
        title: game.i18n.localize("XPRPG.Actions.TreatWounds.Label"),
        content: `
<div>${game.i18n.localize("XPRPG.Actions.TreatWounds.Label")}</div>
<hr/>
<form>
<div class="form-group">
<label for="skill-${domIdAppend}">${game.i18n.localize("XPRPG.Actions.TreatWounds.SkillSelect")}</label>
...
<select id="dc-type-${domIdAppend}" name="dc-type">
  <option value="1">${game.i18n.localize("XPRPG.Actions.TreatWounds.DC.Trained")}</option>
  <option value="2">${game.i18n.localize("XPRPG.Actions.TreatWounds.DC.Expert")}</option>
  <option value="3">${game.i18n.localize("XPRPG.Actions.TreatWounds.DC.Master")}</option>
  <option value="4">${game.i18n.localize("XPRPG.Actions.TreatWounds.DC.Legendary")}</option>
</select>
</div>
</form>
<form>
<div class="form-group">
<label for="modifier-${domIdAppend}">${game.i18n.localize("XPRPG.Actions.TreatWounds.DC.Mod")}</label>
...
<label for="risky-surgery-${domIdAppend}">${game.i18n.localize("XPRPG.Actions.TreatWounds.Feats.RiskySurgery")}</label>
...
${
    CheckFeat(actor, "mortal-healing")
        ? `<form><div class="form-group">
<label for="mortal-healing-${domIdAppend}">${game.i18n.localize("XPRPG.Actions.TreatWounds.Feats.MortalHealing")}</label>
...
`,
        buttons: {
            yes: {
                ...
                label: game.i18n.localize("XPRPG.Actions.TreatWounds.Label"),
...
async function treat(
    ...
    if (!skill?.proficient) {
        return ui.notifications.warn(game.i18n.format("XPRPG.Actions.TreatWounds.Error", { name }));
    }
    ...
    const increaseDoS = (locKey: string): DegreeOfSuccessAdjustment => ({
        adjustments: {
            success: {
                label: `XPRPG.Actions.TreatWounds.Rolls.${locKey}`,
    ...
    skill.check.roll({
        ...
        callback: async (_roll, outcome) => {
            const successLabel = outcome ? game.i18n.localize(`XPRPG.Check.Result.Degree.Check.${outcome}`) : "";
            ...
            if (riskySurgery) {
                ChatMessageXPRPG.create({
                    ...
                    flavor: `<strong>${game.i18n.localize("XPRPG.Actions.TreatWounds.Rolls.RiskySurgery")}</strong>`,
            ...
            if (healFormula) {
                ...
                const rollType =
                    outcome !== "criticalFailure"
                        ? game.i18n.localize("XPRPG.Actions.TreatWounds.Rolls.TreatWounds")
                        : game.i18n.localize("XPRPG.Actions.TreatWounds.Rolls.TreatWoundsCriticalFailure");
                ChatMessageXPRPG.create({
                    type: CONST.CHAT_MESSAGE_TYPES.ROLL,
