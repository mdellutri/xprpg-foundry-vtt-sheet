...
import { ChatMessageSourceXPRPG } from "@module/chat-message/data";
...
export async function restForTheNight(options: ActionDefaultOptions): Promise<ChatMessageXPRPG[]> {
    const actors = Array.isArray(options.actors) ? options.actors : [options.actors];
    const characters = actors.filter((a): a is CharacterXPRPG => a?.type === "character");
    if (actors.length === 0) {
        ui.notifications.error(game.i18n.localize("XPRPG.ErrorMessage.NoPCTokenSelected"));
        return [];
    }
    const translations = LocalizeXPRPG.translations.XPRPG.Action.RestForTheNight;
    ...
    const messages: PreCreate<ChatMessageSourceXPRPG>[] = [];

    for (const actor of characters) {
        const actorUpdates: ActorUpdates = { attributes: {}, resources: {} };
        const itemUpdates: EmbeddedDocumentUpdateData<ItemXPRPG>[] = [];
        ...
        // Stamina points
        const staminaEnabled = !!game.settings.get("xprpg", "staminaVariant");
        ...
        for (const slug of reducedConditions) {
            const { name } = game.xprpg.ConditionManager.getCondition(slug);
        ...
        Hooks.callAll("xprpg.restForTheNight", actor);
    }

    return ChatMessageXPRPG.createDocuments(messages);
}
