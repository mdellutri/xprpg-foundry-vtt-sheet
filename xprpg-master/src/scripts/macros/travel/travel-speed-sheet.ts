...
{
  "key": "XPRPG.RuleElement.FlatModifier",
  "selector": "speed",
...
class TravelSpeedSheet extends FormApplication<{}, TravelSpeedSheetOptions> {
    ...
    static override get defaultOptions(): FormApplicationOptions {
        ...
        options.title = game.i18n.localize("XPRPG.TravelSpeed.Title");
        options.template = "systems/xprpg/templates/gm/travel/travel-speed-sheet.hbs";
        options.width = "auto";
    ...
    private actorFormToSheetData(actor: CharacterXPRPG, data: FormActorData): SheetActorData {
        return {
    ...
    private getInitialActorData(actor: CharacterXPRPG): SheetActorData {
    ...
    private formToSheetData(actors: CharacterXPRPG[], data: TravelFormData): SheetData {
        const journey: Trip[] = [
            {
                terrainSlowdown: {
    ...
    private getInitialFormData(actors: CharacterXPRPG[]): SheetData {
 ...
 */
function getHustleMinutes(actors: CharacterXPRPG[]): number {
    return Math.min(
        ...actors.map((actor) => {
...
function hasFeat(actor: CharacterXPRPG, slug: string): boolean {
    return actor.itemTypes.feat.some((feat) => feat.slug === slug);
}

function parseExplorationOptions(actor: CharacterXPRPG): ExplorationOptions {
...
export function launchTravelSheet(actors: CharacterXPRPG[]): void {
