...
type Rewards = Record<OneToFour, CoinsXPRPG>;
...
function buildRewards(...rewards: Coins[]): Rewards {
    ...
    return {
        1: new CoinsXPRPG(trained),
        2: new CoinsXPRPG(expert ?? trained),
        3: new CoinsXPRPG(master ?? expert ?? trained),
        4: new CoinsXPRPG(legendary ?? master ?? expert ?? trained),
    };
}
...
type IncomeForLevel = { failure: CoinsXPRPG; rewards: Rewards };
function getIncomeForLevel(level: number): IncomeForLevel {
    ...
    return {
        failure: new CoinsXPRPG(income.failure),
...
function applyIncomeOptions({ result, options, level, proficiency }: ApplyIncomeOptionsParams): void {
    if (options.useLoreAsExperiencedProfessional) {
        if (result.degreeOfSuccess === DegreeOfSuccess.CRITICAL_FAILURE) {
            ...
            result.rewards = new CoinsXPRPG(getIncomeForLevel(level).failure);
        } else if (result.degreeOfSuccess === DegreeOfSuccess.FAILURE && proficiency !== 1) {
            result.rewards = new CoinsXPRPG(result.rewards).scale(2);
        }
    }
}
...
function earnIncome({ level, days, rollBrief, proficiency, options, dc }: EarnIncomeParams): EarnIncomeResult {
    ...
    const result = { rewards: new CoinsXPRPG(), degreeOfSuccess: degree.value };
    ...
    return {
        rewards: {
            ...
            combined: new CoinsXPRPG(result.rewards).scale(days),
        },
