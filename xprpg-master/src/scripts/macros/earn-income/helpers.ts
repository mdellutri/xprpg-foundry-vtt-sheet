...
function isExperiencedProfessional(actor: CharacterXPRPG) {
...
function degreeOfSuccessLabel(degreeIndex: DegreeOfSuccessIndex): string {
    ...
    return game.i18n.localize(`XPRPG.Check.Result.Degree.Check.${degreeSlug}`);
}

function coinsToString(coins: CoinsXPRPG, degreeOfSuccess: DegreeOfSuccessIndex): string {
...
function chatTemplate(skillName: string, earnIncomeResult: EarnIncomeResult): string {
    ...
    return `
    <div class="xprpg chat-card">
        <header class="card-header flexrow">
            <img src="systems/xprpg/icons/equipment/treasure/currency/gold-pieces.webp" title="Income" width="36" height="36">
...
function postToChat(skillName: string, earnIncomeResult: EarnIncomeResult): Promise<ChatMessageXPRPG | undefined> {
    const content = chatTemplate(skillName, earnIncomeResult);
    return ChatMessageXPRPG.create({
        user: game.user.id,
        content,
...
function isProficiencyWithoutLevel() {
    return game.settings.get("xprpg", "proficiencyVariant") === "ProficiencyWithoutLevel";
}
...
function runEarnIncome({ actor, event, skill, level, days }: RunEarnIncomeParams): void {
    ...
    game.xprpg.Check.roll(
        new game.xprpg.CheckModifier(`Earn Income: ${skill.label}`, skill, []),
