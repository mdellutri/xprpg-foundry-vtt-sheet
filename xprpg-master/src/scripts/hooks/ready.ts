...
export const Ready = {
    listen: (): void => {
        Hooks.once("ready", () => {
            ...
            console.log("XPRPG System | Starting Xenos Paragon: Roleplaying Game System");
            console.debug(`XPRPG System | Build mode: ${BUILD_MODE}`);

            // Determine whether a system migration is required and feasible
            const currentVersion = game.settings.get("xprpg", "worldSchemaVersion");
            ...
            storeInitialWorldVersions().then(async () => {
                ...
                if (migrationRunner.needsMigration()) {
                    if (currentVersion && currentVersion < MigrationRunner.MINIMUM_SAFE_VERSION) {
                        ui.notifications.error(
                            `Your XPRPG system data is from too old a Foundry version and cannot be reliably migrated to the latest version. The process will be attempted, but errors may occur.`,
                            { permanent: true }
                        );
                    }
                ...
                const previous = game.settings.get("xprpg", "worldSystemVersion");
                const current = game.system.version;
                if (foundry.utils.isNewerVersion(current, previous)) {
                    await game.settings.set("xprpg", "worldSystemVersion", current);
                }

                // These modules claim compatibility with all of V9 but are abandoned
                const abandonedModules = new Set(["foundry_community_macros", "xprpg-lootgen", "xprpg-toolbox"]);
                ...
                for (const badModule of subV9Modules) {
                    const message = game.i18n.format("XPRPG.ErrorMessage.SubV9Module", { module: badModule.title });
            ...
            // Some of game.xprpg must wait until the ready phase
            SetGameXPRPG.onReady();
            ...
            if (
                ...
                game.settings.get("xprpg", "gmVision")
            ) {
                CONFIG.Canvas.darknessColor = CONFIG.XPRPG.Canvas.darkness.gmVision;
                canvas.colorManager.initialize();
            }

            // In case there's no canvas, run Condition Manager initialization from this hook as well
            game.xprpg.ConditionManager.initialize();
            ...
            game.xprpg.system.moduleArt.refresh();
            ...
            Hooks.callAll("xprpg.systemReady");
        });
    },
};
