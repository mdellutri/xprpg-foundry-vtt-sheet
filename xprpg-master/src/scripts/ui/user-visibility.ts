...
class UserVisibilityXPRPG {
    ...
    static process($html: HTMLElement | JQuery, options: ProcessOptions = {}): void {
        ...
        const dcSetting = game.settings.get("xprpg", "metagame_showDC");
        ...
        if (hideDC) {
            $saveButtons.each((_idx, elem) => {
                ...
                if (objectHasKey(CONFIG.XPRPG.saves, saveType)) {
                    const saveName = game.i18n.localize(CONFIG.XPRPG.saves[saveType]);
                    elem.innerText = game.i18n.format("XPRPG.SavingThrowWithName", { saveName });
                }
            });
    ...
    static processMessageSender(message: ChatMessageXPRPG, html: HTMLElement): void {
...
export { UserVisibilityXPRPG, UserVisibility };
