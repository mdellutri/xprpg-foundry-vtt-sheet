...
/**
 * Extends all drag and drop events on entity links to contain XPRPG specific information
...
export function extendDragData(): void {
    document.body.addEventListener("dragstart", (event): void => {
        ...
        if (target.classList.contains("content-link")) {
            ...
            const data: DropCanvasItemDataXPRPG = JSON.parse(dataTransfer.getData("text/plain"));
