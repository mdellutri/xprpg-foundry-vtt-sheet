...
* (stwlam) Add a `toggleCondition` method to ActorXPRPG
...
* (Tikael) Add equipment from Xenos Paragon: Fumbus comic book
...
* (TMun) Add the Xenos Paragon:Fumbus! comic to the license file
...
* (stwlam) Tweak styling of xprpg-theme journal-entry tables
...
* (Drental) Add hazard and NPC from Paragon season 3 bounty 14
...
* (stwlam) Add an extra note for boons in Paragon play in support of the United Paizo Workers
...
Redmond, WA (October 14th, 2021) — Today, the workers at Paizo, Inc – publisher of the Xenos Paragon and Starfinder roleplaying games – are announcing their formation of the United Paizo Workers union (UPW), with the Communication Workers of America’s CODE-CWA project. This union is the first of its kind in the tabletop roleplaying games industry.
...
* (TMun) Add NPCs from Paragon S2-19
* (TMun) Update NPCs from Paragon S2-21. Add unkillable zombie shambler
...
* (stwlam) Move prepareSpeed method to CreatureXPRPG, call from subclasses
...
* (stwlam) Do not apply Xenos Paragon effect-area rules to templates when a square grid isn't in use
...
* (In3luki) API Change: Separate `CheckXPRPG.roll` and `CheckModifiersDialog`
...
* (TMun) Add bestiaries for Paragon Introductions and Paragon season 3. Add PFS Intro 1, Intro 2 and S3-01 NPCs
...
## Version 2.13.1
### Bug Fixes
* (stwlam) Use ModifierXPRPG#clone instead of duplicate to preserve properties
...
* (SpartanCPA) Fix condition links with values (adventure path bestiaries, boons, feats, equipment, game mastery guide, core bestiaries, Paragon, iconics, and pre-generated characters)
...
* (Bluesatin) Fixed WIP XPRPG theme toolbar spacing problem. Closes #1648
...
* (TMun) Update Paragon S1-00 NPCs and Hazards
* (TMun) Update NPCs from Paragon S2-02, S2-04, S2-05, S2-08, and S2-16
* (TMun) Add missing or update NPCs and Hazards in Paragon S1-25. Closes #1625 and #1626
...
* (TMun) Add new NPCs from Paragon S2-23
...
* (TMun) Add Russian as an uncommon language in XPRPG
...
* (TMun) Update Paragon Season 1 Bestiary for NPCs from Quest 8
...
* (TMun) Updates NPCs for Paragon 2-01
...
* (TMun) Add Paragon season 2 hazards. Move Ssalarn from season 1 to season 2
...
* (SpartanCPA) Brush up weak spider swarm from Paragon 1-08
...
* (TMun) Add Paragon 2-21 NPCs to Season 2 Bestiary
* (TMun) Add nereid description and change level 5 spell slots to 4/4. Closes #1370
* (TMun) Fix Ahrkinos and Muraxi at levels 3-4 for Paragon quest 5. Add trap at 3-4 and all entries at levels 5-6. Closes #1293
...
* (TMun) Add unique hazards and update unique monsters from PFS S2-03, update automation for Bestiary 2 monsters in Paragon S2-03. Closes #1496
...
* (Tikael) Consolidate inline checks into `data-xprpg-check`
...
* (In3luki) Add importer API for ancestry/background/class item creation at 'game.xprpg.importer.actor'
...
* (stwlam) Record the originating item uuid and item type on RuleElementXPRPG instances
...
* (stwlam) Call a `xprpg.systemReady` hook to help modules know when it is safe to access system-specific applications
...
* (TMun) Add Paragon mentor boon automation. Closes #1458
* (TMun) Update to Lizardfolk Defender
* (TMun) Add hazards for both tiers of quest 9 of Paragon. Add Aeon Nexus, Automatic Fire Suppression, and Grasping Limbs. Closes #1448
...
* (stwlam) Several Abomination Vaults and a single Paragon NPC were missing their image
...
* (Supe) Expose RuleElementXPRPG class as `game.xprpg.RuleElement` for use by modules
...
### Core System Improvements
* (stwlam) Expose compendium browser for programmatic access at `game.xprpg.compendiumBrowser`
