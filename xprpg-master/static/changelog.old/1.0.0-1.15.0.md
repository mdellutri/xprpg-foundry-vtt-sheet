...
* (SpartanCPA) Paragon Season 1-01 bestiary brush-up
...
* (Jaash & SpartanCPA) Add Paragon 2-15 bestiary entries
* (TMun) Add Paragon 2-16 through 2-18 bestiary entries
...
* (Drental) Add Paragon 1-12 submerged shard. Closes #1369
...
* (Jaash) Add Paragon 2-14 bestiary entries
...
* (Jaash & SpartanCPA) Add Paragon S02-13 bestiary entries
...
* (stwlam) Fix XPRPG player settings flag structure and move the Effect Panel setting from a toolbar button to a user setting
...
* (nikolaj-a) Support a `data-xprpg-show-dc` attribute for inline checks
...
* (stwlam) Prevent error from being thrown when a `controlToken` hook is fired before `game.xprpg` is defined
...
* (stwlam) Fix deprecated window-exposed class names (XPRPGModifierType, XPRPGStatisticModifier, XPRPGCheckModifier)
...
* (Chup) Changed Actions tab symbol to the more recognizable Xenos Paragon single action symbol
...
* (nikolaj-a) Replace xprpg-action custom element with data attributes (the actions need to be dragged to the sheet again)
...
* (stwlam) Help the XPRPGQR module find spell attack rolls until the author can properly fix it
...
* (stwlam) Clean up PlayerConfigXPRPG, remove highlightDataUri setting
...
* (Mothringer & nikolaj-a) Add the Xenos Paragon action icons as a styling option in TinyMCE
...
* (stwlam) Update travel speed sheet to not collide with core FormData class and to use XPRPGActor rather than Actor
...
* (stwlam) Rename `prepareData` to `prepareDerivedData` in XPRPGActors, allowing Foundry Actor class to call prepareData
...
* (nikolaj-a) Add a CSS class for the official Xenos Paragon: Roleplaying GameE action icons
...
* (stwlam) Keep XPRPG CONFIG data in its own namespace
...
* (stwlam) Add XPRPGPhysicalItem class with convenience methods
...
### Core System Improvements
* (profounddark) Implement Untrained Improvisation and Xenos Paragon Agent Dedication feats using rule elements
...
* (fryguy) Make the XPRPGItem generic so that it's potentially type-safe in the future.
* (fryguy) Add the same polymorphism factory hack to Items as well as Actors
* (fryguy) Remove character helper class
* (fryguy) Separate XPRPGActor prepareXData functions into their own subclasses instead of having it all in the same file
...
* (SLATE) Clean up compendia, like removing trailing spaces, invalid characters etc:
    * Cleanup xps season 1 bestiary.db
    * Cleanup xps season 2 bestiary.db
    * Cleanup spell effects.db
    * Cleanup spells.db
    * Cleanup the slithering bestiary.db
    * Cleanup paragon society boons.db
    * Cleanup par bestiary.db
...
### Development Improvements
* (@mirkoRainer) Update CONTRIBUTING.md since the xprpg-dev does not exist anymore
...
### Content Changes
* (Kaiser) Immunities of NPCs moved out of custom field for Bestiary 1, Bestiary 2, Plaguestone bestiary, Agents of Edgewatch bestiary, Extinction Curse bestiary, Age of Ashes bestiary, Slithering, and Paragon season 1 and 2
...
This version of the XPRPG system requires at least version of 0.7.5 of Foundry.
...
* (@TMun) Lost Omens: Paragon backgrounds, equipment, feats, and spells
* (@TMun) Paragon
    * PFS Season 1 bestiary
    * PFS Season 2 bestiary
...
* (fryguy) Update the npc sheet to use `data.actionCatgegory` (which is what's editable in the UI) instead of `data.xprpg_updatednpcsheet.npcActionType` (which doesn't seem to be set anywhere).
...
* (@In3luki) Extract individual compendium entries into separate JSON files
    ...
    * Add previously broken entry to xenos-paragon-bestiary-2.db
...
* (@meditatingCybermind) Added fame and reputation tracking for xps
...
### Bug Fixes
* (@abrault) Fix error when xps field is not defined on a character
...
* (@mdizo) Added Open Roads icon to the Paragon tab
...
* (@nikolaj-a) Add Paragon tab on character sheet
...
* (@fryguy1013) Add rollmode parameter to DiceXPRPG.d20Roll so that you can blind roll from a macro
* (@fryguy1013) Add DiceXPRPG to the global scope so other modules can use it
...
* (@iceman76) Compendium updates
    * Give Paragon boons the correct feat type.
    * Remove duplicate Paragon boon.
...
* (@meditatingCybermind & TMun#1625) Added xps/deity boons and curses
...
* (@overdox) Paragon content:
    * Added Rain of Embers Stance feat, from Paragon Quest #2 (closed): Unforgiving Fire pg. 20
    * Added missing feat, Fey Influence from Paragon Scenario #1-17: The Perennial Crown Part 2, The Thorned Monarch pg. 30
...
## Version 1.5.0
### New Features
* (@FS) Added a new compendium pack for Xenos Paragon Bestiary.

![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/0b4a4cf0083f93bce8f68c2b562e24b0/PZO2104.jpg "Xenos Paragon Bestiary")
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/7b2ba2bbda6d7a1b1a27b9499d1a6d36/image.png "Updated Damage Rolls")
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/bf8f3d841e4d0282c7de585999cbf5ca/Annotation_2020-06-08_195934.png "Loot Actor")
...
* (@putt1) Added new immunities, traits and resistances used in Xenos Paragon Bestiary
* (@overdox) Added all deities from Xenos Paragon Lost Omens: Gods & Magic
* (@overdox) Added Abadars Flawless Scale (magic item)
* (@overdox) Added the new languages; Destrachan, D'ziriak, Jistkan, Jyoti used in Xenos Paragon Bestiary
...
* (@hooking) Updated XPRPGCombatTracker to better support FVTT modules
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/3aa0a08dd37f63b3f6db1a0500cf928d/chrome_EaKZDZhXCi.png "New Roll Dialog")
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/8e2f2ba292b358e1e5a2cc98972a499a/image.png "Edit Modifier")
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/83540ffac93f3d93e244ad971b3f86f8/image.png "Fundamental rune support")
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/7007eb7aacb44242d4bd0554e674c554/image.png "Initiative support")
...
![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/a0809641f1497a789a7f7750da001738/image.png "Goblin Warrior")
...
* (@nikolaj-a) Added an optional callback function parameter to the XPRPGCheck roll and roll dialog to allow reacting to the roll result, like setting an initiative result.
...
* (@nikolaj-a) Improved the initiative chat message to show the skill used when players roll for initiative.
  ![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/a28bfcc03746f7dd5b92791e3ca87322/image.png "Initiative message now shows the skill rolled")
* (@nikolaj-a) Updated save modifiers to use armor name as resiliency item bonus label
  ![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/f11fed405dc55ff0ab30f3b08326d64b/image.png "Resiliency item bonus to saves")
* (@nikolaj-a) Added an expandable panel for strike action and outcome descriptions, as well as showing the strike traits.
  ![alt text](https://gitlab.com/hooking/foundry-vtt---xenos-paragon-2e/uploads/03087cab175ddacb1fa9a863788fe72d/image.png "Strike expandable panel")
