...
* (Avagdu) Add Backgrounds from Paragon Year 4 Blog
...
* (kageru) Fix malformed roll expressions in xps-3 bestiary
...
* (In3luki) Support XPRPG inline links in all chat messages
...
* (Vyklade) expand `SpellXPRPG#rollAttack()` method to allow more arguments to be passed from macros
...
* (stwlam) Migrate ToggleProperty rule elements with the older key naming schema ("XPRPG.RuleElement.ToggleProperty")
...
* (stwlam) Fix nameless export of themed xprpg journal sheet
* (stwlam) Fix rendering of vehicle-sheet actions tab
* (stwlam) Ignore token scale in TokenXPRPG#distanceTo
...
* (TMun) Add Sasmira's Xenos Paragon-themed Dice So Nice! textures to the system
...
* (stwlam) Expose `JournalSheetXPRPG` class at `CONFIG.XPRPG.JournalEntry.sheetClass`
...
* (Ehlii) Fix ActorXPRPG#increaseCondition not incrementing values if existing value is between min/max values.
