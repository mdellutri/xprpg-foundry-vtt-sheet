...
*  (FS) Fixed spell heightening for prepared spell casters in the Xenos Paragon Bestiary and Age of Ashes Bestiary.
...
*  (@nikolaj-a) Added (Experimental) Strikes section to the action tab. This section Automatically creates Strike actions for each weapon and unarmed including the XPRPGModifiers rules.
...
## Version 0.577.0
### New Features
*  Added support for Xenos Paragon style measurement templates. Huge thanks to Furysparks and the Xenos Paragon 1e system for allowing us to use thier implementation of this!
...
*  (@nikolaj-a) Added initial support for Class DC to use the XPRPGModifier system.
*  (@nikolaj-a) Added initial support for custom XPRPGModifiers - Provide methods for adding and removing custom modifiers that will properly hook into the modifier stacking rules.
### Content Changes
...
*  (Brault) Fixed the xprpg system implementation distance measure function to correctly measure 5-10-5 distances to match the fix introduced in Foundry 0.5.6.
...
*  (@nikolaj-a)Add support for the already existing perception modifiers as XPRPGModifier, as well as prepare integration with the conditions modifiers.
...
*  v0.569.0: (mdizo#0928) Enabled souremaps in Gulp and edited the system.json to point to the legacy xprpg.scss file.
...
*  v0.569.0: (overdox#9174) added Blood Feast, Entrancing Eyes, Necrotic Radiation spells and Statuette ritual from Xenos Paragon #154 (closed): Siege of the Dinosaurs
...
*  v0.566.0: (yeroon#2590) NPC's now have an Initiative roll of +0.9 if rolled from CombatTracker, as they always go first in XPRPG CRB Rules. (issue#284)
...
*  v0.566.0: (yeroon#2590) Updated the dying, wounded and doomed titles on the charactersheet to explain their effects according the XPRPG Core Rulebook.
...
*  v0.562.0: (yeroon#2590) Bugfix: Conditions are now working again if your xprpg translation file doesn't include the latest translations.
*  v0.562.0: (yeroon#2590) Set-up the structure to modify the Combat Tracker later (XPRPGCombatTracker)
...
*  v0.558.0: (yeroon#2590) New feature: XPRPG conditions as status effects. You can now select XPRPG conditions in the token Status Effect HUD. Left click to add, right click to remove. Shift click stays a toggle to add an overlay icon. Conditions that can have values can be clicked multiple times. Once you exit the HUD, all the current XPRPG conditions will be output to the chat. The status effects are also now linked to the Actor, and will be shown on all Scenes if that Actor has tokens on other scenes as well. In the System Settings you can choose between multiple icon sets. (Note: if the community has other free to use sets, we can easily add these.) You can also disable showing the default FoundryVTT status effects in the System Settings. (PS. This system is planned to be expanded upon in the future to include (de)buffs and other status effects linked to Actions and Feats.)
...
*  v0.557.0: (nikolaj-a) Added an english translation for the XPRPG.TraitFire.
...
*  v0.554.0: (Arshea#4444) Initial implementation of Stamina variant rules for xprpg.
...
*  v0.551.0: (yeroon#2590) Added structure for static content that can be accessed via XPRPG.DB that always need to be loaded (in contrary to things like spells that are loaded into a world from a compendium on a need basis).
...
*  v0.543.0: (overdox#9174) refactored the structure for images to reflect the equipment structure for xprpg.
...
*  v0.537: (yeroon#2590) Updated the modifyTokenAttribute code to correctly apply xprpg shield mechanics when damage is applied to a tokens Shield bar.
...
*  v0.526: Changed the default theme of the xprpg system from Blue / Red / Dark Red to Blue / Grey. Hopefully it's a bit nicer on the eyes now.
...
*  v0.524: Created game.xprpg.convertPackToBase64Embedded(packname) function to assist with converting compendium packs to base64 encoded images.
...
*  v0.504: Made several improvements to the xprpg template to support damage resistances, weaknesses, etc. different speed types and a lot of additional NPC data.
*  v0.500: Added support for Foundry VTT v0.5.0 (please backup your world before updating)
*  v0.500: Added support for localisation and localised most labels in all of the xprpg html temlplates as well as attribute, saves and skill names.
...
*  v0.464: (Trey#9048) Incorporated eslint and started moving the xprpg system to use es6 style modules.
...
*  v0.460: (Trey#9048) Added a new automated deployment process to build, package and publish xprpg system changes. Thanks!!!
...
*  v0.441: Added support for FVTT v0.4.0. Please note this is the minimum support needed to get it working. A lot more work is required to bring the xprpg system in alignment with all changes.
...
*  v0.348: Added the font XenosParagonActions.ttf (thanks u/baughberick (https://www.reddit.com/user/baughberick/))
