...
export class FakeActor {
    _data: ActorSourceXPRPG;

    items: FakeCollection<ItemXPRPG> = new FakeCollection();

    effects: FakeCollection<ActiveEffectXPRPG> = new FakeCollection();

    _itemGuid = 1;

    id: string;

    constructor(data: ActorSourceXPRPG, public options: DocumentConstructionContext<null> = {}) {
        this.id = data._id;
    ...
    prepareData(): void {
        ...
        for (const source of this._data.items) {
            ...
            if (item) {
                ...
                this.items.set(
                    source._id,
                    new FakeItem(source, { parent: this as unknown as ActorXPRPG }) as unknown as ItemXPRPG
                );
            }
        }
    }
    ...
    static async updateDocuments(
        ...
        return updates.flatMap((update) => {
            ...
            const itemUpdates = (update.items ?? []) as DeepPartial<ItemSourceXPRPG>[];
            delete update.items;
            mergeObject(actor.data, update);
            for (const partial of itemUpdates) {
                const source = (actor as any)._data.items.find(
                    (maybeSource: ItemSourceXPRPG) => maybeSource._id === partial._id
                );
    ...
    async updateEmbeddedDocuments(type: string, data: any[]): Promise<void> {
        for (const changes of data) {
            if (type === "Item") {
                const source = this.data.items.find((itemData: ItemSourceXPRPG) => itemData._id === changes._id);
